...
Updated: 2026-08-31
Status: Shelf, Routine, bounded private requests, complete Explore, member-Product OTP, global report helper, deterministic authenticated Ask with dual network/account abuse protection, opt-in order-service notifications, private-safe aggregate service telemetry, and customer sign-in recovery ship. Protected activation, request operating closure, authenticated production evidence, SLO observation, alerts, and recovery remain.
...
The baseline was rechecked against the current Phase 1 candidate on 2026-08-13.
...
| -------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Entry and authentication         | `/me` and every released child route are dynamic and derive a verified customer session. Signed-out entry preserves only `/me`, each released root (`explore`, `shelf`, `routine`, `consult`, `orders`, `notifications`, and `locations`), or the exact allowlisted `/me/product/[slug]?from=home\|explore\|shelf\|routine` intent through OTP. Private Shelf add/request identifiers, nested paths, and external destinations collapse to `/me`. A normal no-session result uses the standard sign-in journey; an unavailable sign-in check shows a plain retry plus the same sign-in action without SDK detail or private data. The recovery state passed desktop and 320 x 700 rendered checks with keyboard focus, a 44-pixel retry target, one main landmark, and no horizontal overflow. Public search, catalogue, product, concern, and `/consult` routes remain account-free.                                                                                                                                                                           | Authenticated production evidence for the complete sign-in-return-new-session-export/delete-sign-out journey remains.                                                                                                                                                                                                                     | `lib/auth/subject.ts`, `lib/customer/access.ts`, `lib/auth/sign-in-intent.ts`, `modules/me/customer-access.test.ts`, `modules/moderation/sign-in-otp-flow.test.ts`                  |
...
| Ask Me and Concerns              | `/me/consult` reuses public `/consult`'s deterministic reviewed safety and guidance authority. Private Concerns and exact active Shelf/Routine products are excluded by default, explicitly selected and previewed per session, then intersected server-side with the authenticated owner's current Concern, Shelf, and Routine context; a required selected-domain read failure stops with a private recoverable response, while an empty selection performs no private read. The local synthetic member derives ownership only from its server fixture, and no transcript is persisted. Only a deterministic clarification outcome may use the bounded AI Gateway classifier to select a missing-detail enum; JeloCare owns the question and all care decisions. Anonymous requests retain the existing network limiter; verified members must pass both that limiter and a separate HMAC-pseudonymized account bucket. Raw account subjects never become limiter identifiers. Real accounts still have no customer-controlled canonical Concern persistence. | Authenticated production smoke, persistent customer-controlled Concerns, active-cell latency/cost evidence, and a separately accepted safety policy before any optional model wording.                                                                                                                                                    | [Ask Jelo](../ASK_JELO_EXPERIENCE.md), `app/api/consult/route.ts`, `lib/customer/consult-context.ts`, `components/me/consult/consult-view.tsx`, `lib/consult/security.ts`           |
...
| Private missing-product requests | A zero-match canonical search can create an owner-isolated draft or pending request with bounded identity fields and an optional private photo. Requests remain separate from saved Shelf counts and canonical/public catalogue truth; customers can inspect, edit within lifecycle limits, change photo consent, and delete them. Intake is atomically bounded to 12 open requests and six active private photos per owner.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | A governed review-to-closure operating path, customer feedback for matched/published outcomes, account-wide request export/clear semantics, protected activation through migration `0036`, and authenticated isolation/photo smoke remain.                                                                                                | `db/migrations/0036_customer_product_requests.sql`, `lib/customer/product-request-service.ts`, `components/me/product-requests/`                                                    |
...
| Saved locations                  | `/me/locations` manages up to eight owner-isolated delivery or billing locations with optimistic edits and hard delete. Guest checkout stays complete with manual entry; signed-in checkout may copy a saved delivery location. Geoapify suggestions are optional, Nigeria-filtered, server-proxied and fail open to manual entry.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Apply migration `0043`, configure provider/rate-limit secrets, and complete two-owner plus guest/signed-in production smoke.                                                                                                                                                                                                              | `db/migrations/0043_customer_saved_locations.sql`, [Smart locations](../commerce/SMART_LOCATIONS.md), [ADR 0017](../adr/0017-private-saved-locations-and-optional-geocoding.md)     |
...
| Resilience and observability     | Me has route-owned loading and retryable error boundaries. Exact offer labels fail closed when current evidence cannot produce a market summary. Authenticated Me reads and mutations schedule environment-separated, enum-only hourly success/failure and latency counters after the response; exposed Shelf, Routine, and Location unavailable states count as route-operation failures. The operator command reads exact hourly hashes and defaults to the rolling 28-day production partition.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Authenticated production evidence, the minimum SLO observation period, alerts, rollback signals, offline/stale recovery, and per-source Concern outage fidelity remain. The current read model collapses an unavailable Concern source to an empty set, which is a successful rendered route read. There is no offline mutation contract. | `lib/customer/private-telemetry.ts`, `modules/me/customer-private-telemetry.test.ts`, `npm run customer:telemetry:report`                                                           |
...
closure before broad launch. Repository-level owner capacity now serializes
intake and fails closed at 12 open requests and six active private photos.
...
at the service boundary under the agreed launch load. Current Me service
telemetry emits only environment-separated aggregate read/write
success/failure and coarse latency counts. It carries no trace ID, auth outcome,
business status, owner, or Shelf contents. Private request fields and photos
stay out of default Operations access, and request/upload limits protect
storage and review capacity. The
...
member release permits only the shared clarification-focus classifier; it
returns an enum and no displayed prose. A later language-only model lane
requires its own accepted ADR, privacy review, abuse/cost ceiling, canary, and
rollback.
...
default and bounded two-model cost policy are accepted. Per-account keying and its
...
fixtures; clarification model failure returns the unchanged deterministic
question; every model call records bounded token, latency, and cost evidence;
deterministic non-clarification response p95 is at most 1.5 seconds at the
service boundary.
...
customer closure. The per-owner request/photo capacity gate is implemented and
must remain part of release verification. Routine
