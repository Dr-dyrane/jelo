...
### Public concern-to-product coverage

Everyday concern pages fetch the current canonical public catalogue and partition products through the reviewed product-care record. The public presentation has two deliberately different tiers:

- **Catalogue matches** contains only `supportive_eligible` products whose reviewed approved use names the exact canonical concern slug and whose product area matches the guide.
- **Reviewed product context** may contain `pharmacist_review` products with the same exact reviewed concern relationship. This section must state that the products are linked by reviewed evidence, are not direct recommendations, and need a pharmacist or clinician suitability check.
