...
The consult route runs deterministic safety and clinical logic before any model call.

- Safety stops return zero products and zero model calls.
- Insufficient detail returns clarification without a model call.
- The model sees only rule-filtered product candidates.
- Displayed care, pattern, routine, and referral copy is built deterministically.
...
- Vercel AI Gateway: model routing for Ask Jelo.
