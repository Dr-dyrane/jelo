...
3. **Browser fetch + structured-data extraction** — headless Chromium via `playwright-core` for hosts that block server-side HTTP (e.g. Jumia/Cloudflare 403). Production uses the serverless runtime's default persistent context because its isolated contexts can close during target creation; anonymous pages are serialized and reset between offers, including pages, cookies, permissions, local storage and session storage. Lazy-loaded so it never affects cold start. Same extraction and confidence rules as HTTP fetch.
