...
4. Nigerian regulatory clearance is a typed, reviewer-attributed NAFDAC record bound to its final authority response representation and the candidate GTIN. Greenbook may bind the manufacturer GTIN directly. For cosmetics verified through NAPAMS by NAFDAC registration number, one reviewed exact-package image must show both the candidate EAN/UPC and that registration number; the separately hash-bound NAPAMS result must repeat the same number, registered product name and an explicit active status. A retailer claim, package number or NAPAMS result cannot pass alone. Matches reject inactive, revoked or expired records; a recorded expiry must still be current. A `not-required` record must quote an authority-sourced product class and rationale. Regulatory evidence expires after 90 days and must be re-observed; `pending`, stale, arbitrary-host and unrelated-scope records remain private.

An unresolved public registry search is also durable evidence, but never clearance. The intake record may retain the exact NAFDAC query, decoded response hash, MIME type, byte size, retrieval and review time, and zero-result counts for 90 days. Its caveat must state that no active public match is not proof of non-registration. It removes no blocker and cannot substitute for a product-specific number, package binding or active authority result.
