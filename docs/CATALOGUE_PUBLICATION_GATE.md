...
3. A reviewer records a formula archetype, care tier, supportive-care scope and explicit advisory boundary. A reviewed record binds the exact official product page plus independent clinical guidance on a different host; manufacturer marketing or Open Beauty Facts ingredients alone are insufficient.
4. Nigerian regulatory status must be matched or explicitly documented as not required. `pending` remains private.
5. Market evidence must use one of two routes: Tier-A identity evidence plus two independent fresh exact Nigerian offers, or brand-confirmed Nigerian authorization plus one fresh exact offer. An offer counts as exact only when the page explicitly labels the same manufacturer number as GTIN, EAN or UPC and the intake records that label basis. A retailer SKU, an unlabeled number or matching digits never substitutes for that evidence.
...
Research can fan out across independent exact-SKU dossiers in parallel. The gates inside each dossier remain sequential and fail-closed, evidence from one SKU cannot satisfy another, and a manifest update is merged only after the source bytes and decision are replayed locally.
