...
The authoritative 2026-07-23T18:00Z pipeline state is 23 reviewed research records, 39 live products, 977 frozen private bulk candidates, 1,000 screened Nigerian retailer leads, 24 deliberate per-SKU intake candidates, 24 canonical identity artifacts, 24 approval-ready candidates, 24 private publication dossiers and 24 explicit releases. The twenty-fourth release is Face Facts Vitamin C Body Lotion 400 ml. The official Face Facts page binds the current variant, size, formula, directions and pump presentation; independent rendered accessibility captures from eBay UK and Apollo Wholesale corroborate EAN `5031413929089`; BuyBetter contributes the current ₦6,095 in-stock observation; and Allure contributes the current ₦8,000 in-stock observation. Its final transparent 2,000 × 2,000 render is content-addressed and manually checked against the official package. The product is linked only to supportive daily moisturising for dry or rough body skin; manufacturer brightening language is not converted into a treatment claim. The chronological release paragraph below retains its earlier count snapshot; `npm run catalogue:pipeline:status` is the source of truth for current totals.
