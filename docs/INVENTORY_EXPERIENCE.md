...
Current derived state is 23 reviewed research records, 15 display-approved public records, 977 frozen private bulk candidates, 1,000 screened Nigerian retailer leads, 12 deliberate per-SKU intake candidates, 12 canonical identity artifacts, 0 approval-ready intake candidates, 0 private publication dossiers and 0 explicit public releases. Seven of the original 14-product dossier now pass the public image gate; the other seven remain in the reviewed catalogue while their exact packshots are improved. All 12 intake candidates have passed identity and bounded care review and are now at Nigerian regulatory and exact-offer evidence. The two exact Aqua Rich 500 ml identities have narrow daily-moisturising reviews; sunscreen and treatment claims remain excluded, and their Nigerian retailer pages remain research observations until those pages expose the exact manufacturer identity. The Garnier Vitamin C Brightening Day Cream 50 ml now has an official GTIN and current BuyBetter and Slique observations, but those retailer pages do not yet bind an exact manufacturer identifier, so no comparison price is published. Dove Melanin Even Tone Body Wash now has an official GTIN, variant and size bound through a hash-reviewed browser DOM capture because the official page rejects unattended HTTP retrieval; its two Nigerian listings remain excluded because one exposes only a retailer SKU and the other publishes a conflicting EAN. KeraCare Dry & Itchy Scalp Anti-Dandruff Moisturizing Conditioner now has its current 32 oz manufacturer GTIN and a bounded 1% pyrithione-zinc review tied to the revisioned DailyMed label; BuyBetter's 950 ml price remains visible only as a private observation because its retailer SKU conflicts with the current official GTIN. The Hydrating Cleanser has one brand-authorized Nigerian offer bound to the printed package EAN in the retailer gallery. Its reviewed public NAFDAC product-name search returned no active match; that hash-bound negative observation is retained with the explicit caveat that no public result is not proof of non-registration. The candidate therefore remains private until a product-specific NAFDAC number and active authority result can be bound. The `publicCatalogueCount` value inside the frozen legacy bulk metadata describes the retired count-first release target; neither that value nor the discovery-lead count is current public inventory or suitable UI copy.

Run `npm run catalogue:pipeline:status` for the current live, discovery, prioritized-research, intake, exact-offer, almost-ready, dossier and explicit-release counts. “Almost ready” is deliberately narrow: the candidate must already have a fresh exact Nigerian offer and no remaining blockers except NAFDAC evidence.

Anonymous community submissions feed a separate research-signal report. With a server database environment loaded, run `npm run community:research:signals` to see aggregate product, retailer, purpose, price and pending-vocabulary signals. The report contains no contributor identity and labels every result `community_reported`; it can change research order but cannot satisfy identity, regulatory, exact-offer, care, rights, image or publication gates.
