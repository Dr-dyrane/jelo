...
- **Runtime lifecycle:** production prewarms one shared default persistent
  context before the batch because the serverless Chromium runtime can close
  isolated contexts during target creation. Browser pages are serialized,
  closed, and have cookies, permissions, local storage and session storage
  cleared between offers. The process retains its executable and profile while
  Chromium is connected and removes both only after disconnect. A replacement
  launch waits for that cleanup, avoiding runtime unlink races inside the
  bounded `/tmp` volume.
