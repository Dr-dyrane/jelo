...
this receipt is
`f3c033880a7b84bc0ae1a67dcc60122da8be778b`. Its exact production deployment
is `dpl_8kxRXJcTnaa4S3zNqx3qLN6FXzik`, which is READY and aliased to
...
| Active customer-data export                      | `f3c033880a7b84bc0ae1a67dcc60122da8be778b` | `dpl_8kxRXJcTnaa4S3zNqx3qLN6FXzik` | The authenticated export contains current Shelf items and active saved Concerns without owner identity, and fails closed if either private source is unavailable. |
...
The customer-data export cell passed 63 focused tests after one bounded dialog
state correction, then passed the full local suite, typecheck, documentation,
migration, catalogue, publication, asset, formatting, and tracked-surface lint
gates. One independent review accepted the final export and controlled-dialog
diff with no findings. The exact clean Vercel build ran 1,842 tests: 1,838
passed, four expected environment tests were skipped, and none failed; every
remaining release verifier and the 507-page production build also passed.

Exact Git deployment `dpl_8kxRXJcTnaa4S3zNqx3qLN6FXzik` cloned
`f3c033880a7b84bc0ae1a67dcc60122da8be778b`, reached READY, and acquired both
canonical aliases. `/`, `/basket`, `/checkout`, `/order`, and `/me/orders`
returned HTTP 200. Signed-out `/me/shelf/export` redirected to the safe
`/sign-in?next=/me` continuation and exposed no JSON. The authenticated account
sheet exposed one separate export action, and native Escape dismissal restored
focus and the collapsed trigger state. A later authorized disposable-account
check found zero private product requests; the exact prior request route was
not found, and sign-out restored the same `/sign-in?next=/me` boundary.
...
| Customer and commerce flows                 | Global care handoffs, governed Ask assessment, server-verified selected context, active Shelf-and-Concern export privacy, compact accessibility, owner isolation, bounded recovery and production route smokes | Released and authorized destructive behavior closed; external or lane-owned gates 6, 11, and 12 remain          |
