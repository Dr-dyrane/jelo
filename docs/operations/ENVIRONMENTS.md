...
| `GITHUB_REPO_BRANCH`            | Static file sync       | Required review branch. Production uses exact `inventory-sync-review`, whose proposals are semantically gated and atomically integrated by GitHub Actions; the wider safe namespace remains proposal-only. |
...
GitHub Actions, not Vercel cron, owns the subsequent idempotent integration;
