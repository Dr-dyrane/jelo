...
test("ranking requires canonical concern slugs instead of raw lexicon concern ids", () => {
  ...
  assert.deepEqual(
    rankProducts(fullCatalogue, { concernSlugs: ["oily-congested-skin"] }).map(
  ...
  assert.deepEqual(
    rankProducts(fullCatalogue, { concernSlugs: ["dry-dehydrated-skin"] }).map(
      (product) => product.slug,
    ),
    [
...
test("skin-type copy cannot authorize a product without a canonical care concern", () => {
  assert.deepEqual(rankProducts(catalogue, { concernSlugs: ["normal"] }), []);
});
