...
test('every dossier-released product has an explicit post-publication care decision', () => {
  ...
  assert.equal(productSlugs.length, 30);
  assert.deepEqual(reviewSlugs, productSlugs);
  assert.equal(states.filter(state => state === 'supportive_eligible').length, 14);
  assert.equal(states.filter(state => state === 'pharmacist_review').length, 10);
