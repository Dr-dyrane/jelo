const ignoredTitleWords = new Set([
  ...
  "at",
  "pot",
  // Descriptive adjectives retailers frequently drop from product titles.
  // These do not distinguish one SKU from a sibling variant of the same line.
  "facial",
  "ageless",
]);
...
function titleTokens(value: string) {
  return Array.from(
    new Set(
      normalizeKnownBrandPhrases(value)
...
function titleMatches(expected: string, observed: string) {
  ...
  return (
    ...
    expectedTokens.every((token) => observedTokens.has(token))
  );
}
...
function sizes(value: string): Measurement[] {
  ...
  for (const match of matches) {
    ...
    if (unit === "ml" || unit.startsWith("millilit"))
      measurements.push({ dimension: "volume", baseValue: amount });
    else if (unit === "cl" || unit.startsWith("centilit"))
      measurements.push({ dimension: "volume", baseValue: amount * 10 });
    else if (unit === "l" || unit.startsWith("lit"))
      measurements.push({ dimension: "volume", baseValue: amount * 1_000 });
      ...
      measurements.push({ dimension: "mass", baseValue: amount / 1_000 });
    else if (unit === "kg" || unit.startsWith("kilogram"))
      measurements.push({ dimension: "mass", baseValue: amount * 1_000 });
    else if (unit === "g" || unit.startsWith("gram"))
      measurements.push({ dimension: "mass", baseValue: amount });
    else if (unit === "oz") {
...
export function assertRetailerResponseScope(input: RetailerResponseScope) {
  ...
  if (!sameProductRoute(requested, response)) {
    throw new Error(
      "Retailer redirected away from the verified product route.",
    );
  }
  ...
  const acceptedTitles = [
    input.expectedTitle,
    ...(input.expectedTitleAliases ?? []),
  ].filter(
    (value, index, titles) => value.trim() && titles.indexOf(value) === index,
  );
  if (
    !acceptedTitles.some((title) => titleMatches(title, input.observedTitle!))
  ) {
    throw new Error(
      "Retailer product title does not match the catalogue product.",
    );
  }
  ...
  if (
    !expectedSizes.some((expected) =>
      observedSizes.some((observed) => sameSize(expected, observed)),
    )
  ) {
    throw new Error(
      "Retailer product size does not match the catalogue product.",
    );
  }
  ...
  if (
    input.currencyCode &&
    expectedCurrency &&
    input.currencyCode !== expectedCurrency
  ) {
    throw new Error(
      `Retailer currency does not match the ${input.marketCode} market.`,
    );
  }
}
