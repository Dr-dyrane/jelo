...
test("unconditional urgent patterns publish a concise action before optional care", async () => {
  ...
  for (const [slug, urgency] of expected) {
    ...
    assert.ok(
      (guide.urgentAction?.guidance.length ?? 0) > 30,
      `${slug} needs a useful first action`,
    );
    assert.doesNotMatch(
      guide.urgentAction?.guidance ?? "",
      /product|browse|shop/i,
      slug,
    );
  }
  ...
  for (const guide of concerns.filter((candidate) => candidate.urgentAction)) {
    assert.equal(
      guide.kind,
      "condition-pattern",
      `${guide.slug} must not manufacture shopping urgency`,
    );
    assert.deepEqual(
      guide.productTerms,
      [],
      `${guide.slug} must remain product-ineligible`,
    );
  }

  const page = await readFile(
    path.join(process.cwd(), "app/(site)/concerns/[slug]/page.tsx"),
    "utf8",
  );
  ...
  assert.match(
    page,
    /concern\.kind === "concern" \?\s*\([\s\S]*<section className="concern-matches"/,
  );
...
test("static urgency stays in parity with representative deterministic referrals", () => {
  ...
  for (const item of cases) {
    ...
    assert.ok(
      guide.kind === "condition-pattern" &&
        guide.clinicalPatternIds.includes(
          assessment.differential.primary?.id ?? "",
        ),
  ...
  const heatAssessment = assessClinicalRoutine(
    "Small prickly bumps appeared after heavy sweating.",
  );
...
test("ingredient and sexual-health copy keeps the audited safety qualifiers beside the action", () => {
  ...
  const adapalene = acne.ingredients.find((item) =>
    item.toLowerCase().includes("adapalene"),
  );
  assert.match(adapalene ?? "", /do not use during pregnancy/i);
  assert.ok(
    acne.sources.some(
      (source) =>
        source.url === "https://www.nhs.uk/conditions/acne/treatment/",
    ),
  ...
  assert.match(
    guidance,
    /avoid sexual contact until assessed and symptoms have resolved/i,
  );
