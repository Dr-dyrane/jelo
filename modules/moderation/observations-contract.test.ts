...
test("the observation queue is complete, oldest-first, and keyset paginated", async () => {
  ...
  assert.match(
    query,
    /order by observation\.created_at asc,\s*observation\.id asc/,
  );
  ...
  assert.match(
    continuation,
    /listPendingObservations\([\s\S]*?safeLimit \+ 1,[\s\S]*?createdAt: afterCreatedAt,[\s\S]*?id:/,
  );
...
test("observation imagery comes only from the exact presentation projection", async () => {
  ...
  assert.doesNotMatch(
    inbox,
    /humanizeRef|row\.product|subject\.image|product-placeholder/,
  );
...
test("observation actions expose only supported audited decisions", async () => {
  ...
  assert.match(
    ...
    /transition\(\s*sql,\s*["']community_observation["']/,
  );
...
test("Activity opens one selected settled observation without adding it to pending work", async () => {
  ...
  assert.match(
    activity,
    /activityObservationHref\(\s*decision\.queue,\s*decision\.targetRef,?\s*\)/,
  );
  ...
  assert.match(
    activityLinks,
    /`\/ops\/observations\?id=\$\{encodeURIComponent\(targetRef\)\}`/,
  );
  assert.match(page, /selectedQueueUuid\(await searchParams\)/);
  assert.match(
    page,
    /findPendingObservation\(sql, selectedId\)[\s\S]*?findSettledObservation\(sql, selectedId\)/,
  );
  ...
  assert.match(
    inbox,
    /name="rationale"[\s\S]*?required[\s\S]*?defaultValue=""/,
  );
  assert.match(
    inbox,
    /data-item-id=\{row\.id\}[\s\S]*?className=\{`\$\{styles\.decideSection\} \$\{observationStyles\.correctionFooter\}`\}/,
  );
  ...
  assert.equal(
    (inbox.match(/className=\{observationStyles\.liveStatus\}/g) ?? []).length,
    1,
  );
  ...
  assert.ok(
    detailScroll !== -1 &&
      detailScrollClose > detailScroll &&
      correctionFooter > detailScrollClose,
  );
  ...
  assert.match(
    inboxStyles,
    /@media \(max-width: 1179px\)[\s\S]*?\.btn\s*\{[\s\S]*?min-height:\s*var\(--touch-min\)/,
  );
  assert.match(inboxStyles, /\.note:focus-visible\s*\{/);
  assert.match(inboxStyles, /\.btn:focus-visible\s*\{/);
  assert.match(
    tabletStyles,
    /form\[data-item-id\] \[data-ops-decision-actions\]/,
  );
  ...
  assert.doesNotMatch(
    pendingCounts,
    /findSettledObservation|approved', 'rejected/,
  );
});
