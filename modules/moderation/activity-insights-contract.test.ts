...
test('Insights only aggregates retained approved notes and suppresses every tiny pattern cell', async () => {
  ...
  assert.match(model, /count\(distinct contribution_id\)::int as mention_count/);
  assert.doesNotMatch(model, /anonymous_user_id|contributor_(?:id|email)|ip_address|user_agent/);
});

test('pattern labels come from the canonical registry or an accepted moderation value', async () => {
  const model = await readSource('lib/moderation/activity-read-model.ts');

  assert.match(model, /jsonb_to_recordset\(\$\{sql\.json\(communityPurposeOptions\)\}/);
  assert.match(model, /jsonb_to_recordset\(\$\{sql\.json\(communityRetailerOptions\)\}/);
  ...
  assert.doesNotMatch(model, /group by purpose ->> 'label'|group by retailer ->> 'label'/);
});
...
test('Insights keeps early community patterns distinct from proof', async () => {
  ...
  assert.match(view, /Early sample · \{number\.format\(community\.approvedNotes\)\} approved notes/);
  assert.match(view, /Community reported/);
  assert.match(view, /do not prove results or retailer trust/);
  assert.match(view, /approvedNoteWindow\(community\.firstNoteAt, community\.lastNoteAt, community\.activeDays\)/);
...
test('Decision history is human-readable while raw references remain secondary', async () => {
  ...
  assert.match(model, /left join products product on product\.slug = event\.product_slug/);
...
test('every triage empty state stays put and offers Insights', async () => {
  const pages = await Promise.all([
    ...
    'vocabulary',
    'retailers',
  ].map(route => readSource(`app/(ops)/ops/${route}/page.tsx`)));
