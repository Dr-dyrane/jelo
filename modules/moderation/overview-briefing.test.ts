...
function queueFacts(
  overrides: Partial<
    Record<OverviewQueueFact["kind"], Partial<OverviewQueueFact>>
  > = {},
): OverviewQueueFact[] {
...
test("overview total is the sum of its visible queue topology", () => {
  const briefing = buildOverviewBriefing({
    queueFacts: queueFacts({
      orders: { pendingCount: 2 },
      contributions: { pendingCount: 2 },
      observations: { pendingCount: 3 },
    }),
    actionableQueueKinds: OVERVIEW_QUEUE_ORDER,
    generatedAt,
  });

  assert.equal(briefing.pendingTotal, 7);
  assert.equal(
...
test("assisted orders join the oldest-actionable policy without a special boost", () => {
  const briefing = buildOverviewBriefing({
    queueFacts: queueFacts({
      orders: { pendingCount: 2, oldestPendingAt: "2026-07-23T12:00:00.000Z" },
      observations: {
        pendingCount: 1,
        oldestPendingAt: "2026-07-22T12:00:00.000Z",
      },
    }),
    actionableQueueKinds: OVERVIEW_QUEUE_ORDER,
    oldestItems: [
  ...
  assert.equal(
    briefing.queues.find((queue) => queue.kind === "orders")?.href,
    "/ops/orders",
  );
  assert.deepEqual(
    briefing.upNext.map((item) => item.queueKind),
    ["observations"],
  );
});

test("assisted orders are recommended only when the operator can manage them", () => {
  const queueState = queueFacts({
    orders: { pendingCount: 1, oldestPendingAt: "2026-07-20T12:00:00.000Z" },
    observations: {
      pendingCount: 1,
      oldestPendingAt: "2026-07-21T12:00:00.000Z",
    },
  });
  const actionable = buildOverviewBriefing({
    queueFacts: queueState,
    actionableQueueKinds: ["orders", "observations"],
    oldestItems: [
      {
        id: "order-1",
        queueKind: "orders",
  ...
  assert.deepEqual(actionable.upNext, []);
  const viewOnly = buildOverviewBriefing({
    queueFacts: queueState,
    actionableQueueKinds: ["observations"],
    generatedAt,
  });

  assert.deepEqual(actionable.nextAction, {
    queueKind: "orders",
    href: "/ops/orders",
  ...
  assert.equal(viewOnly.nextAction?.queueKind, "observations");
  assert.equal(
    viewOnly.queues.find((queue) => queue.kind === "orders")?.operatorCanAct,
    false,
  );
});
...
test("overview recommends the oldest actionable pending queue with a human reason", () => {
  const briefing = buildOverviewBriefing({
    queueFacts: queueFacts({
      observations: {
        pendingCount: 4,
        oldestPendingAt: "2026-07-24T12:00:00.000Z",
      },
...
test("up next shows the first two real records from the recommended queue", () => {
  ...
  assert.deepEqual(
    briefing.upNext.map((item) => [item.id, item.queueKind]),
    [
      ["edge-1", "edges"],
      ["edge-2", "edges"],
    ],
...
test("up next excludes records from a queue the operator cannot decide", () => {
  const briefing = buildOverviewBriefing({
    queueFacts: queueFacts({
      contributions: {
        pendingCount: 1,
        oldestPendingAt: "2026-07-20T12:00:00.000Z",
      },
      observations: {
        pendingCount: 1,
        oldestPendingAt: "2026-07-21T12:00:00.000Z",
  ...
  assert.deepEqual(
    briefing.upNext.map((item) => item.id),
    ["observation-1"],
  );
});
...
test("permissions exclude unavailable decisions from the recommendation but retain viewable topology", () => {
  ...
  assert.equal(briefing.nextAction?.queueKind, "observations");
  assert.equal(
    briefing.queues.find((queue) => queue.kind === "contributions")
      ?.operatorCanAct,
    false,
  );
  assert.equal(
    briefing.queues.find((queue) => queue.kind === "contributions")?.href,
    "/ops/contributions",
  );
});
...
test("recent audit activity is a compact five-entry projection", () => {
  ...
  assert.equal(
    briefing.recentDecisions[0]?.description,
...
test("queue activity is projected independently from the global five-entry window", () => {
  ...
  const contributions = briefing.queues.find(
    (queue) => queue.kind === "contributions",
  );
  assert.equal(
    briefing.recentDecisions.some(
      (decision) => decision.queueKind === "contributions",
    ),
    false,
  );
  assert.equal(contributions?.recentDecisions.length, 1);
  assert.equal(
    contributions?.recentDecisions[0]?.targetLabel,
    "COSRX Snail Mucin",
  );
});
...
test("a positive queue count without waiting age becomes a truthful attention item", () => {
  const briefing = buildOverviewBriefing({
    queueFacts: queueFacts({
      retailers: { pendingCount: 2, oldestPendingAt: null },
    }),
    actionableQueueKinds: OVERVIEW_QUEUE_ORDER,
    generatedAt,
  });
