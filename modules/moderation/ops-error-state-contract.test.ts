...
test("route failures use the shared full-workspace error surface", async () => {
  ...
  assert.match(
    styles,
    /\.errorRetry[\s\S]*border-radius:\s*var\(--ops-control-radius\)/,
  );
  assert.doesNotMatch(
    styles,
    /\.errorRetry[\s\S]*border-radius:\s*var\(--radius-pill\)/,
  );
  assert.match(
    styles,
    /\.emptyAction[\s\S]*border-radius:\s*var\(--ops-control-radius\)/,
  );
  assert.doesNotMatch(
    styles,
    /\.emptyAction[\s\S]*border-radius:\s*var\(--radius-pill\)/,
  );
  ...
  for (const [index, route] of routes.entries()) {
    ...
    assert.doesNotMatch(
      route,
      /\b(?:SQL|UUID|payload|schema|provider|database)\b/i,
    );
  }
});
