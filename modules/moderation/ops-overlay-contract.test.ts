...
test('temporary Ops overlays share one complete interaction contract', async () => {
  ...
  assert.match(hook, /if \(!previousInert\[index\]\) target\.removeAttribute\('inert'\)/);
  assert.match(hook, /event\.key === 'Escape'/);
  assert.match(hook, /event\.key !== 'Tab'/);
  assert.match(hook, /document\.addEventListener\('keydown', handleKeyDown, true\)/);
  assert.match(hook, /document\.removeEventListener\('keydown', handleKeyDown, true\)/);
...
test('Overview uses the shared overlay contract without inventing a second modal system', async () => {
  ...
  assert.match(overview, /import \{ useOpsOverlay \} from '@\/components\/ops\/shell\/use-ops-overlay'/);
  assert.match(overview, /const overlayActive = !isDesktop && overlayOpen/);
  assert.match(overview, /const overlayMounted = overlayActive && detailPortalTarget != null/);
  assert.match(overview, /useOpsOverlay\(\{[\s\S]*open: overlayMounted,[\s\S]*returnFocusRef: lastTrigger/);
  assert.match(overview, /initialFocusSelector: '#queue-inspector-heading'/);
  assert.match(overview, /lastTrigger\.current = trigger/);
  assert.match(overview, /role="dialog"[\s\S]*aria-modal="true"[\s\S]*aria-label=\{`\$\{selectedQueue\?\.label/);
  assert.match(overview, /className=\{styles\.overlayScrim\}[\s\S]*tabIndex=\{-1\}[\s\S]*aria-hidden="true"/);
...
test('queue inspectors use the shared overlay contract and inert every shell plane', async () => {
  ...
  assert.match(inbox, /import \{[\s\S]*OPS_OVERLAY_INERT_TARGETS,[\s\S]*useOpsOverlay/);
  assert.match(inbox, /const overlayMounted = \([\s\S]*usesOverlayInspector[\s\S]*detailPortalTarget != null/);
  assert.match(inbox, /useOpsOverlay\(\{[\s\S]*open: overlayMounted,[\s\S]*returnFocusRef: lastTriggerRef/);
...
test('Ops native dialogs preserve public defaults while owning the Ops scroll plane', async () => {
  ...
  assert.match(hook, /OPS_MODAL_DIALOG_OPTIONS[\s\S]*scrollOwnerSelector: '\[data-ops-main\]'/);
  assert.match(hook, /inertTargetSelectors: OPS_OVERLAY_INERT_TARGETS/);
  assert.match(modal, /interface UseModalDialogOptions/);
  assert.match(modal, /document\.querySelector<HTMLElement>\(scrollOwnerSelector\)/);
  assert.match(modal, /\?\? document\.body/);
  assert.match(modal, /!target\.contains\(element\)/);
  assert.match(modal, /target\.setAttribute\('inert', ''\)/);
  assert.match(modal, /if \(!environment\.previousInert\[index\]\) target\.removeAttribute\('inert'\)/);
  assert.match(operators, /useModalDialog\(OPS_MODAL_DIALOG_OPTIONS\)/);
  assert.equal((operators.match(/useModalDialog\(OPS_MODAL_DIALOG_OPTIONS\)/g) ?? []).length, 2);
...
test('Overview context is a touch bottom sheet, compact side sheet, and one-scroll-owner inspector', async () => {
  ...
  const persistentShellStart = overviewCss.indexOf('@media (min-width: 820px)', touchStart);
  ...
  assert.match(touchRules, /\.overlayInspector\s*\{[\s\S]*height:\s*min\(76dvh,\s*680px\)/);
  assert.match(touchRules, /border-radius:\s*var\(--ops-shell-radius\) var\(--ops-shell-radius\) 0 0/);
  ...
  assert.match(overviewCss, /\.inspectorContent\s*\{[\s\S]*?overflow:\s*hidden/);
