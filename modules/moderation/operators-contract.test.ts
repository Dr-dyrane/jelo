...
test("operator invitations grant nothing until Neon Auth verifies the exact email", async () => {
  ...
  assert.match(
    migration,
    /accepted_operator_id uuid references moderation_operators/,
  );
...
test("operator access changes are admin-only, guarded, and separately audited", async () => {
  const [actions, access, signInUrl, signInPage, mailer, templates] =
    await Promise.all([
  ...
  assert.doesNotMatch(
    signInUrl,
    /NEXT_PUBLIC_SITE_URL|process\.env|localhost|vercel\.app/,
  );
...
test("the Operators route is a title-only split-view workspace with honest states", async () => {
  const [page, directory, loading, error, readModel, css, inbox] =
    await Promise.all([
  ...
  assert.match(
    directory,
    /<\/div>\s*\{accessLifecycleReady \? \(\s*<section className=\{styles\.detailDecision\}>/,
  );
  ...
  assert.match(
    css,
    /\[aria-current=["']true["']\]:focus-visible[\s\S]*outline:\s*2px solid var\(--ops-focus-ring\)/,
  );
  ...
  assert.match(
    css,
    /\.primaryButton[\s\S]*border-radius:\s*var\(--ops-control-radius\)/,
  );
});
