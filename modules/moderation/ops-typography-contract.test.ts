...
test('Ops interface labels stay sentence case without all-caps tracking', async () => {
  const sources = await Promise.all(opsTypographySources.map(async relativePath => ({
    relativePath,
  ...
  for (const { relativePath, source } of sources) {
    ...
    for (const match of source.matchAll(/letter-spacing:\s*([+-]?(?:\d*\.)?\d+)em/gi)) {
      assert.ok(
        Number(match[1]) <= 0,
...
test('Ops eyebrows, subtle section labels, and status pills use regular text', async () => {
  ...
  assert.match(overview, /\.upNextEyebrow\s*\{[\s\S]*?font-weight:\s*var\(--weight-regular\)/);
  assert.match(edges, /\.relationshipKicker\s*\{[\s\S]*?font-weight:\s*var\(--weight-regular\)/);
  assert.match(observations, /\.featureEyebrow\s*\{[\s\S]*?font-weight:\s*var\(--weight-regular\)/);
  assert.match(contributions, /\.featureEyebrow\s*\{[\s\S]*?font-weight:\s*var\(--weight-regular\)/);
  assert.match(vocabulary, /\.targetResultsHeading\s*\{[\s\S]*?font-weight:\s*var\(--weight-regular\)/);
  assert.match(activity, /\.detailSection h3\s*\{[\s\S]*?font-weight:\s*var\(--weight-regular\)/);
  assert.match(operators, /\.featureEyebrow\s*\{[\s\S]*?font-weight:\s*var\(--weight-regular\)/);
  assert.match(operators, /\.detailSection > h3,[\s\S]*?font-weight:\s*var\(--weight-regular\)/);
  assert.match(signals, /\.eyebrow\s*\{[\s\S]*?font-weight:\s*var\(--weight-regular\)/);
  assert.match(inbox, /\.detailEyebrow,[\s\S]*?font-weight:\s*var\(--weight-regular\)/);
  assert.match(chips, /\.pill\s*\{[\s\S]*?font-weight:\s*var\(--weight-regular\)/);
});

test('Ops JSX copy has no hard-coded all-caps interface labels', async () => {
  const sources = await Promise.all(opsLiteralSources.map(async relativePath => ({
    relativePath,
    source: await readSource(relativePath),
  })));
