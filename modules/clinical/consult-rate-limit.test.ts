...
test("consult rate limiting fails closed in production when configuration is absent", async () => {
  const decision = await checkConsultRateLimit(
    request,
    {},
    {
      limiter: null,
...
test("consult rate limiting preserves provider allow and deny decisions", async () => {
  ...
  const allow = await checkConsultRateLimit(
    request,
    {},
    {
      limiter: {
  ...
  const deny = await checkConsultRateLimit(
    request,
    {},
    {
      limiter: {
...
test("consult rate limiting calculates a bounded Retry-After from the provider reset", async () => {
  const now = 1_000_000;
  const future = await checkConsultRateLimit(
    request,
    {},
    {
      limiter: {
  ...
  const elapsed = await checkConsultRateLimit(
    request,
    {},
    {
      limiter: {
...
test("authenticated consults require opaque network and account buckets", async () => {
  ...
  assert.equal(
    identifiers.some((identifier) => identifier.includes(accountSubject)),
    false,
  );
});
...
test("authenticated consults deny on either bucket and use the longest applicable retry", async () => {
  const now = 1_000_000;
  const networkDenied = await checkConsultRateLimit(
    request,
    {
    ...
    },
    {
      limiter: {
        limit: async (identifier) => ({
          ...
          reset: identifier.startsWith("consult:account:")
            ? now + 2_001
            : now + 6_001,
        }),
      },
  ...
  const accountDenied = await checkConsultRateLimit(
    request,
    {
  ...
  const bothDenied = await checkConsultRateLimit(
    request,
    {
...
test("authenticated consults fail closed when either bucket provider call fails", async () => {
  ...
  const accountFailure = await checkConsultRateLimit(
    request,
    {
    ...
    },
    {
      limiter: {
        limit: async (identifier) => {
          if (identifier.startsWith("consult:account:"))
            throw new Error("provider unavailable");
  ...
  const networkFailure = await checkConsultRateLimit(
    request,
    {
...
test("the route derives optional account authority before limiting and body parsing", () => {
  ...
  const auth = route.indexOf("await getCustomerIdentityResult()");
  ...
  assert.match(
    route,
    /customerResult\.status === ["']authenticated["'][\s\S]*?customerResult\.identity\.subject/,
  );
