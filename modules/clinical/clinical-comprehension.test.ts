...
const primitives = readFileSync('components/clinical/clinical-primitives.tsx', 'utf8');
const primitivesCss = readFileSync('components/clinical/clinical-primitives.module.css', 'utf8');
const ingredientExplorer = readFileSync('components/ingredients/ingredient-explorer.tsx', 'utf8');
const ingredientPage = readFileSync('app/(site)/ingredients/page.tsx', 'utf8');
const decisionSummary = readFileSync('components/products/product-decision-summary.tsx', 'utf8');
...
test('clinical primitives are data-gated and render nothing on empty input', () => {
  ...
  assert.match(primitives, /if \(hideUnknown && status === 'unknown'\) return null/);
...
test('ingredient explorer uses shared clinical primitives', () => {
  assert.match(ingredientExplorer, /from '@\/components\/clinical\/clinical-primitives'/);
...
test('ingredient page enriches cards with clinical knowledge from the core module', () => {
  assert.match(ingredientPage, /from '@\/modules\/clinical\/core\/ingredients'/);
...
test('product decision summary uses shared EvidenceGradeBadge and ClinicalCaution', () => {
  assert.match(decisionSummary, /from '@\/components\/clinical\/clinical-primitives'/);
...
test('product decision summary surfaces the strongest caution from care review state', () => {
  ...
  assert.match(decisionSummary, /Ingredient-level evidence, not a formula guarantee/);
});

test('product decision summary shows evidence sources only when care review provides them', () => {
  assert.match(decisionSummary, /evidenceSourceUrls && evidenceSourceUrls\.length > 0/);
...
test('concern page uses shared ReviewedOn and SourceList primitives', () => {
  assert.match(concernPage, /from '@\/components\/clinical\/clinical-primitives'/);
...
test('concern page preserves the clinical content grammar order', () => {
  ...
  assert.ok(sourcesIdx < matchesIdx, 'Sources must render before product matches');
});
...
test('concern page preserves urgent action rendering before signs and optional care', () => {
  ...
  assert.ok(
    concernPage.indexOf('concern-urgent-action') < concernPage.indexOf('concern-detail-grid'),
    'The immediate action must render before signs and optional care.',
  );
  assert.match(concernPage, /concern\.kind === 'concern' \? <section className="concern-matches"/);
});
...
test('no unsupported clinical claim appears through fallback logic', () => {
  ...
  assert.ok(unknownCount === 0, 'Ingredient explorer must not hardcode "unknown" as display text');
