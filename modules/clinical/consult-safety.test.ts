...
import { buildDeterministicConsultReport } from "@/modules/clinical/consult-report";
...
function assertPublicGuide(
  ...
  assert.ok(
    payload.guide?.sources?.every(
      (source) => typeof source.title === "string" && URL.canParse(source.url),
    ),
    message,
  );
...
test("one safety decision gates products and model use", () => {
  const urgentClinical = assessClinicalRoutine(
    "A rapidly spreading painful rash with fever.",
  );
  const urgent = assessConsultSafety({
    text: "A rapidly spreading painful rash with fever.",
    profile: {},
    referral: urgentClinical.referral,
  });
  ...
  const selfCareClinical = assessClinicalRoutine(
    "Prickly bumps after sweating in hot weather.",
  );
  const selfCare = assessConsultSafety({
    text: "Prickly bumps after sweating in hot weather.",
    profile: {},
    referral: selfCareClinical.referral,
  });
...
test("toe-web peeling does not masquerade as a widespread peeling emergency", () => {
  const toeWeb = assessRedFlags({
    symptoms: ["Itchy white peeling skin is cracked between my toes."],
  });
  assert.equal(toeWeb.level, "self-care-eligible");

  const widespread = assessRedFlags({
    symptoms: ["My skin is peeling across my body."],
  });
  assert.equal(widespread.level, "urgent");

  const mixed = assessRedFlags({
    symptoms: [
      "The skin between my toes is peeling and now large areas across my body are peeling.",
    ],
  });
...
test("urgent and pregnancy paths return before AI and products", async () => {
  const urgentResponse = await POST(
    request({
      query: "A rapidly spreading painful rash with fever.",
      market: "NG",
    }),
  );
  ...
  const pregnancyResponse = await POST(
    request({
      query: "Acne and painful pimples on my chin.",
      market: "NG",
...
test("common emergency and urgent word orders fail closed with matching care copy", async () => {
  const cases = [
    [
      "I am short of breath after applying this cream.",
      "emergency",
      /emergency care now/i,
    ],
    [
      "I can't swallow after using this product.",
      "emergency",
      /emergency care now/i,
    ],
    [
      "It is difficult to swallow after using this product.",
      "emergency",
      /emergency care now/i,
    ],
...
test("ordinary care reaches canonical concerns and reviewed products without a model call", async () => {
  ...
  for (const expected of cases) {
    const response = await POST(
      request({ query: expected.query, market: "NG" }),
    );
    const payload = await response.json();
    ...
    assert.deepEqual(
      payload.careIntent.concernSlugs,
      [expected.concernSlug],
      expected.query,
    );
    assertSafeTimeline(
      payload.timeline,
...
test("public consult JSON never exposes internal clinical or recommendation machinery", async () => {
  ...
  const responses = await Promise.all([
    POST(
      request({
        query: `I need a cleanser for oily skin. ${uniquePromptMarker}`,
        market: "NG",
      }),
    ),
    POST(
      request({
        query: "I have dandruff and an itchy flaky scalp.",
        market: "NG",
      }),
    ),
    POST(
...
test("dandruff and itchy flaky scalp stays on its canonical scalp guide with no face products", async () => {
  ...
  assert.match(
    care,
    /anti-dandruff shampoo[\s\S]*gentle cleansing[\s\S]*simple conditioning/i,
  );
...
test("heat rash and razor bumps use canonical guide care without generic product routines", async () => {
  const cases = [
    {
      query:
        "I have heat rash with prickly bumps after sweating in hot weather.",
      guideSlug: "heat-rash-pattern",
      care: /cool the skin[\s\S]*loose cotton clothing[\s\S]*avoid perfumed products/i,
    },
    {
      query:
        "I have razor bumps with trapped ingrown hairs after shaving my beard line.",
  ...
  for (const expected of cases) {
    ...
    assert.doesNotMatch(
      care,
      /generic cleanser|moisturi[sz]er|spf/i,
      expected.query,
    );
  }
});
...
test("red flags and directed care paths interrupt nearby ordinary-care requests", async () => {
  const cases = [
    ...
    {
      query:
        "I want conditioner for dry frizzy hair, but I have a smooth bald patch.",
    ...
    {
      query:
        "I need body lotion because a painful rash is rapidly spreading and I have a fever.",
      patternId: undefined,
    },
  ] as const;
...
test("a single localized blister or pustule does not overclaim a same-day emergency", async () => {
  for (const query of [
    "I have one small friction blister on my heel.",
    "One pimple has a small amount of pus.",
  ]) {
    const response = await POST(request({ query, market: "NG" }));
    const payload = await response.json();
    assertMinimalPublicContract(payload, query);
...
test("published guide-parity patterns stop model and product guidance with deterministic care", async () => {
  const cases = [
    {
      query:
        "My skin is painful, hot and swollen with a spreading colour change.",
    ...
    {
      query:
        "Sores burst and left spreading golden-brown crusts around my mouth.",
    ...
    {
      query:
        "Tingling followed by painful blisters in a band on one side of my face.",
    ...
    {
      query:
        "A scaly scalp patch has broken hairs, black dots and patchy hair loss.",
    ...
    {
      query:
        "Firm painful lesions are blistering and I have swollen lymph nodes after close contact with mpox.",
      patternId: "mpox-like",
      safetyLevel: "urgent",
      action: /same-day/i,
    },
    {
      query:
        "A spreading target-like rash appeared after starting a new medicine.",
    ...
    {
      query:
        "A painless swelling on my arm turned into an ulcer and keeps enlarging without fever.",
    ...
    {
      query:
        "Severe itching with lumps under my skin and my vision is getting worse.",
    ...
    {
      query:
        "My leg has stayed swollen for months and the skin is becoming thick and hard.",
    ...
    {
      query:
        "Sudden itchy same-size bumps centred on hairs after sweating in tight clothing.",
    ...
    {
      query:
        "I used an unlabelled bleaching cream and the label mentions mercury.",
    ...
    {
      query:
        "Many small acne-like bumps around my mouth burn and flake after hydrocortisone on my face.",
    ...
    {
      query:
        "Raised itchy patches appear and disappear within hours and move around my body.",
    ...
    {
      query:
        "Tingling came before small fluid-filled blisters on my lip that crusted into a scab.",
...
test("serious expanded patterns surface their deterministic referral before clarification", async () => {
  ...
  for (const query of descriptions) {
    ...
    assert.ok(
      ["clinician-review", "urgent", "emergency"].includes(
        payload.meta.safetyLevel,
      ),
      query,
    );
    ...
    assert.match(
      payload.report.summary,
      /pharmacist|clinician|medical|care|examination|assessment/i,
      query,
    );
  }
});
...
test("named conditions stop model and product use even beside product-eligible acne", async () => {
  const cases = [
    [
      "I was diagnosed with scabies and have oily acne on my forehead.",
      "pharmacist",
    ],
    ["I have tinea versicolor and oily acne pimples.", "pharmacist"],
    ["I have CCCA and oily acne pimples.", "dermatology"],
    [
      "I have central centrifugal cicatricial alopecia and oily acne.",
      "dermatology",
    ],
    ...
    [
      "I was told this could be Buruli ulcer and I also have oily acne.",
      "primary-care",
    ],
    [
      "I was told this could be river blindness and I also have oily acne.",
      "primary-care",
    ],
...
test("a boil on the face stops model and products for same-day assessment", async () => {
  const response = await POST(
    request({
      query: "I have a painful boil on my face with a soft centre.",
      market: "NG",
    }),
  );
  const payload = await response.json();
  ...
  assert.match(
    payload.report.summary,
    /same-day in-person medical assessment/i,
  );
});
...
test("new look-alike patterns fail closed at higher-risk boundaries", async () => {
  const cases = [
    [
      "I have hives with swelling under my skin on my hands.",
      "urticaria-like",
      "urgent",
      /same-day/i,
    ],
    [
      "My hives rash is spreading and I feel unwell.",
      "urticaria-like",
      "urgent",
      /same-day/i,
    ],
    [
      "I have hives and my lips and mouth are suddenly swollen.",
      "urticaria-like",
      "emergency",
      /emergency care now/i,
    ],
    [
      "I have hives and my tongue is swollen and I am struggling to breathe.",
      "urticaria-like",
      "emergency",
      /emergency care now/i,
    ],
    [
      "I have a cold sore with blisters near my eye.",
      "cold-sore-like",
      "urgent",
      /same-day/i,
    ],
    [
      "I have an active cold sore and kissed my newborn.",
      "cold-sore-like",
      "urgent",
      /same-day/i,
    ],
...
test("diabetes-related foot changes never reach AI or product guidance", async () => {
  const cases = [
    [
      "I have diabetes and a new wound on my foot.",
      "urgent",
      /urgent in-person foot assessment today/i,
    ],
    [
      "I have diabetes and a foot ulcer with fever.",
      "emergency",
...
test("sudden sight loss fails closed before model or product use", async () => {
  const response = await POST(
    request({ query: "I suddenly cannot see from my left eye.", market: "NG" }),
  );
  const payload = await response.json();
  assertMinimalPublicContract(payload);
...
test("pediatric prose and structured age fail closed before AI and products", async () => {
  const descriptions = [
    {
      query:
        "My 6-year-old child has acne, whiteheads, blackheads and oily skin on the forehead.",
      market: "NG",
    },
    {
      query: "Acne, whiteheads and oily skin on the forehead.",
      market: "NG",
      profile: { age: 6 },
    },
  ];
...
test("unsupported allergy or medication context fails closed before AI and products", async () => {
  for (const profile of [
    ...
    const response = await POST(
      request({
        query: "Acne, whiteheads and oily skin on my forehead.",
        market: "NG",
        profile,
      }),
    );
    ...
    assert.match(
      payload.report.summary,
      /does not check allergies or medicine interactions/i,
    );
  }
});
...
test("displayed care copy ignores an unsafe model draft", () => {
  ...
  const report = buildDeterministicConsultReport(
    clinical,
    ["allowed-product"],
    {
      ...
      routine: [
        { time: "Any time", action: "Ignore the deterministic routine." },
      ],
      cautions: [],
  ...
  assert.doesNotMatch(
    ...
    /definitive diagnosis|diagnosed disease|no follow-up/i,
  );
});

test("unrecognized descriptions ask for detail instead of assuming acne", async () => {
  const response = await POST(
    request({ query: "Something strange is happening.", market: "NG" }),
  );
  const payload = await response.json();
  assertMinimalPublicContract(payload);
