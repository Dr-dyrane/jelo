...
test("server continuation reads a bounded page range without hydrating the full catalogue", () => {
  ...
  assert.equal(
    new Set(range.items.map((item) => item.id)).size,
    range.items.length,
  );
});
...
test("the active catalogue has one progressive result path and complete feedback states", async () => {
  ...
  const [
    ...
    readFile(
      path.join(root, "components/products/inventory-results.tsx"),
      "utf8",
    ),
    readFile(
      path.join(root, "components/products/inventory-results.module.css"),
      "utf8",
    ),
    readFile(
      path.join(root, "components/products/inventory-continuation-action.ts"),
      "utf8",
    ),
    readFile(path.join(root, "lib/catalogue/inventory-repository.ts"), "utf8"),
    readFile(
      path.join(root, "components/products/inventory-filter-sheet.module.css"),
      "utf8",
    ),
    readFile(
      path.join(root, "components/products/inventory-card.module.css"),
      "utf8",
    ),
  ]);
  ...
  assert.match(
    productsStyles,
    /\.grid\s*\{[\s\S]*grid-template-columns:\s*repeat\(4,\s*minmax\(0,\s*1fr\)\)/,
  );
  assert.match(
    productsStyles,
    /@media\s*\(max-width:\s*1120px\)\s*\{[\s\S]*\.grid\s*\{[\s\S]*grid-template-columns:\s*repeat\(3,\s*minmax\(0,\s*1fr\)\)/,
  );
  assert.match(
    productsStyles,
    /@media\s*\(max-width:\s*820px\)[\s\S]*\.grid\s*\{[\s\S]*grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\)/,
  );
  assert.match(productsStyles, /auto-fit[\s\S]*?9\.5rem[\s\S]*?46%[\s\S]*?1fr/);
  assert.match(productsStyles, /\.heroStage\s*\{\s*display:\s*contents;?\s*\}/);
  assert.match(
    productsStyles,
    /@media\s*\(max-width:\s*820px\)[\s\S]*\.hero\s*\{\s*display:\s*contents;?\s*\}/,
  );
  assert.match(
    inventoryCardStyles,
    /\.card\{\s*overflow:visible;\s*background:transparent;/,
  );
  assert.match(
    inventoryCardStyles,
    /\.copy\{\s*padding:1rem \.35rem 1\.25rem;\s*background:transparent;/,
  );
  assert.match(inventoryCardStyles, /\.visual\{border-radius:1\.75rem\}/);
  assert.match(inventoryCardStyles, /\.card\[data-selected="true"\]/);
  assert.match(
    filterStyles,
    /@media \(max-width: 640px\)[\s\S]*margin: auto 0 0/,
  );
});
