...
type ExternalOffer =
  ExternalCatalogueApproval["nigeria"]["exactOffers"][number];
...
function withOfferEvidence<T extends ExternalOffer>(offer: T): T {
  ...
  const evidence: ReviewedExactOfferEvidence = {
    ...
    reviewedAt: new Date(
      Date.parse(offer.observedAt) + 5 * 60_000,
    ).toISOString(),
  };
...
function candidate(
  overrides: Partial<ExternalCatalogueGateCandidate> = {},
): ExternalCatalogueGateCandidate {
  return {
    ...
    sourceImageUrl:
      "https://images.openbeautyfacts.org/images/products/400/580/831/9695/front_en.1.400.jpg",
...
function approval(
  item: ExternalCatalogueGateCandidate,
  overrides: Partial<ExternalCatalogueApproval> = {},
): ExternalCatalogueApproval {
  return {
    ...
    nigeria: {
      ...
      exactOffers: [
        withOfferEvidence({
          ...
          listingUrl: "https://beautybydaz.com/product/example",
          observedAt: "2026-07-22T08:00:00Z",
          variant: item.name,
          size: item.quantity,
          observedGtin: item.barcode,
        ...
        withOfferEvidence({
          retailer: "Teeka4",
          listingUrl: "https://teeka4.com/product/example",
...
test("the legacy approval schema fails closed after exact-offer evidence changed", () => {
  assert.throws(
    () =>
      gateExternalCatalogue(
        [candidate()],
        { schemaVersion: 1, approvals: [] } as never,
        asOf,
      ),
...
test("an identity-bound licensed original photograph can pass the public gate", () => {
  const item = candidate();
  const decision = evaluateExternalCatalogueCandidate(
    item,
    approval(item),
    asOf,
  );
...
test("a magazine-ready styled composite can pass when the package remains exact and manually checked", () => {
  const item = candidate({
    imageTreatment: "identity-verified-styled-composite",
  });
  ...
  assert.equal(
    evaluateExternalCatalogueCandidate(item, styled, asOf).status,
    "approved",
  );
});
...
test("approval cannot survive a candidate or image change", () => {
  ...
  assert.equal(
    evaluateExternalCatalogueCandidate(changed, stale, asOf).reason,
...
test("regulatory status is context while insufficient Nigerian offers remain private", () => {
  ...
  assert.equal(
    evaluateExternalCatalogueCandidate(item, pending, asOf).reason,
    "approved",
  );
  assert.equal(
    evaluateExternalCatalogueCandidate(item, oneOffer, asOf).reason,
...
test("brand-confirmed Nigerian authorization can support one fresh exact offer", () => {
  ...
  const brandRoute: ExternalCatalogueApproval = {
    ...
    sku: {
      ...base.sku,
      evidenceUrl:
        "https://uk.lorealdermatologicalbeautypartnershop.com/on/demandware.static/catalogue.pdf",
    },
    nigeria: {
      ...base.nigeria,
  ...
  assert.equal(
    evaluateExternalCatalogueCandidate(item, brandRoute, asOf).status,
    "approved",
  );
});
...
test("brand authorization is bound to both the official brand host and the named seller", () => {
  ...
  const spoofedAuthorization: ExternalCatalogueApproval = {
    ...
    nigeria: {
      ...
      tierAIdentityEvidenceUrl: undefined,
      brandAuthorizationEvidenceUrl:
        "https://cerave.attacker.example/nigeria/authorized",
  ...
  assert.equal(
    evaluateExternalCatalogueCandidate(item, spoofedAuthorization, asOf).reason,
    "nigeria-market-evidence-insufficient",
  );
  assert.equal(
    evaluateExternalCatalogueCandidate(item, spoofedSkuSource, asOf).reason,
    "exact-sku-not-approved",
  );
  assert.equal(
    evaluateExternalCatalogueCandidate(item, unboundSeller, asOf).reason,
    "nigeria-market-evidence-insufficient",
  );
});
...
test("Tier-A market evidence must reuse the exact SKU identity source", () => {
  ...
  const unbound: ExternalCatalogueApproval = {
    ...
    nigeria: {
      ...base.nigeria,
      tierAIdentityEvidenceUrl:
        "https://identity.example/products/4005808319695",
    },
  };

  assert.equal(
    evaluateExternalCatalogueCandidate(item, unbound, asOf).reason,
    "nigeria-market-evidence-insufficient",
  );
});
  ...
  const retailerSku: ExternalCatalogueApproval = {
    ...
    nigeria: {
      ...
      exactOffers: base.nigeria.exactOffers.map((offer) => ({
        ...offer,
        observedGtinBasis:
          "retailer-sku" as ExternalCatalogueApproval["nigeria"]["exactOffers"][number]["observedGtinBasis"],
      })),
    },
  };

  assert.equal(
    evaluateExternalCatalogueCandidate(item, siblingGtin, asOf).reason,
    "nigeria-market-evidence-insufficient",
  );
  assert.equal(
    evaluateExternalCatalogueCandidate(item, retailerSku, asOf).reason,
    "nigeria-market-evidence-insufficient",
  );
});
...
test("legacy exact offers require the same reviewed representation and explicit label evidence", () => {
  ...
  assert.equal(
    evaluateExternalCatalogueCandidate(item, bare, asOf).reason,
    "nigeria-market-evidence-insufficient",
  );
  assert.equal(
    evaluateExternalCatalogueCandidate(item, skuLabel, asOf).reason,
    "nigeria-market-evidence-insufficient",
  );
});
...
test("legacy approvals cannot predate bound market or regulatory evidence", () => {
  ...
  assert.equal(
    evaluateExternalCatalogueCandidate(item, early, asOf).reason,
    "invalid-approval",
  );
});
