...
function context(
  ...
  const facets: InventoryResult['facets'] = {
    ...
    concerns: [
      { value: 'acne-breakouts', label: 'Acne & breakouts', count: 2, total: 4 },
      { value: 'atopic-eczema-pattern', label: 'Eczema-like pattern', count: 0, total: 0 },
    ],
    reviewed: 10,
    supportive: 2,
    community: 0,
...
test('query context stays deterministic and never promotes concern language into product matching', () => {
  ...
  const moisturiser = inventoryRefinementPlan(context({ q: 'moisturiser' }, 'category'));
...
test('keeps every active group visible even when its current count is zero', () => {
  ...
  current.total = 0;
  current.facets.categories = current.facets.categories.map(facet => ({ ...facet, count: 0 }));
  current.facets.brands = [];
  current.facets.steps = current.facets.steps.map(facet => ({ ...facet, count: 0 }));
  current.facets.concerns = current.facets.concerns.map(facet => ({ ...facet, count: 0 }));
  ...
  assert.deepEqual(new Set(plan.primary), new Set([
    'category',
    'routine',
    'company',
    'concern',
...
test('the disclosure and nested choices preserve keyboard and focus affordances', async () => {
  ...
  assert.match(source, /id="catalogue-all-refinements"[\s\S]*hidden=\{!showAllRefinements\}/);
  assert.ok(source.indexOf('aria-controls="catalogue-all-refinements"') < source.indexOf('id="catalogue-all-refinements"'));
  assert.ok((source.match(/setShowAllRefinements\(false\)/g) ?? []).length >= 2);
  assert.match(source, /companySearchRef\.current\?\.focus\(\)/);
  assert.match(source, /previous === 'concerns' \? concernButtonRef\.current\?\.focus\(\) : companyButtonRef\.current\?\.focus\(\)/);
