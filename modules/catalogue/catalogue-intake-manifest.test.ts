...
test('checked-in canonical identity artifacts match every declared byte and hash', async () => {
  assert.equal(await verifyCatalogueIdentityEvidenceArtifacts(catalogueIntakeCandidates), 47);
});

test('the deliberate intake cohort exposes readiness without treating NAFDAC as a gate', () => {
  assert.equal(catalogueIntakeCandidates.length, 47);
  assert.equal(catalogueIntakeDecisions.length, 47);
  ...
  assert.equal(catalogueIntakeDecisions.filter(decision => decision.stage === 'nigeria').length, 43);
...
test('the Beauty Formulas serum binds its exact formula, Nigerian prices and reviewed render', () => {
  ...
  assert.match(candidate.care.advisoryBoundary ?? '', /Vitamin E is not listed/i);
  assert.ok(candidate.care.evidenceUrls.some(url => url.includes('GLOWING_SERUM_BACK')));
  assert.equal(candidate.nigeria.exactOffers.length, 3);
  ...
  assert.equal(
    candidate.asset.publicImageSha256,
    '420f76cf802aab4c8328b9b8d10830add8496cb8efd10f6152da5bad84532329',
  );
  assert.equal(decision.stage, 'approval-ready');
...
test('the TRESemmé conditioner binds its official GTIN, three Nigerian prices and transparent official packshot', () => {
  const candidate = catalogueIntakeCandidates.find(
    item => item.id === 'tresemme-keratin-smooth-weightless-conditioner-828ml',
  );
  assert.ok(candidate);
  assert.equal(candidate.identity.gtin, '00022400011738');
  assert.equal(candidate.identity.officialEvidence?.observedSize, '28 fl oz / 828 ml');
  assert.equal(candidate.identity.officialEvidence?.canonicalExtraction.schemaVersion, 4);
  assert.deepEqual(
    candidate.nigeria.exactOffers.map(offer => offer.retailer),
...
test('the Dove Skin Replenish wash binds its exact package, two Nigerian prices and reviewed transparent render', () => {
  const candidate = catalogueIntakeCandidates.find(
    item => item.id === 'dove-skin-replenish-serum-body-wash-547ml',
  );
  assert.ok(candidate);
  assert.equal(candidate.identity.gtin, '00011111055230');
...
test('the Face Facts moisturising gel cream binds two independent EAN sources, two Nigerian offers, and its reviewed render', () => {
  const candidate = catalogueIntakeCandidates.find(item => (
    item.id === 'facefacts-ceramide-moisturising-gel-cream-50ml'
  ));
  assert.ok(candidate);
  assert.equal(candidate.identity.gtin, '5031413928570');
  ...
  assert.equal(candidate.asset.publicImageSha256, '0eb13b51e08b874ad74f707f86c809e035962d27288acadf3167dbeeb87bff54');
  assert.equal(candidate.asset.publicImageByteSize, 1_061_434);
  assert.equal(candidate.asset.width, 2_000);
  assert.equal(candidate.asset.height, 2_000);
  const generation = candidate.asset.generationRecord;
  assert.ok(generation);
...
test('the 4% acne cleanser keeps exact size evidence, current availability and its own generated tube', () => {
  const candidate = catalogueIntakeCandidates.find(item => item.id === 'cerave-acne-foaming-cream-cleanser-4-150ml');
  assert.ok(candidate);
...
test('the Face Facts Vitamin C lotion binds independent EAN evidence and two Nigerian prices', () => {
  const candidate = catalogueIntakeCandidates.find(item => item.id === 'facefacts-vitamin-c-body-lotion-400ml');
  assert.ok(candidate);
  assert.equal(candidate.identity.gtin, '5031413929089');
  assert.equal(candidate.identity.officialEvidence?.observedVariant, 'Vitamin C Body Lotion');
  assert.equal(candidate.identity.officialEvidence?.observedSize, '400 ml');
  const extraction = candidate.identity.officialEvidence?.canonicalExtraction;
  assert.ok(extraction);
  assert.equal(extraction.schemaVersion, 6);
  assert.equal(extraction.method, 'reviewed-browser-accessibility-identity-with-independent-ean-corroboration');
  assert.deepEqual(
    extraction.identifierCorroborations?.map(source => new URL(source.sourceUrl).hostname),
    ['www.ebay.co.uk', 'www.eapollowholesale.co.uk'],
  );
  assert.deepEqual(candidate.nigeria.exactOffers.map(offer => offer.retailer), ['BuyBetter', 'Allure Beauty']);
