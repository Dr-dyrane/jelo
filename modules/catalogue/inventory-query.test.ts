...
} from "@/lib/catalogue/inventory-query";
...
test("returns only reviewed products and deliberately approved community records", () => {
  ...
  const supportiveTotal = products.filter(
    (product) =>
  ...
  assert.deepEqual(
    result.facets.categories.map((facet) => facet.value),
    inventoryCategories,
  );
  assert.equal(
    result.facets.categories.reduce((sum, facet) => sum + facet.count, 0),
    publicTotal,
  );
  assert.equal(
  ...
  assert.deepEqual(
    ...
    [...expectedStepCounts].sort(([left], [right]) =>
...
test("a selected zero-result category remains available for clear and undo", () => {
  ...
  const selected = result.facets.categories.find(
    (facet) => facet.value === "Hair & scalp",
  );
...
test("a private candidate barcode cannot leak into public search", () => {
  ...
  const privateCandidate = externalCatalogueCandidates.find(
    (candidate) => !approved.has(candidate.barcode),
  );
  assert.ok(privateCandidate);
  const result = queryInventoryRecords(products, {
    q: privateCandidate.barcode,
  });
  assert.equal(result.total, 0);
});

test("normalizes punctuation without fuzzy clinical matching", () => {
  const punctuation = queryInventoryRecords(products, {
    q: "la roche posay spf 30",
  });
  assert.ok(punctuation.items.some((item) => item.name.includes("SPF 30")));

  const joined = queryInventoryRecords(products, {
    q: "la roche posay double repair matte",
  });
  assert.ok(
    joined.items.some((item) => item.name.includes("Double Repair Matte")),
  );

  const joinedBrand = queryInventoryRecords(products, { q: "facefacts" });
  assert.ok(
    joinedBrand.items.some(
      (item) =>
        item.kind === "reviewed" &&
...
test("indexes only manifest-approved supportive discovery terms", () => {
  const barrier = queryInventoryRecords(products, {
    q: "barrier",
    review: "reviewed",
  });
  assert.deepEqual(
    barrier.items.map((item) => item.id),
    [
  ...
  const approvedUse = queryInventoryRecords(products, {
    q: "oiliness",
    review: "reviewed",
  });
  assert.deepEqual(
    approvedUse.items.map((item) => item.id),
    [
  ...
  const oiliness = queryInventoryRecords(products, {
    q: "oiliness",
    review: "reviewed",
  });
  assert.deepEqual(
    oiliness.items.map((item) => item.id),
    approvedUse.items.map((item) => item.id),
  );
});

test("concern browsing includes explicit reviewed references without turning them into direct recommendations", () => {
  const concern = queryInventoryRecords(products, {
    concern: "acne-breakouts",
  });
  assert.deepEqual(
    concern.items.map((item) => item.id),
    [
      ...
      "reviewed:cosrx-salicylic-acid-daily-gentle-cleanser",
      ...
      "reviewed:panoxyl-acne-creamy-wash-4-170g",
      "reviewed:panoxyl-acne-foaming-wash-10-benzoyl-peroxide",
      "reviewed:some-by-mi-aha-bha-pha-miracle-toner",
      "reviewed:the-ordinary-azelaic-acid-suspension-10",
    ],
  );

  const approvedConcern = queryInventoryRecords(products, {
    concern: "oily-congested-skin",
  });
  assert.deepEqual(
    approvedConcern.items.map((item) => item.id),
    [
      "reviewed:balance-niacinamide-blemish-recovery-serum-30ml",
      "reviewed:balance-salicylic-acid-zinc-clarifying-toner-200ml",
      "reviewed:cerave-foaming-facial-cleanser",
  ...
  const conditionPattern = queryInventoryRecords(products, {
    concern: "hidradenitis-pattern",
  });
  ...
  assert.deepEqual(
    queryInventoryRecords(products).facets.concerns.map((facet) => [
      facet.value,
      facet.total,
    ]),
    [
      ...
      ["dark-spots", 8],
      ["sensitive-barrier", 5],
      ["dry-dehydrated-skin", 8],
      ["dry-rough-body-skin", 11],
      ["sweat-body-odour", 1],
      ["oily-congested-skin", 7],
      ["daily-sun-protection", 3],
      ["dandruff-itchy-scalp", 2],
      ["dry-frizzy-hair", 9],
    ],
  );

  const routine = queryInventoryRecords(products, { step: "protect" });
  const expectedRoutine = products.filter(
    (product) => product.step === "Protect",
  );
  assert.equal(routine.total, expectedRoutine.length);
  assert.equal(routine.filters.step, "Protect");
  assert.ok(
    routine.items.every(
      (item) => item.kind === "reviewed" && item.step === "Protect",
    ),
  );
  assert.deepEqual(
    new Set(routine.items.map((item) => item.id)),
    new Set(expectedRoutine.map((product) => `reviewed:${product.slug}`)),
  );
});
...
test("supportive source filtering stays limited to reviewed eligible products", () => {
  ...
  const expected = products
    .filter(
      (product) =>
        getReviewedProductCare(product.slug)?.careState ===
        "supportive_eligible",
    )
  ...
  assert.ok(
    result.items.every(
      (item) =>
        item.kind === "reviewed" && item.careState === "supportive_eligible",
    ),
  );
});
...
test("browse surfaces separate neutral records from supportive use", async () => {
  const [
    ...
    readFile(
      path.join(
        process.cwd(),
        "components/products/catalogue-merchandising.tsx",
      ),
      "utf8",
    ),
    readFile(
      path.join(process.cwd(), "components/products/product-card.tsx"),
      "utf8",
    ),
    readFile(
      path.join(process.cwd(), "components/products/inventory-card.tsx"),
      "utf8",
    ),
  ...
  assert.doesNotMatch(
    home,
    /product\.(?:bestFor|concerns|skinTypes|sensitiveFriendly|step|displayLine)/,
  );
  ...
  assert.match(
    productsPage,
    /externalProductsCount[\s\S]*?"Two catalogue sources"[\s\S]*?"Catalogue context"/,
  );
  assert.match(
    productsPage,
    /externalProductsCount[\s\S]*?<a[\s\S]*?href="https:\/\/world\.openbeautyfacts\.org\/data"/,
  );
  assert.doesNotMatch(
    productsPage,
    /product\.(?:bestFor|concerns|skinTypes|sensitiveFriendly|step|displayLine)/,
  ...
  assert.doesNotMatch(
    productExperience,
    /\.product-visual(?:::\w+|[\s\S]{0,40})::after/,
  );
  assert.match(
    storefront,
    /\.product-rail::\-webkit-scrollbar\s*\{\s*display:\s*none/,
  );
...
test("applies combined facets and clamps invalid pages", () => {
  const bodyCommunity = queryInventoryRecords(products, {
    category: "Body care",
    review: "community",
    page: "2",
  });
  const approvedBody = externalProducts.filter(
    (product) => product.category === "Body care",
  ).length;
  assert.equal(bodyCommunity.total, approvedBody);
  assert.equal(
    bodyCommunity.page,
    Math.min(2, Math.max(1, Math.ceil(approvedBody / 24))),
  );
  assert.ok(
    bodyCommunity.items.every(
      (item) => item.kind === "community" && item.category === "Body care",
    ),
  );
  ...
  const expectedLastPage =
    (products.length + externalProducts.length) % 24 ||
    Math.min(24, products.length + externalProducts.length);
...
test("sorts only valid source update timestamps as recently updated", () => {
  ...
  try {
    ...
    const result = queryInventoryRecords([reviewed], {
      q: fixtureName,
      sort: "newest",
    });
...
test("filters by an exact normalized brand and exposes company facets", () => {
  ...
  assert.ok(
    result.facets.brands.some(
      (facet) => facet.value === "CeraVe" && facet.count >= result.total,
    ),
  );

  const missing = queryInventoryRecords(products, {
...
test("uses only fresh exact offers for market and price filters", () => {
  ...
  const exactNg = fixture("fresh-ng", [
    {
      retailer: "Exact NG",
      url: "https://example.com/exact-ng",
      trust: 90,
      available: true,
      priceNgn: 9_500,
      checkedAt: today,
  ...
  const searchNg = fixture("search-ng", [
    {
      retailer: "Search NG",
      url: "https://example.com/search-ng",
      trust: 90,
      available: true,
      priceNgn: 8_000,
      checkedAt: today,
  ...
  const staleNg = fixture(
    "stale-ng",
    [
      {
        retailer: "Stale NG",
        url: "https://example.com/stale-ng",
        trust: 90,
        available: true,
        priceNgn: 7_000,
  ...
  const exactUs = fixture("fresh-us", [
    {
      retailer: "Exact US",
      url: "https://example.com/exact-us",
      trust: 90,
      available: true,
      priceUsd: 20,
      checkedAt: today,
  ...
  const ng = queryInventoryRecords(fixtures, {
    review: "reviewed",
    availability: "priced",
    market: "NG",
  });
  assert.deepEqual(
    ng.items.map((item) => item.id),
    ["reviewed:fresh-ng"],
  );
  ...
  assert.equal(
    queryInventoryRecords(fixtures, {
      review: "reviewed",
      price: "low",
      market: "NG",
    }).total,
  ...
  const us = queryInventoryRecords(fixtures, {
    review: "reviewed",
    availability: "priced",
    price: "mid",
    market: "US",
