...
test('newer protected seed evidence must be exact and pass publication scope checks', () => {
  ...
  assert.match(seed, /\['api', 'retailer_page'\]\.includes\(verificationMethod\)/);
  assert.match(seed, /\(offer\.match \?\? 'exact'\) === 'exact'/);
  assert.match(seed, /offer\.priceObservation != null/);
  assert.match(seed, /assertRetailerResponseScope\(\{[\s\S]*requestedUrl: offer\.url/);
  assert.match(seed, /expectedTitle: `\$\{product\.brand\} \$\{product\.name\}`/);
  assert.match(seed, /expectedSize: product\.size/);
  assert.match(seed, /marketCode: market/);
  assert.match(seed, /currencyCode/);
  assert.match(seed, /catch \{[\s\S]*cannot replace protected[\s\S]*current evidence/);
});
...
test('a protected current observation is replaced only by strictly newer scope-checked evidence', () => {
  ...
  for (const field of [
    ...
    assert.match(
      seed,
      new RegExp(`${field} = case when ${precedence} then offers\\.${field} else excluded\\.${field} end`),
...
test('seed reconciliation retains append-only price history', () => {
  assert.match(seed, /insert into offer_price_history \([\s\S]*on conflict do nothing/);
