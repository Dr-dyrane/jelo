...
test("catalogue and concern filters acknowledge changes and stay reversible", async () => {
  ...
  assert.match(concerns, /View products/);
  assert.match(concerns, /Selections cleared/);
  assert.match(concerns, /rankReviewedContextForConcerns/);
  assert.match(concerns, /care-cleared/);
  assert.match(concerns, /pharmacist-reviewed/);
  assert.match(concerns, /No care-cleared product yet/);
  assert.match(concerns, /not direct recommendations/);
  ...
  assert.match(concernsPage, /listCatalogueProducts/);
  assert.match(concernsPage, /productsWithReviewedConcernLinks/);
  assert.doesNotMatch(concernsPage, /listRecommendationEligibleProducts/);
