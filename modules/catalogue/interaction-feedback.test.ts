...
test('catalogue and concern filters acknowledge changes and stay reversible', async () => {
  ...
  const navigationStyles = await readFile(path.join(root, 'components/navigation/site-header.module.css'), 'utf8');
  const footerStyles = await readFile(path.join(root, 'app/trust.css'), 'utf8');
  const customerAccess = await readFile(path.join(root, 'lib/customer/access.ts'), 'utf8');
  const signInIntent = await readFile(path.join(root, 'lib/auth/sign-in-intent.ts'), 'utf8');
  ...
  assert.match(ruleDeclarations(navigationStyles, '.mobileLinks>a'), /min-height:\s*4\.4rem;/);
  assert.match(ruleDeclarations(footerStyles, '.footer-group .footer-member-link'), /min-height:\s*2\.75rem;/);
  ...
  assert.doesNotMatch(navigation, /requireCustomer|getSession|customerSignInPath|sign-in/);
