...
test("prepares a bounded, traceable static batch while keeping retailer codes out of product identity", async () => {
  ...
  assert.equal(
    manifest.packets.every((packet) => packet.source === "static-priority"),
    true,
  );
  assert.doesNotThrow(() =>
  ...
  if (first.source !== "static-priority")
    throw new Error("Expected static packet.");
  const firstQueueItem = (researchQueue as CatalogueResearchQueue).items[0]!;
  assert.equal(first.research.kind, firstQueueItem.researchKind);
  assert.deepEqual(first.research.knownProduct, firstQueueItem.knownProduct);
  assert.equal(
    first.productLead.canonicalSlug,
    firstQueueItem.knownProduct?.canonicalProductSlug ?? null,
  );
  assert.ok(first.discoveryEvidence.observations.length > 0);
  assert.deepEqual(
    first.discoveryEvidence.observations.map(
      (observation) => observation.offerKey,
    ),
    firstQueueItem.offerTargets.map((target) => target.offerKey),
  );
  assert.ok(first.discoveryEvidence.retailerCodeLeads.length > 0);
  assert.equal(
    first.discoveryEvidence.retailerCodeLeads.every(
      (code) =>
...
test("selects a later deterministic shard without relaxing the twelve-packet batch", async () => {
  ...
  const expectedDiscoveryId = (researchQueue as CatalogueResearchQueue)
    .items[24]!.discoveryId;
  assert.equal(
    manifest.packets.some(
      (packet) =>
        packet.source === "static-priority" &&
        packet.productLead.discoveryId === expectedDiscoveryId,
    ),
    true,
  );
  assert.throws(
    () =>
      buildStaticResearchEvidencePacketManifest(
        ...
        catalogueResearchQueueDigest(snapshotBytes),
        {
          mode: "batch",
          count: 12,
          offset: (researchQueue as CatalogueResearchQueue).items.length - 1,
        },
      ),
    /extends beyond/,
  );
...
test("accepts aggregate community tasks without retaining contributor identifiers", () => {
  ...
  const manifest = buildCommunityResearchEvidencePacketManifest(report, raw, {
    mode: "batch",
    count: 1,
  });
  assert.doesNotThrow(() =>
  ...
  assert.doesNotMatch(
    serialized,
    /contributionId|contributorId|draftId|email|phone|userId|sessionId/i,
  );
...
test("refuses community tasks carrying contributor identifiers and any prefilled proof", () => {
  const unsafe = communityReport() as CommunityAggregateResearchReport & {
    researchQueue: Array<
      CommunityAggregateResearchReport["researchQueue"][number] & {
        contributionId?: string;
      }
    >;
  };
  unsafe.researchQueue[0]!.contributionId =
    "b08d4cc3-8e72-4189-89a3-6d77f2c700bc";
  assert.throws(
    () =>
      buildCommunityResearchEvidencePacketManifest(
        unsafe,
    ...
    researchQueue: Array<
      CommunityAggregateResearchReport["researchQueue"][number] & {
        identity: NonNullable<
          CommunityAggregateResearchReport["researchQueue"][number]["identity"]
        > & {
          sessionId?: string;
        };
      }
  ...
  const report = communityReport();
  const manifest = buildCommunityResearchEvidencePacketManifest(
    report,
  ...
  const malformed = manifest as unknown as {
    packets: Array<{
      proofSlots: { officialIdentity: { manufacturerGtin: string | null } };
    }>;
  };
  malformed.packets[0]!.proofSlots.officialIdentity.manufacturerGtin =
    "4005808319695";
  assert.throws(
    () => assertPrivateResearchEvidencePacketManifest(malformed as never),
    /uncollected and non-publishable/,
  );
});
...
test("content-addressed shards compile all priorities and remain invisible to public catalogue sources", async () => {
  ...
  assert.equal(
    stored.sharding.packetCount,
    (researchQueue as CatalogueResearchQueue).selectedCount,
  );
  assert.equal(
    stored.sharding.shardCount,
    Math.ceil((researchQueue as CatalogueResearchQueue).selectedCount / 12),
  );
  assert.equal(
    stored.shards.every((shard) => shard.packetCount <= 12),
    true,
  );
  assert.equal(
    stored.shards
      .flatMap((shard) => shard.discoveryIds)
      .includes(
        (researchQueue as CatalogueResearchQueue).items[24]!.discoveryId,
      ),
    true,
  );
