...
function responseFor(
  value: unknown,
  url = planItem().sourceProductApiUrl,
...
test("joins only the bounded checked-in packet selection to exact retailer product API endpoints", () => {
  ...
  if (first.source !== "static-priority")
    throw new Error("Expected a static research packet.");
  const candidate = snapshot.candidates.find(
    (item) => item.discoveryId === first.productLead.discoveryId,
  );
  assert.ok(candidate);

  const plan = buildResearchOfferCapturePlan(packets, snapshot, 1);
  assert.equal(plan.length, first.discoveryEvidence.observations.length);
  assert.equal(
    plan.every((item) => item.packetId === first.id),
    true,
  );
  assert.deepEqual(
    plan.map((item) => item.offerKey),
    first.discoveryEvidence.observations.map((item) => item.offerKey),
  );
  assert.equal(
    plan.every(
      (item) =>
  ...
  const thirdShard = JSON.parse(
    readFileSync(
      path.join(process.cwd(), thirdShardDescriptor.sourcePath),
      "utf8",
    ),
  ) as CatalogueResearchEvidencePacketManifest;
  const laterPacket = thirdShard.packets[0];
  assert.ok(laterPacket);
  const laterPlan = buildResearchOfferCapturePlan(thirdShard, snapshot, [
    laterPacket.id,
  ]);
  assert.equal(
    laterPlan.every(
      (item) =>
        item.discoveryId ===
        (laterPacket.source === "static-priority"
          ? laterPacket.productLead.discoveryId
          : ""),
    ),
    true,
  );
  ...
  const staleCandidate = staleSnapshot.candidates.find(
    (item) => item.discoveryId === first.productLead.discoveryId,
  );
  ...
  staleObservation.sourceProductApiUrl =
    staleObservation.sourceProductApiUrl.replace(
      /\/\d+$/,
      `/${staleObservation.sourceProductId}`,
    );
  assert.throws(
    () => buildResearchOfferCapturePlan(packets, staleSnapshot, 1),
    /immutable snapshot bindings|not bound to the research packet/,
  );

  const missingBinding = structuredClone(packets);
  const missingPacket = missingBinding.packets[0];
  assert.equal(missingPacket?.source, "static-priority");
  if (missingPacket?.source !== "static-priority")
    throw new Error("Expected a static research packet.");
  missingPacket.discoveryEvidence.observations[0]!.offerKey = "f".repeat(24);
  assert.throws(
    () => buildResearchOfferCapturePlan(missingBinding, snapshot, 1),
    /exact offer binding is invalid|immutable snapshot bindings/,
  );
});
test("captures exact JSON bytes while keeping retailer SKU local and publication authority absent", async () => {
  ...
  assert.equal(
    manifest.captures[0].publicationStatus,
...
test("retains source bytes only under an explicit private, no-reuse source grant", async () => {
  ...
  const lux = catalogueDiscoverySources.find(
    (source) => source.key === "lux-beauty-ng",
  );
  ...
  const denied = planItem({
    ...
    evidencePath: `data/catalogue-offer-source-evidence/${"b".repeat(24)}--lux-beauty.json`,
  });
...
test("rejects route, product, permalink, title, size, price and response-boundary drift", async () => {
  ...
  await assert.rejects(
    captureResearchOfferResponse(
      planItem({
        sourceProductApiUrl:
          "https://attacker.example/wp-json/wc/store/v1/products/421",
      }),
      { fetchImpl: fetchResponse(responseFor(product()).response) },
    ),
  ...
  await assert.rejects(
    captureResearchOfferResponse(item, {
      fetchImpl: fetchResponse(responseFor(product({ id: 422 })).response),
    }),
    /product id changed/,
  );
  await assert.rejects(
  ...
  await assert.rejects(
    captureResearchOfferResponse(item, {
      fetchImpl: fetchResponse(
        responseFor(product({ name: "A different cleanser 473 ml" })).response,
      ),
    }),
  ...
  await assert.rejects(
    captureResearchOfferResponse(item, {
      fetchImpl: fetchResponse(
        responseFor(product({ name: "CeraVe Hydrating Cleanser 236 ml" }))
          .response,
      ),
    }),
    /title does not match/,
  ...
  await assert.rejects(
    captureResearchOfferResponse(item, {
      fetchImpl: fetchResponse(
        responseFor(product(), item.sourceProductApiUrl, {
...
test("atomically writes exact evidence bytes and verifies the private manifest offline", async () => {
  ...
  const temporaryRoot = await mkdtemp(
    path.join(os.tmpdir(), "jelocare-offer-capture-"),
  );
  try {
    await writeResearchOfferCaptureBundle(temporaryRoot, manifest, [captured]);
    assert.deepEqual(
      await readFile(path.join(temporaryRoot, item.evidencePath)),
      bytes,
    );
    const storedManifest = JSON.parse(
      await readFile(
        path.join(temporaryRoot, researchOfferCaptureManifestPath),
        "utf8",
      ),
    ) as ResearchOfferCaptureManifest;
    assert.deepEqual(storedManifest, manifest);
    assert.deepEqual(
      await verifyResearchOfferCaptureBundle(temporaryRoot, storedManifest),
      {
        packetCount: 1,
        responseCount: 1,
      },
...
test("manifest verification rejects publication authority and retailer identity promotion", async () => {
  ...
  const publishable = structuredClone(manifest) as unknown as {
  ...
  assert.throws(
    () =>
      assertPrivateResearchOfferCaptureManifest(
        publishable as unknown as ResearchOfferCaptureManifest,
      ),
  ...
  promoted.captures[0].offer.retailerSku!.treatment =
    "manufacturer-identity" as never;
  assert.throws(
