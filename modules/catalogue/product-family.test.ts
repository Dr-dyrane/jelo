...
import { catalogueProductFamilies, type CatalogueProductFamily } from '@/data/product-families';
...
test('the L’Occitane family admits only the released exact 250 mL bottle', () => {
  const family = catalogueProductFamilies.find(record => record.id === 'loccitane-almond-shower-oil');
  assert.ok(family);
  ...
  const searchRecord = publicCatalogueSearchArtifact.products.find(record => record.slug === loccitane250Slug);
  ...
  assert.equal(family.members.some(member => member.productSlug.includes('500ml')), false);
});

test('family resolution preserves independent SKU routes, images, offers and package forms', () => {
  const exactProducts = [
    { slug: 'almond-250', size: '250 ml', image: '/250.png', offers: [{ id: 'offer-250' }] },
    { slug: 'almond-500', size: '500 ml', image: '/500.png', offers: [{ id: 'offer-500' }] },
    { slug: 'almond-refill-500', size: '500 ml', image: '/refill.png', offers: [{ id: 'offer-refill' }] },
  ];
  ...
  const resolved = resolveCatalogueProductFamily('almond-500', exactProducts, families);
  assert.ok(resolved);
  assert.deepEqual(
    ...
    [
      { slug: 'almond-250', image: '/250.png', offerId: 'offer-250', packageForm: 'bottle', optionLabel: '250 mL' },
      { slug: 'almond-500', image: '/500.png', offerId: 'offer-500', packageForm: 'bottle', optionLabel: '500 mL bottle' },
      { slug: 'almond-refill-500', image: '/refill.png', offerId: 'offer-refill', packageForm: 'refill', optionLabel: '500 mL refill' },
    ],
  );
  assert.equal(new Set(resolved.members.map(member => member.product.slug)).size, 3);
  assert.equal(new Set(resolved.members.map(member => member.optionLabel)).size, 3);
});
...
test('the selector routes to each exact public member and selects only the current route', () => {
  ...
  const family = resolveCatalogueProductFamily('almond-250', exactProducts, families);
  assert.ok(family);

  const markup = renderToStaticMarkup(createElement(ProductSizeSelector, { family }));
  assert.match(markup, /<a[^>]*aria-current="page"[^>]*href="\/products\/almond-250"/);
  assert.match(markup, /href="\/products\/almond-500"/);
  assert.equal(markup.match(/aria-current="page"/g)?.length, 1);

  const page = readFileSync('app/(site)/products/[slug]/page.tsx', 'utf8');
  assert.match(page, /resolveCatalogueProductFamily\(product\.slug, products\)/);
