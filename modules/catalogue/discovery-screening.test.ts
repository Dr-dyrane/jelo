...
const directorySource: CatalogueDiscoverySource = {
  ...
  retailerAuthority: "reviewed-static-registry",
...
const provisionalSource: CatalogueDiscoverySource = {
  ...
  trust: 75,
  retailerAuthority: "unmatched-private-only",
};
...
function product(overrides: Partial<WooStoreProduct> = {}): WooStoreProduct {
  return {
    ...
    permalink:
      "https://directory.example/product/cerave-hydrating-cleanser-473ml/",
    sku: "3337875597333",
...
function input(
  ...
  overrides: Partial<WooStoreProduct> = {},
...
test("a GTIN-shaped retailer SKU stays an unverified identity lead", () => {
  ...
  assert.equal(
    result.seed.retailerGtinHintWarning,
    "retailer-sku-is-not-manufacturer-identity",
  );
...
test("matching retailer-number leads group for research without becoming identity proof", () => {
  const provisional = input(provisionalSource, {
    ...
    permalink:
      "https://provisional.example/product/cerave-hydrating-cleanser-473ml/",
  ...
  assert.equal(
    snapshot.candidates[0].identityStatus,
    "unverified-retailer-lead",
  );
...
test("the checked-in discovery batch is large, traceable and incapable of publication", async () => {
  const snapshotPath = path.join(
    process.cwd(),
    "data/catalogue-discovery-screening.json",
  );
  ...
  assert.equal(
    snapshot.sourceResponses.every(
      (response) =>
  ...
  const declaredResponseIds = new Set(
    snapshot.sourceResponses.map((response) => response.id),
  );
  assert.equal(
    snapshot.candidates.every(
      (candidate) =>
        ...
        candidate.retailerObservations.every((observation) =>
          declaredResponseIds.has(observation.responseEvidenceId),
        ),
    ),
    true,
  );
  assert.ok(
    snapshot.candidates.filter((candidate) => candidate.retailerGtinHint)
      .length >= 900,
  );
  assert.equal(
    snapshot.candidates.every(
      (candidate) =>
        ...
        candidate.retailerGtinHintWarning ===
          "retailer-sku-is-not-manufacturer-identity",
    ),
    true,
  );
  assert.ok(
    snapshot.candidates.filter(
      (candidate) => candidate.imageStatus === "source-url-unreviewed",
    ).length >= 900,
  );
  ...
  for (const runtimePath of [
    "data/catalogue.ts",
    "data/products.ts",
    "lib/catalogue/publication-boundary.ts",
  ]) {
    const runtime = await readFile(
      path.join(process.cwd(), runtimePath),
      "utf8",
    );
