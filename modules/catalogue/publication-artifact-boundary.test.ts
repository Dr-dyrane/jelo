...
test('publication preflight fails closed when retained artifacts cannot be reopened', async () => {
  const emptyRoot = await mkdtemp(path.join(os.tmpdir(), 'jelocare-publication-boundary-'));
  try {
...
test('production release entrypoints use artifact-aware constructors and verifiers', async () => {
  const [releaseCommand, dossierGate, releaseGate, imageGate] = await Promise.all([
    readFile(path.join(process.cwd(), 'scripts/release-catalogue-candidate.ts'), 'utf8'),
    readFile(path.join(process.cwd(), 'scripts/verify-catalogue-publication-dossiers.ts'), 'utf8'),
    readFile(path.join(process.cwd(), 'scripts/verify-catalogue-publication-releases.ts'), 'utf8'),
    readFile(path.join(process.cwd(), 'scripts/verify-catalogue-publication-images.ts'), 'utf8'),
  ]);
  ...
  assert.match(dossierGate, /verifyCataloguePublicationDossierManifestWithArtifacts/);
  assert.match(releaseGate, /verifyCataloguePublicationReleaseManifestWithArtifacts/);
  assert.match(imageGate, /verifyCataloguePublicationDossierManifestWithArtifacts/);
});
