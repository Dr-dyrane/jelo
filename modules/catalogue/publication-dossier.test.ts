...
test('the checked-in publication manifest contains the verified neutral reference dossiers', () => {
  ...
  assert.equal(result.dossiers[125].candidateId, 'cerave-hydrating-cleanser-473ml');
  assert.equal(result.dossiers[125].nigeria.regulatoryStatus, 'pending');
  assert.equal(result.dossiers[126].candidateId, 'cerave-moisturising-cream-454g');
  ...
  assert.equal(result.dossiers[127].candidateId, 'dove-melanin-even-tone-body-wash-18-5oz');
  ...
  assert.equal(result.dossiers[128].candidateId, 'keracare-dry-itchy-scalp-conditioner-950ml');
  ...
  assert.equal(result.dossiers[119].candidateId, 'balance-salicylic-acid-zinc-clarifying-toner-200ml');
  ...
  assert.equal(result.dossiers[115].candidateId, 'cerave-acne-foaming-cream-wash-10-150ml');
  ...
  assert.equal(result.dossiers[6].candidateId, 'de-la-cruz-acne-treatment-10-sulfur-73-7g');
  assert.equal(result.dossiers[6].nigeria.exactOffers.length, 2);
  assert.equal(result.dossiers[7].candidateId, 'olay-super-serum-body-wash-normal-skin-547ml');
  assert.equal(result.dossiers[7].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[7].nigeria.exactOffers.map(offer => offer.priceNgn), [20_000, 21_200]);
  assert.equal(result.dossiers[7].rights.generationRecord?.outputSha256, result.dossiers[7].finalImage.sha256);
  assert.equal(result.dossiers[8].candidateId, 'dove-calming-moisture-body-wash-547ml');
  assert.equal(result.dossiers[8].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[8].nigeria.exactOffers.map(offer => offer.priceNgn), [22_600, 17_800]);
  assert.equal(result.dossiers[8].rights.generationRecord?.outputSha256, result.dossiers[8].finalImage.sha256);
  assert.equal(result.dossiers[114].candidateId, 'cerave-sa-smoothing-cleanser-473ml');
  ...
  assert.equal(result.dossiers[120].candidateId, 'garnier-vitamin-c-brightening-day-cream-50ml');
  ...
  assert.equal(result.dossiers[2].candidateId, 'aqua-rich-ceramide-body-lotion-500ml');
  assert.equal(result.dossiers[2].identity.name, 'Hydrate + Protect Body Lotion');
  assert.equal(result.dossiers[2].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[2].nigeria.exactOffers.map(offer => offer.priceNgn), [12_800, 13_000]);
  assert.equal(result.dossiers[2].nigeria.regulatoryStatus, 'pending');
  assert.equal(result.dossiers[2].rights.generationRecord?.outputSha256, result.dossiers[2].finalImage.sha256);
  assert.equal(result.dossiers[124].candidateId, 'aqua-rich-turmeric-vitamin-c-body-lotion-500ml');
  assert.equal(result.dossiers[124].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[124].nigeria.exactOffers.map(offer => offer.priceNgn), [12_800, 12_000]);
  ...
  assert.equal(result.dossiers[3].candidateId, 'balance-niacinamide-blemish-recovery-serum-30ml');
  assert.equal(result.dossiers[3].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[3].nigeria.exactOffers.map(offer => offer.priceNgn), [8_400, 10_700]);
  ...
  assert.equal(result.dossiers[4].candidateId, 'nineless-a-control-10-azelaic-acid-serum-30ml');
  assert.equal(result.dossiers[4].identity.packageVersion, 'Original green dropper bottle');
  assert.equal(result.dossiers[4].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[4].nigeria.exactOffers.map(offer => offer.priceNgn), [12_500, 14_500]);
  assert.equal(result.dossiers[4].rights.generationRecord?.outputSha256, result.dossiers[4].finalImage.sha256);
  assert.equal(result.dossiers[5].candidateId, 'nineless-mela-pro-rice-txa-toner-200ml');
  assert.equal(result.dossiers[5].identity.packageVersion, 'Original translucent bottle with orange cap');
  assert.equal(result.dossiers[5].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[5].nigeria.exactOffers.map(offer => offer.priceNgn), [15_500, 19_000]);
  assert.equal(result.dossiers[5].rights.generationRecord?.outputSha256, result.dossiers[5].finalImage.sha256);
  assert.equal(result.dossiers[121].candidateId, 'facefacts-ceramide-oil-control-foaming-cleanser-400ml');
  assert.equal(result.dossiers[121].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[121].nigeria.exactOffers.map(offer => offer.priceNgn), [7_500, 7_300]);
  assert.equal(result.dossiers[121].rights.generationRecord?.outputSha256, result.dossiers[121].finalImage.sha256);
  assert.equal(result.dossiers[110].candidateId, 'facefacts-ceramide-hydrating-gentle-cleanser-400ml');
  assert.equal(result.dossiers[110].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[110].nigeria.exactOffers.map(offer => offer.priceNgn), [6_950, 7_200, 7_300]);
  assert.equal(result.dossiers[110].rights.generationRecord?.outputSha256, result.dossiers[110].finalImage.sha256);
  assert.equal(result.dossiers[122].candidateId, 'facefacts-ceramide-foaming-cleanser-400ml');
  assert.equal(result.dossiers[122].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[122].nigeria.exactOffers.map(offer => offer.priceNgn), [6_450, 6_800]);
  assert.equal(result.dossiers[122].rights.generationRecord?.outputSha256, result.dossiers[122].finalImage.sha256);
  assert.equal(result.dossiers[123].candidateId, 'facefacts-vitamin-c-body-lotion-400ml');
  assert.equal(result.dossiers[123].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[123].nigeria.exactOffers.map(offer => offer.priceNgn), [6_095, 8_000]);
...
test('the checked-in release manifest explicitly publishes the verified neutral references', () => {
  ...
  assert.equal(report.products[125].slug, 'cerave-hydrating-cleanser-473ml');
  assert.equal(report.products[125].offers[0].priceNgn, 15_265);
  assert.equal(report.products[126].slug, 'cerave-moisturising-cream-454g');
  assert.equal(report.products[126].offers[0].priceNgn, 22_500);
  ...
  assert.equal(report.products[127].slug, 'dove-melanin-even-tone-body-wash-18-5oz');
  ...
  assert.equal(report.products[127].offers[0].priceNgn, 22_600);
  assert.equal(report.products[127].offers[0].available, true);
  assert.equal(report.products[127].offers[1].retailer, 'Perona Beauty');
  assert.equal(report.products[127].offers[1].priceNgn, 20_000);
  assert.equal(report.products[127].offers[1].available, true);
  assert.equal(report.products[128].slug, 'keracare-dry-itchy-scalp-conditioner-950ml');
  ...
  assert.equal(report.products[128].offers[0].priceNgn, 38_485);
  ...
  assert.equal(report.products[128].offers[1].priceNgn, 43_485);
  assert.equal(report.products[128].offers[1].available, true);
  assert.equal(report.products[119].slug, 'balance-salicylic-acid-zinc-clarifying-toner-200ml');
  ...
  assert.equal(report.products[119].offers[1].priceNgn, 9_200);
  assert.equal(report.products[119].offers[1].available, true);
  assert.equal(report.products[115].slug, 'cerave-acne-foaming-cream-wash-10-150ml');
  ...
  assert.equal(report.products[115].offers[0].priceNgn, 23_850);
  assert.equal(report.products[115].offers[0].available, true);
  assert.equal(report.products[115].offers[1].retailer, 'Teeka4');
  assert.equal(report.products[115].offers[1].priceNgn, 24_500);
  assert.equal(report.products[115].offers[1].available, true);
  assert.equal(report.products[114].slug, 'cerave-sa-smoothing-cleanser-473ml');
  assert.equal(report.products[114].category, 'Face');
  assert.equal(report.products[114].step, 'Cleanse');
  assert.equal(report.products[114].offers[0].retailer, 'Teeka4');
  assert.equal(report.products[114].offers[0].priceNgn, 20_900);
  assert.equal(report.products[114].offers[0].available, true);
  assert.equal(report.products[114].offers[1].retailer, '24Eleven');
  assert.equal(report.products[114].offers[1].priceNgn, 23_800);
  assert.equal(report.products[114].offers[1].available, true);
  assert.equal(report.products[120].slug, 'garnier-vitamin-c-brightening-day-cream-50ml');
  assert.equal(report.products[120].category, 'Face');
  assert.equal(report.products[120].step, 'Moisturize');
  assert.equal(report.products[120].offers[0].retailer, 'Care to Beauty');
  assert.equal(report.products[120].offers[0].priceNgn, 17_253.98);
  assert.equal(report.products[120].offers[0].available, true);
  assert.equal(report.products[120].offers[1].retailer, 'Teeka4');
  assert.equal(report.products[120].offers[1].priceNgn, 11_833);
  assert.equal(report.products[120].offers[1].available, true);
  assert.equal(report.products[2].slug, 'aqua-rich-ceramide-body-lotion-500ml');
  assert.equal(report.products[2].category, 'Body');
  assert.equal(report.products[2].step, 'Moisturize');
  assert.equal(report.products[2].offers[0].retailer, 'BuyBetter');
  assert.equal(report.products[2].offers[0].priceNgn, 12_800);
  assert.equal(report.products[2].offers[0].available, true);
  assert.equal(report.products[2].offers[1].retailer, 'CSi Grocery');
  assert.equal(report.products[2].offers[1].priceNgn, 13_000);
  assert.equal(report.products[2].offers[1].available, true);
  assert.equal(report.products[124].slug, 'aqua-rich-turmeric-vitamin-c-body-lotion-500ml');
  ...
  assert.equal(report.products[124].offers[0].priceNgn, 12_800);
  assert.equal(report.products[124].offers[0].available, true);
  assert.equal(report.products[124].offers[1].retailer, 'Kadimez Essentials');
  assert.equal(report.products[124].offers[1].priceNgn, 12_000);
  assert.equal(report.products[124].offers[1].available, true);
  assert.equal(report.products[3].slug, 'balance-niacinamide-blemish-recovery-serum-30ml');
  ...
  assert.equal(report.products[3].offers[0].priceNgn, 8_400);
  assert.equal(report.products[3].offers[0].available, true);
  assert.equal(report.products[3].offers[1].retailer, 'CSi Grocery');
  assert.equal(report.products[3].offers[1].priceNgn, 10_700);
  assert.equal(report.products[3].offers[1].available, true);
  assert.equal(report.products[4].slug, 'nineless-a-control-10-azelaic-acid-serum-30ml');
  assert.equal(report.products[4].category, 'Face');
  assert.equal(report.products[4].step, 'Treat');
  assert.equal(report.products[4].offers[0].retailer, 'Beauty by Daz');
  assert.equal(report.products[4].offers[0].priceNgn, 12_500);
  assert.equal(report.products[4].offers[1].retailer, 'BuyBetter');
  assert.equal(report.products[4].offers[1].priceNgn, 14_500);
  assert.equal(report.products[5].slug, 'nineless-mela-pro-rice-txa-toner-200ml');
  assert.equal(report.products[5].category, 'Face');
  assert.equal(report.products[5].step, 'Tone');
  assert.equal(report.products[5].offers[0].retailer, 'BuyBetter');
  assert.equal(report.products[5].offers[0].priceNgn, 15_500);
  assert.equal(report.products[5].offers[0].available, true);
  assert.equal(report.products[5].offers[1].retailer, 'Muna Cosmetics');
  assert.equal(report.products[5].offers[1].priceNgn, 19_000);
  assert.equal(report.products[5].offers[1].available, true);
  assert.equal(report.products[121].slug, 'facefacts-ceramide-oil-control-foaming-cleanser-400ml');
  assert.equal(report.products[121].category, 'Face');
  assert.equal(report.products[121].step, 'Cleanse');
  assert.equal(report.products[121].offers[0].retailer, 'CSi Grocery');
  assert.equal(report.products[121].offers[0].priceNgn, 7_500);
  assert.equal(report.products[121].offers[0].available, true);
  assert.equal(report.products[121].offers[1].retailer, '24Eleven');
  assert.equal(report.products[121].offers[1].priceNgn, 7_300);
  assert.equal(report.products[121].offers[1].available, true);
  assert.equal(report.products[110].slug, 'facefacts-ceramide-hydrating-gentle-cleanser-400ml');
  ...
  assert.equal(report.products[110].offers[0].priceNgn, 6_950);
  assert.equal(report.products[110].offers[1].retailer, 'Teeka4');
  assert.equal(report.products[110].offers[1].priceNgn, 7_200);
  assert.equal(report.products[110].offers[2].retailer, '24Eleven');
  assert.equal(report.products[110].offers[2].priceNgn, 7_300);
  assert.equal(report.products[122].slug, 'facefacts-ceramide-foaming-cleanser-400ml');
  ...
  assert.equal(report.products[122].offers[0].priceNgn, 6_450);
  assert.equal(report.products[122].offers[0].available, true);
  assert.equal(report.products[122].offers[1].retailer, '24Eleven');
  assert.equal(report.products[122].offers[1].priceNgn, 6_800);
  assert.equal(report.products[122].offers[1].available, true);
  assert.equal(report.products[6].slug, 'de-la-cruz-acne-treatment-10-sulfur-73-7g');
  assert.equal(report.products[6].category, 'Face');
  assert.equal(report.products[6].step, 'Treat');
  assert.equal(report.products[6].offers[0].retailer, 'Beauty by Daz');
  assert.equal(report.products[6].offers[0].priceNgn, 12_500);
  assert.equal(report.products[6].offers[0].available, true);
  assert.equal(report.products[6].offers[1].retailer, 'BuyBetter');
  assert.equal(report.products[6].offers[1].priceNgn, 17_738);
  assert.equal(report.products[6].offers[1].available, false);
  assert.equal(report.products[7].slug, 'olay-super-serum-body-wash-normal-skin-547ml');
  assert.equal(report.products[7].category, 'Body');
  assert.equal(report.products[7].step, 'Cleanse');
  assert.equal(report.products[7].offers[0].retailer, 'BuyBetter');
  assert.equal(report.products[7].offers[0].priceNgn, 20_000);
  assert.equal(report.products[7].offers[0].available, true);
  assert.equal(report.products[7].offers[1].retailer, 'Perona Beauty');
  assert.equal(report.products[7].offers[1].priceNgn, 21_200);
  assert.equal(report.products[7].offers[1].available, true);
  assert.equal(report.products[8].slug, 'dove-calming-moisture-body-wash-547ml');
  ...
  assert.equal(report.products[8].offers[0].priceNgn, 22_600);
  assert.equal(report.products[8].offers[0].available, true);
  assert.equal(report.products[8].offers[1].retailer, 'Teeka4');
  assert.equal(report.products[8].offers[1].priceNgn, 17_800);
  assert.equal(report.products[8].offers[1].available, false);
  assert.equal(report.products[123].slug, 'facefacts-vitamin-c-body-lotion-400ml');
  ...
  assert.equal(report.products[123].offers[0].priceNgn, 6_095);
  assert.equal(report.products[123].offers[0].available, true);
  assert.equal(report.products[123].offers[1].retailer, 'Allure Beauty');
  assert.equal(report.products[123].offers[1].priceNgn, 8_000);
