...
test('the checked-in publication manifest contains the verified neutral reference dossiers', () => {
  ...
  assert.equal(result.dossiers[126].candidateId, 'balance-salicylic-acid-zinc-clarifying-toner-200ml');
  ...
  assert.equal(result.dossiers[122].candidateId, 'cerave-acne-foaming-cream-wash-10-150ml');
  ...
  assert.equal(result.dossiers[12].candidateId, 'de-la-cruz-acne-treatment-10-sulfur-73-7g');
  assert.equal(result.dossiers[12].nigeria.exactOffers.length, 2);
  assert.equal(result.dossiers[13].candidateId, 'olay-super-serum-body-wash-normal-skin-547ml');
  assert.equal(result.dossiers[13].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[13].nigeria.exactOffers.map(offer => offer.priceNgn), [20_000, 21_200]);
  assert.equal(result.dossiers[13].rights.generationRecord?.outputSha256, result.dossiers[13].finalImage.sha256);
  assert.equal(result.dossiers[14].candidateId, 'dove-calming-moisture-body-wash-547ml');
  assert.equal(result.dossiers[14].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[14].nigeria.exactOffers.map(offer => offer.priceNgn), [22_600, 17_800]);
  assert.equal(result.dossiers[14].rights.generationRecord?.outputSha256, result.dossiers[14].finalImage.sha256);
  assert.equal(result.dossiers[121].candidateId, 'cerave-sa-smoothing-cleanser-473ml');
  ...
  assert.equal(result.dossiers[127].candidateId, 'garnier-vitamin-c-brightening-day-cream-50ml');
  ...
  assert.equal(result.dossiers[6].candidateId, 'aqua-rich-ceramide-body-lotion-500ml');
  assert.equal(result.dossiers[6].identity.name, 'Hydrate + Protect Body Lotion');
  assert.equal(result.dossiers[6].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[6].nigeria.exactOffers.map(offer => offer.priceNgn), [12_800, 13_000]);
  assert.equal(result.dossiers[6].nigeria.regulatoryStatus, 'pending');
  assert.equal(result.dossiers[6].rights.generationRecord?.outputSha256, result.dossiers[6].finalImage.sha256);
  assert.equal(result.dossiers[7].candidateId, 'aqua-rich-turmeric-vitamin-c-body-lotion-500ml');
  assert.equal(result.dossiers[7].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[7].nigeria.exactOffers.map(offer => offer.priceNgn), [10_750, 12_000]);
  assert.equal(result.dossiers[7].nigeria.regulatoryStatus, 'pending');
  assert.equal(result.dossiers[7].rights.generationRecord?.outputSha256, result.dossiers[7].finalImage.sha256);
  assert.equal(result.dossiers[8].candidateId, 'balance-niacinamide-blemish-recovery-serum-30ml');
  assert.equal(result.dossiers[8].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[8].nigeria.exactOffers.map(offer => offer.priceNgn), [8_400, 10_700]);
  ...
  assert.equal(result.dossiers[9].candidateId, 'nineless-a-control-10-azelaic-acid-serum-30ml');
  assert.equal(result.dossiers[9].identity.packageVersion, 'Original green dropper bottle');
  assert.equal(result.dossiers[9].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[9].nigeria.exactOffers.map(offer => offer.priceNgn), [12_500, 14_500]);
  assert.equal(result.dossiers[9].rights.generationRecord?.outputSha256, result.dossiers[9].finalImage.sha256);
  assert.equal(result.dossiers[10].candidateId, 'nineless-mela-pro-rice-txa-toner-200ml');
  assert.equal(result.dossiers[10].identity.packageVersion, 'Original translucent bottle with orange cap');
  assert.equal(result.dossiers[10].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[10].nigeria.exactOffers.map(offer => offer.priceNgn), [15_500, 19_000]);
  assert.equal(result.dossiers[10].rights.generationRecord?.outputSha256, result.dossiers[10].finalImage.sha256);
  assert.equal(result.dossiers[128].candidateId, 'facefacts-ceramide-oil-control-foaming-cleanser-400ml');
  assert.equal(result.dossiers[128].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[128].nigeria.exactOffers.map(offer => offer.priceNgn), [7_500, 7_300]);
  assert.equal(result.dossiers[128].rights.generationRecord?.outputSha256, result.dossiers[128].finalImage.sha256);
  assert.equal(result.dossiers[117].candidateId, 'facefacts-ceramide-hydrating-gentle-cleanser-400ml');
  assert.equal(result.dossiers[117].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[117].nigeria.exactOffers.map(offer => offer.priceNgn), [6_950, 7_200, 7_300]);
  assert.equal(result.dossiers[117].rights.generationRecord?.outputSha256, result.dossiers[117].finalImage.sha256);
  assert.equal(result.dossiers[11].candidateId, 'facefacts-ceramide-foaming-cleanser-400ml');
  assert.equal(result.dossiers[11].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[11].nigeria.exactOffers.map(offer => offer.priceNgn), [6_450, 6_800]);
  assert.equal(result.dossiers[11].rights.generationRecord?.outputSha256, result.dossiers[11].finalImage.sha256);
  assert.equal(result.dossiers[17].candidateId, 'facefacts-vitamin-c-body-lotion-400ml');
  assert.equal(result.dossiers[17].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[17].nigeria.exactOffers.map(offer => offer.priceNgn), [6_095, 8_000]);
...
test('the checked-in release manifest explicitly publishes the verified neutral references', () => {
  ...
  assert.equal(report.products[126].slug, 'balance-salicylic-acid-zinc-clarifying-toner-200ml');
  ...
  assert.equal(report.products[126].offers[1].retailer, '24Eleven');
  assert.equal(report.products[126].offers[1].priceNgn, 9_200);
  assert.equal(report.products[126].offers[1].available, true);
  assert.equal(report.products[122].slug, 'cerave-acne-foaming-cream-wash-10-150ml');
  ...
  assert.equal(report.products[122].offers[0].priceNgn, 23_850);
  assert.equal(report.products[122].offers[0].available, true);
  assert.equal(report.products[122].offers[1].retailer, 'Teeka4');
  assert.equal(report.products[122].offers[1].priceNgn, 24_500);
  assert.equal(report.products[122].offers[1].available, true);
  assert.equal(report.products[121].slug, 'cerave-sa-smoothing-cleanser-473ml');
  assert.equal(report.products[121].category, 'Face');
  assert.equal(report.products[121].step, 'Cleanse');
  assert.equal(report.products[121].offers[0].retailer, 'Teeka4');
  assert.equal(report.products[121].offers[0].priceNgn, 20_900);
  assert.equal(report.products[121].offers[0].available, true);
  assert.equal(report.products[121].offers[1].retailer, '24Eleven');
  assert.equal(report.products[121].offers[1].priceNgn, 23_800);
  assert.equal(report.products[121].offers[1].available, true);
  assert.equal(report.products[127].slug, 'garnier-vitamin-c-brightening-day-cream-50ml');
  assert.equal(report.products[127].category, 'Face');
  assert.equal(report.products[127].step, 'Moisturize');
  assert.equal(report.products[127].offers[0].retailer, 'Care to Beauty');
  assert.equal(report.products[127].offers[0].priceNgn, 17_253.98);
  assert.equal(report.products[127].offers[0].available, true);
  assert.equal(report.products[127].offers[1].retailer, 'Teeka4');
  assert.equal(report.products[127].offers[1].priceNgn, 11_833);
  assert.equal(report.products[127].offers[1].available, true);
  assert.equal(report.products[6].slug, 'aqua-rich-ceramide-body-lotion-500ml');
  assert.equal(report.products[6].category, 'Body');
  assert.equal(report.products[6].step, 'Moisturize');
  assert.equal(report.products[6].offers[0].retailer, 'BuyBetter');
  assert.equal(report.products[6].offers[0].priceNgn, 12_800);
  assert.equal(report.products[6].offers[0].available, true);
  assert.equal(report.products[6].offers[1].retailer, 'CSi Grocery');
  assert.equal(report.products[6].offers[1].priceNgn, 13_000);
  assert.equal(report.products[6].offers[1].available, true);
  assert.equal(report.products[7].slug, 'aqua-rich-turmeric-vitamin-c-body-lotion-500ml');
  ...
  assert.equal(report.products[7].offers[0].priceNgn, 10_750);
  assert.equal(report.products[7].offers[0].available, true);
  assert.equal(report.products[7].offers[1].retailer, 'Kadimez Essentials');
  assert.equal(report.products[7].offers[1].priceNgn, 12_000);
  assert.equal(report.products[7].offers[1].available, true);
  assert.equal(report.products[8].slug, 'balance-niacinamide-blemish-recovery-serum-30ml');
  ...
  assert.equal(report.products[8].offers[0].priceNgn, 8_400);
  assert.equal(report.products[8].offers[0].available, true);
  assert.equal(report.products[8].offers[1].retailer, 'CSi Grocery');
  assert.equal(report.products[8].offers[1].priceNgn, 10_700);
  assert.equal(report.products[8].offers[1].available, true);
  assert.equal(report.products[9].slug, 'nineless-a-control-10-azelaic-acid-serum-30ml');
  assert.equal(report.products[9].category, 'Face');
  assert.equal(report.products[9].step, 'Treat');
  assert.equal(report.products[9].offers[0].retailer, 'Beauty by Daz');
  assert.equal(report.products[9].offers[0].priceNgn, 12_500);
  assert.equal(report.products[9].offers[1].retailer, 'BuyBetter');
  assert.equal(report.products[9].offers[1].priceNgn, 14_500);
  assert.equal(report.products[10].slug, 'nineless-mela-pro-rice-txa-toner-200ml');
  assert.equal(report.products[10].category, 'Face');
  assert.equal(report.products[10].step, 'Tone');
  assert.equal(report.products[10].offers[0].retailer, 'BuyBetter');
  assert.equal(report.products[10].offers[0].priceNgn, 15_500);
  assert.equal(report.products[10].offers[0].available, true);
  assert.equal(report.products[10].offers[1].retailer, 'Muna Cosmetics');
  assert.equal(report.products[10].offers[1].priceNgn, 19_000);
  assert.equal(report.products[10].offers[1].available, true);
  assert.equal(report.products[128].slug, 'facefacts-ceramide-oil-control-foaming-cleanser-400ml');
  assert.equal(report.products[128].category, 'Face');
  assert.equal(report.products[128].step, 'Cleanse');
  assert.equal(report.products[128].offers[0].retailer, 'CSi Grocery');
  assert.equal(report.products[128].offers[0].priceNgn, 7_500);
  assert.equal(report.products[128].offers[0].available, true);
  assert.equal(report.products[128].offers[1].retailer, '24Eleven');
  assert.equal(report.products[128].offers[1].priceNgn, 7_300);
  assert.equal(report.products[128].offers[1].available, true);
  assert.equal(report.products[117].slug, 'facefacts-ceramide-hydrating-gentle-cleanser-400ml');
  ...
  assert.equal(report.products[117].offers[0].priceNgn, 6_950);
  assert.equal(report.products[117].offers[1].retailer, 'Teeka4');
  assert.equal(report.products[117].offers[1].priceNgn, 7_200);
  assert.equal(report.products[117].offers[2].retailer, '24Eleven');
  assert.equal(report.products[117].offers[2].priceNgn, 7_300);
  assert.equal(report.products[11].slug, 'facefacts-ceramide-foaming-cleanser-400ml');
  assert.equal(report.products[11].category, 'Face');
  assert.equal(report.products[11].step, 'Cleanse');
  assert.equal(report.products[11].offers[0].retailer, 'BuyBetter');
  assert.equal(report.products[11].offers[0].priceNgn, 6_450);
  assert.equal(report.products[11].offers[0].available, true);
  assert.equal(report.products[11].offers[1].retailer, '24Eleven');
  assert.equal(report.products[11].offers[1].priceNgn, 6_800);
  assert.equal(report.products[11].offers[1].available, true);
  assert.equal(report.products[12].slug, 'de-la-cruz-acne-treatment-10-sulfur-73-7g');
  assert.equal(report.products[12].category, 'Face');
  assert.equal(report.products[12].step, 'Treat');
  assert.equal(report.products[12].offers[0].retailer, 'Beauty by Daz');
  assert.equal(report.products[12].offers[0].priceNgn, 12_500);
  assert.equal(report.products[12].offers[0].available, true);
  assert.equal(report.products[12].offers[1].retailer, 'BuyBetter');
  assert.equal(report.products[12].offers[1].priceNgn, 17_738);
  assert.equal(report.products[12].offers[1].available, false);
  assert.equal(report.products[13].slug, 'olay-super-serum-body-wash-normal-skin-547ml');
  assert.equal(report.products[13].category, 'Body');
  assert.equal(report.products[13].step, 'Cleanse');
  assert.equal(report.products[13].offers[0].retailer, 'BuyBetter');
  assert.equal(report.products[13].offers[0].priceNgn, 20_000);
  assert.equal(report.products[13].offers[0].available, true);
  assert.equal(report.products[13].offers[1].retailer, 'Perona Beauty');
  assert.equal(report.products[13].offers[1].priceNgn, 21_200);
  assert.equal(report.products[13].offers[1].available, true);
  assert.equal(report.products[14].slug, 'dove-calming-moisture-body-wash-547ml');
  assert.equal(report.products[14].category, 'Body');
  assert.equal(report.products[14].step, 'Cleanse');
  assert.equal(report.products[14].offers[0].retailer, 'BuyBetter');
  assert.equal(report.products[14].offers[0].priceNgn, 22_600);
  assert.equal(report.products[14].offers[0].available, true);
  assert.equal(report.products[14].offers[1].retailer, 'Teeka4');
  assert.equal(report.products[14].offers[1].priceNgn, 17_800);
  assert.equal(report.products[14].offers[1].available, false);
  assert.equal(report.products[17].slug, 'facefacts-vitamin-c-body-lotion-400ml');
  assert.equal(report.products[17].category, 'Body');
  assert.equal(report.products[17].step, 'Moisturize');
  assert.equal(report.products[17].offers[0].retailer, 'BuyBetter');
  assert.equal(report.products[17].offers[0].priceNgn, 6_095);
  assert.equal(report.products[17].offers[0].available, true);
  assert.equal(report.products[17].offers[1].retailer, 'Allure Beauty');
  assert.equal(report.products[17].offers[1].priceNgn, 8_000);
  assert.equal(report.products[17].offers[1].available, true);
});
