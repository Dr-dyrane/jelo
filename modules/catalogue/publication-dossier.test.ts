...
test('the checked-in publication manifest contains the verified neutral reference dossiers', () => {
  ...
  assert.equal(result.dossiers[16].candidateId, 'de-la-cruz-acne-treatment-10-sulfur-73-7g');
  assert.equal(result.dossiers[16].nigeria.exactOffers.length, 2);
  assert.equal(result.dossiers[17].candidateId, 'olay-super-serum-body-wash-normal-skin-547ml');
  assert.equal(result.dossiers[17].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[17].nigeria.exactOffers.map(offer => offer.priceNgn), [20_000, 21_200]);
  assert.equal(result.dossiers[17].rights.generationRecord?.outputSha256, result.dossiers[17].finalImage.sha256);
  assert.equal(result.dossiers[18].candidateId, 'dove-calming-moisture-body-wash-547ml');
  assert.equal(result.dossiers[18].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[18].nigeria.exactOffers.map(offer => offer.priceNgn), [22_600, 17_800]);
  assert.equal(result.dossiers[18].rights.generationRecord?.outputSha256, result.dossiers[18].finalImage.sha256);
  assert.equal(result.dossiers[128].candidateId, 'cerave-sa-smoothing-cleanser-473ml');
  ...
  assert.equal(result.dossiers[8].candidateId, 'garnier-vitamin-c-brightening-day-cream-50ml');
  ...
  assert.equal(result.dossiers[9].candidateId, 'aqua-rich-ceramide-body-lotion-500ml');
  assert.equal(result.dossiers[9].identity.name, 'Hydrate + Protect Body Lotion');
  assert.equal(result.dossiers[9].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[9].nigeria.exactOffers.map(offer => offer.priceNgn), [12_800, 13_000]);
  assert.equal(result.dossiers[9].nigeria.regulatoryStatus, 'pending');
  assert.equal(result.dossiers[9].rights.generationRecord?.outputSha256, result.dossiers[9].finalImage.sha256);
  assert.equal(result.dossiers[10].candidateId, 'aqua-rich-turmeric-vitamin-c-body-lotion-500ml');
  assert.equal(result.dossiers[10].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[10].nigeria.exactOffers.map(offer => offer.priceNgn), [10_750, 12_000]);
  ...
  assert.equal(result.dossiers[11].candidateId, 'balance-niacinamide-blemish-recovery-serum-30ml');
  assert.equal(result.dossiers[11].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[11].nigeria.exactOffers.map(offer => offer.priceNgn), [8_400, 10_700]);
  assert.equal(result.dossiers[11].nigeria.regulatoryStatus, 'pending');
  assert.equal(result.dossiers[11].rights.generationRecord?.outputSha256, result.dossiers[11].finalImage.sha256);
  assert.equal(result.dossiers[12].candidateId, 'nineless-a-control-10-azelaic-acid-serum-30ml');
  assert.equal(result.dossiers[12].identity.packageVersion, 'Original green dropper bottle');
  assert.equal(result.dossiers[12].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[12].nigeria.exactOffers.map(offer => offer.priceNgn), [12_500, 14_500]);
  assert.equal(result.dossiers[12].rights.generationRecord?.outputSha256, result.dossiers[12].finalImage.sha256);
  assert.equal(result.dossiers[13].candidateId, 'nineless-mela-pro-rice-txa-toner-200ml');
  assert.equal(result.dossiers[13].identity.packageVersion, 'Original translucent bottle with orange cap');
  assert.equal(result.dossiers[13].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[13].nigeria.exactOffers.map(offer => offer.priceNgn), [15_500, 19_000]);
  assert.equal(result.dossiers[13].rights.generationRecord?.outputSha256, result.dossiers[13].finalImage.sha256);
  assert.equal(result.dossiers[14].candidateId, 'facefacts-ceramide-oil-control-foaming-cleanser-400ml');
  assert.equal(result.dossiers[14].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[14].nigeria.exactOffers.map(offer => offer.priceNgn), [6_880, 7_300]);
  assert.equal(result.dossiers[14].rights.generationRecord?.outputSha256, result.dossiers[14].finalImage.sha256);
  assert.equal(result.dossiers[124].candidateId, 'facefacts-ceramide-hydrating-gentle-cleanser-400ml');
  assert.equal(result.dossiers[124].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[124].nigeria.exactOffers.map(offer => offer.priceNgn), [6_950, 7_200, 7_300]);
  assert.equal(result.dossiers[124].rights.generationRecord?.outputSha256, result.dossiers[124].finalImage.sha256);
  assert.equal(result.dossiers[15].candidateId, 'facefacts-ceramide-foaming-cleanser-400ml');
  assert.equal(result.dossiers[15].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[15].nigeria.exactOffers.map(offer => offer.priceNgn), [6_450, 6_800]);
  assert.equal(result.dossiers[15].rights.generationRecord?.outputSha256, result.dossiers[15].finalImage.sha256);
  assert.equal(result.dossiers[21].candidateId, 'facefacts-vitamin-c-body-lotion-400ml');
  assert.equal(result.dossiers[21].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[21].nigeria.exactOffers.map(offer => offer.priceNgn), [6_095, 8_000]);
  assert.equal(result.dossiers[21].rights.generationRecord?.outputSha256, result.dossiers[21].finalImage.sha256);
});
...
test('the checked-in release manifest explicitly publishes the verified neutral references', () => {
  ...
  assert.equal(report.products[7].slug, 'cerave-acne-foaming-cream-wash-10-150ml');
  ...
  assert.equal(report.products[7].offers[0].priceNgn, 23_850);
  assert.equal(report.products[7].offers[0].available, true);
  assert.equal(report.products[7].offers[1].retailer, 'Teeka4');
  assert.equal(report.products[7].offers[1].priceNgn, 24_500);
  assert.equal(report.products[7].offers[1].available, true);
  assert.equal(report.products[128].slug, 'cerave-sa-smoothing-cleanser-473ml');
  ...
  assert.equal(report.products[128].offers[0].priceNgn, 20_900);
  assert.equal(report.products[128].offers[0].available, true);
  assert.equal(report.products[128].offers[1].retailer, '24Eleven');
  assert.equal(report.products[128].offers[1].priceNgn, 23_800);
  assert.equal(report.products[128].offers[1].available, true);
  assert.equal(report.products[8].slug, 'garnier-vitamin-c-brightening-day-cream-50ml');
  ...
  assert.equal(report.products[8].offers[0].priceNgn, 12_728);
  assert.equal(report.products[8].offers[0].available, true);
  assert.equal(report.products[8].offers[1].retailer, 'Teeka4');
  assert.equal(report.products[8].offers[1].priceNgn, 11_833);
  assert.equal(report.products[8].offers[1].available, true);
  assert.equal(report.products[9].slug, 'aqua-rich-ceramide-body-lotion-500ml');
  ...
  assert.equal(report.products[9].offers[0].priceNgn, 12_800);
  assert.equal(report.products[9].offers[0].available, true);
  assert.equal(report.products[9].offers[1].retailer, 'CSi Grocery');
  assert.equal(report.products[9].offers[1].priceNgn, 13_000);
  assert.equal(report.products[9].offers[1].available, true);
  assert.equal(report.products[10].slug, 'aqua-rich-turmeric-vitamin-c-body-lotion-500ml');
  assert.equal(report.products[10].category, 'Body');
  assert.equal(report.products[10].step, 'Moisturize');
  assert.equal(report.products[10].offers[0].retailer, 'BuyBetter');
  assert.equal(report.products[10].offers[0].priceNgn, 10_750);
  assert.equal(report.products[10].offers[0].available, true);
  assert.equal(report.products[10].offers[1].retailer, 'Kadimez Essentials');
  assert.equal(report.products[10].offers[1].priceNgn, 12_000);
  assert.equal(report.products[10].offers[1].available, true);
  assert.equal(report.products[11].slug, 'balance-niacinamide-blemish-recovery-serum-30ml');
  assert.equal(report.products[11].category, 'Face');
  assert.equal(report.products[11].step, 'Treat');
  assert.equal(checkedInReleaseManifest.releases[11].recommendationEligible, false);
  assert.equal(report.products[11].offers[0].retailer, 'BuyBetter');
  assert.equal(report.products[11].offers[0].priceNgn, 8_400);
  assert.equal(report.products[11].offers[0].available, true);
  assert.equal(report.products[11].offers[1].retailer, 'CSi Grocery');
  assert.equal(report.products[11].offers[1].priceNgn, 10_700);
  assert.equal(report.products[11].offers[1].available, true);
  assert.equal(report.products[12].slug, 'nineless-a-control-10-azelaic-acid-serum-30ml');
  ...
  assert.equal(report.products[12].offers[1].priceNgn, 14_500);
  assert.equal(report.products[13].slug, 'nineless-mela-pro-rice-txa-toner-200ml');
  assert.equal(report.products[13].category, 'Face');
  assert.equal(report.products[13].step, 'Tone');
  assert.equal(report.products[13].offers[0].retailer, 'BuyBetter');
  assert.equal(report.products[13].offers[0].priceNgn, 15_500);
  assert.equal(report.products[13].offers[0].available, true);
  assert.equal(report.products[13].offers[1].retailer, 'Muna Cosmetics');
  assert.equal(report.products[13].offers[1].priceNgn, 19_000);
  assert.equal(report.products[13].offers[1].available, true);
  assert.equal(report.products[14].slug, 'facefacts-ceramide-oil-control-foaming-cleanser-400ml');
  ...
  assert.equal(report.products[14].offers[0].priceNgn, 6_880);
  assert.equal(report.products[14].offers[0].available, true);
  assert.equal(report.products[14].offers[1].retailer, '24Eleven');
  assert.equal(report.products[14].offers[1].priceNgn, 7_300);
  assert.equal(report.products[14].offers[1].available, true);
  assert.equal(report.products[124].slug, 'facefacts-ceramide-hydrating-gentle-cleanser-400ml');
  ...
  assert.equal(report.products[124].offers[0].priceNgn, 6_950);
  assert.equal(report.products[124].offers[1].retailer, 'Teeka4');
  assert.equal(report.products[124].offers[1].priceNgn, 7_200);
  assert.equal(report.products[124].offers[2].retailer, '24Eleven');
  assert.equal(report.products[124].offers[2].priceNgn, 7_300);
  assert.equal(report.products[15].slug, 'facefacts-ceramide-foaming-cleanser-400ml');
  ...
  assert.equal(report.products[15].offers[0].priceNgn, 6_450);
  assert.equal(report.products[15].offers[0].available, true);
  assert.equal(report.products[15].offers[1].retailer, '24Eleven');
  assert.equal(report.products[15].offers[1].priceNgn, 6_800);
  assert.equal(report.products[15].offers[1].available, true);
  assert.equal(report.products[16].slug, 'de-la-cruz-acne-treatment-10-sulfur-73-7g');
  assert.equal(report.products[16].category, 'Face');
  assert.equal(report.products[16].step, 'Treat');
  assert.equal(report.products[16].offers[0].retailer, 'Beauty by Daz');
  assert.equal(report.products[16].offers[0].priceNgn, 12_500);
  assert.equal(report.products[16].offers[0].available, true);
  assert.equal(report.products[16].offers[1].retailer, 'BuyBetter');
  assert.equal(report.products[16].offers[1].priceNgn, 17_738);
  assert.equal(report.products[16].offers[1].available, false);
  assert.equal(report.products[17].slug, 'olay-super-serum-body-wash-normal-skin-547ml');
  ...
  assert.equal(report.products[17].offers[0].priceNgn, 20_000);
  assert.equal(report.products[17].offers[0].available, true);
  assert.equal(report.products[17].offers[1].retailer, 'Perona Beauty');
  assert.equal(report.products[17].offers[1].priceNgn, 21_200);
  assert.equal(report.products[17].offers[1].available, true);
  assert.equal(report.products[18].slug, 'dove-calming-moisture-body-wash-547ml');
  assert.equal(report.products[18].category, 'Body');
  assert.equal(report.products[18].step, 'Cleanse');
  assert.equal(report.products[18].offers[0].retailer, 'BuyBetter');
  assert.equal(report.products[18].offers[0].priceNgn, 22_600);
  assert.equal(report.products[18].offers[0].available, true);
  assert.equal(report.products[18].offers[1].retailer, 'Teeka4');
  assert.equal(report.products[18].offers[1].priceNgn, 17_800);
  assert.equal(report.products[18].offers[1].available, false);
  assert.equal(report.products[21].slug, 'facefacts-vitamin-c-body-lotion-400ml');
  assert.equal(report.products[21].category, 'Body');
  assert.equal(report.products[21].step, 'Moisturize');
  assert.equal(report.products[21].offers[0].retailer, 'BuyBetter');
  assert.equal(report.products[21].offers[0].priceNgn, 6_095);
  assert.equal(report.products[21].offers[0].available, true);
  assert.equal(report.products[21].offers[1].retailer, 'Allure Beauty');
  assert.equal(report.products[21].offers[1].priceNgn, 8_000);
  assert.equal(report.products[21].offers[1].available, true);
});
