...
test('the checked-in publication manifest contains the verified neutral reference dossiers', () => {
  ...
  assert.equal(result.dossiers[113].candidateId, 'cerave-hydrating-cleanser-473ml');
  assert.equal(result.dossiers[113].nigeria.regulatoryStatus, 'pending');
  assert.equal(result.dossiers[114].candidateId, 'cerave-moisturising-cream-454g');
  assert.equal(result.dossiers[114].nigeria.regulatoryStatus, 'pending');
  assert.equal(result.dossiers[117].candidateId, 'eucerin-oil-control-sun-gel-cream-spf50-50ml');
  ...
  assert.equal(result.dossiers[118].candidateId, 'eucerin-urearepair-plus-10-urea-body-lotion-250ml');
  ...
  assert.equal(result.dossiers[115].candidateId, 'dove-melanin-even-tone-body-wash-18-5oz');
  ...
  assert.equal(result.dossiers[116].candidateId, 'keracare-dry-itchy-scalp-conditioner-950ml');
  ...
  assert.equal(result.dossiers[107].candidateId, 'balance-salicylic-acid-zinc-clarifying-toner-200ml');
  ...
  assert.equal(result.dossiers[103].candidateId, 'cerave-acne-foaming-cream-wash-10-150ml');
  ...
  assert.equal(result.dossiers[123].candidateId, 'de-la-cruz-acne-treatment-10-sulfur-73-7g');
  assert.equal(result.dossiers[123].nigeria.exactOffers.length, 2);
  assert.equal(result.dossiers[124].candidateId, 'olay-super-serum-body-wash-normal-skin-547ml');
  assert.equal(result.dossiers[124].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[124].nigeria.exactOffers.map(offer => offer.priceNgn), [20_963, 21_200]);
  assert.equal(result.dossiers[124].rights.generationRecord?.outputSha256, result.dossiers[124].finalImage.sha256);
  assert.equal(result.dossiers[125].candidateId, 'sheamoisture-jamaican-black-castor-oil-shampoo-384ml');
  assert.equal(result.dossiers[125].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[125].nigeria.exactOffers.map(offer => offer.priceNgn), [12_685, 13_300]);
  assert.equal(result.dossiers[125].rights.generationRecord?.outputSha256, result.dossiers[125].finalImage.sha256);
  assert.equal(result.dossiers[126].candidateId, 'tresemme-keratin-smooth-weightless-conditioner-828ml');
  assert.equal(result.dossiers[126].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[126].nigeria.exactOffers.map(offer => offer.priceNgn), [7_950, 9_138, 9_900]);
  assert.equal(result.dossiers[126].rights.generationRecord, undefined);
  assert.equal(result.dossiers[127].candidateId, 'laroche-posay-mela-b3-serum-30ml');
  assert.equal(result.dossiers[127].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[127].nigeria.exactOffers.map(offer => offer.priceNgn), [48_500, 44_700]);
  assert.equal(result.dossiers[128].candidateId, 'anua-azelaic-acid-10-hyaluron-redness-soothing-serum-30ml');
  assert.equal(result.dossiers[128].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[128].nigeria.exactOffers.map(offer => offer.priceNgn), [18_000, 16_999]);
  assert.equal(result.dossiers[102].candidateId, 'cerave-sa-smoothing-cleanser-473ml');
  ...
  assert.equal(result.dossiers[108].candidateId, 'garnier-vitamin-c-brightening-day-cream-50ml');
  ...
  assert.equal(result.dossiers[119].candidateId, 'aqua-rich-ceramide-body-lotion-500ml');
  assert.equal(result.dossiers[119].identity.name, 'Hydrate + Protect Body Lotion');
  assert.equal(result.dossiers[119].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[119].nigeria.exactOffers.map(offer => offer.priceNgn), [11_288, 13_000]);
  assert.equal(result.dossiers[119].nigeria.regulatoryStatus, 'pending');
  assert.equal(result.dossiers[119].rights.generationRecord?.outputSha256, result.dossiers[119].finalImage.sha256);
  assert.equal(result.dossiers[112].candidateId, 'aqua-rich-turmeric-vitamin-c-body-lotion-500ml');
  assert.equal(result.dossiers[112].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[112].nigeria.exactOffers.map(offer => offer.priceNgn), [12_800, 12_000]);
  ...
  assert.equal(result.dossiers[120].candidateId, 'balance-niacinamide-blemish-recovery-serum-30ml');
  assert.equal(result.dossiers[120].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[120].nigeria.exactOffers.map(offer => offer.priceNgn), [8_400, 10_700]);
  assert.equal(result.dossiers[120].nigeria.regulatoryStatus, 'pending');
  assert.equal(result.dossiers[120].rights.generationRecord?.outputSha256, result.dossiers[120].finalImage.sha256);
  assert.equal(result.dossiers[121].candidateId, 'nineless-a-control-10-azelaic-acid-serum-30ml');
  assert.equal(result.dossiers[121].identity.packageVersion, 'Original green dropper bottle');
  assert.equal(result.dossiers[121].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[121].nigeria.exactOffers.map(offer => offer.priceNgn), [12_500, 14_500]);
  assert.equal(result.dossiers[121].rights.generationRecord?.outputSha256, result.dossiers[121].finalImage.sha256);
  assert.equal(result.dossiers[122].candidateId, 'nineless-mela-pro-rice-txa-toner-200ml');
  assert.equal(result.dossiers[122].identity.packageVersion, 'Original translucent bottle with orange cap');
  assert.equal(result.dossiers[122].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[122].nigeria.exactOffers.map(offer => offer.priceNgn), [15_500, 19_000]);
  assert.equal(result.dossiers[122].rights.generationRecord?.outputSha256, result.dossiers[122].finalImage.sha256);
  assert.equal(result.dossiers[109].candidateId, 'facefacts-ceramide-oil-control-foaming-cleanser-400ml');
  assert.equal(result.dossiers[109].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[109].nigeria.exactOffers.map(offer => offer.priceNgn), [7_500, 7_300]);
  assert.equal(result.dossiers[109].rights.generationRecord?.outputSha256, result.dossiers[109].finalImage.sha256);
  assert.equal(result.dossiers[98].candidateId, 'facefacts-ceramide-hydrating-gentle-cleanser-400ml');
  assert.equal(result.dossiers[98].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[98].nigeria.exactOffers.map(offer => offer.priceNgn), [6_950, 7_200, 7_300]);
  assert.equal(result.dossiers[98].rights.generationRecord?.outputSha256, result.dossiers[98].finalImage.sha256);
  assert.equal(result.dossiers[110].candidateId, 'facefacts-ceramide-foaming-cleanser-400ml');
  assert.equal(result.dossiers[110].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[110].nigeria.exactOffers.map(offer => offer.priceNgn), [6_450, 6_800]);
  assert.equal(result.dossiers[110].rights.generationRecord?.outputSha256, result.dossiers[110].finalImage.sha256);
  assert.equal(result.dossiers[111].candidateId, 'facefacts-vitamin-c-body-lotion-400ml');
  assert.equal(result.dossiers[111].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[111].nigeria.exactOffers.map(offer => offer.priceNgn), [6_095, 8_000]);
...
test('the checked-in release manifest explicitly publishes the verified neutral references', () => {
  ...
  assert.equal(report.products[113].slug, 'cerave-hydrating-cleanser-473ml');
  assert.equal(report.products[113].offers[0].priceNgn, 15_265);
  assert.equal(report.products[114].slug, 'cerave-moisturising-cream-454g');
  assert.equal(report.products[114].offers[0].priceNgn, 22_500);
  assert.equal(report.products[117].slug, 'eucerin-oil-control-sun-gel-cream-spf50-50ml');
  assert.equal(report.products[117].offers[0].retailer, 'Beauty by Daz');
  assert.equal(report.products[117].offers[0].priceNgn, 19_100);
  ...
  assert.equal(report.products[117].offers[1].priceNgn, 17_363);
  assert.equal(report.products[117].offers[1].available, false);
  assert.equal(report.products[118].slug, 'eucerin-urearepair-plus-10-urea-body-lotion-250ml');
  assert.equal(report.products[118].category, 'Body');
  assert.equal(report.products[118].offers[0].retailer, 'BuyBetter');
  assert.equal(report.products[118].offers[0].priceNgn, 25_000);
  assert.equal(report.products[118].offers[0].available, false);
  assert.equal(report.products[118].offers[1].retailer, 'Nectar Beauty Hub');
  assert.equal(report.products[118].offers[1].priceNgn, 27_000);
  assert.equal(report.products[118].offers[1].available, true);
  assert.equal(report.products[115].slug, 'dove-melanin-even-tone-body-wash-18-5oz');
  ...
  assert.equal(report.products[115].offers[0].priceNgn, 22_600);
  assert.equal(report.products[115].offers[0].available, true);
  assert.equal(report.products[115].offers[1].retailer, 'Perona Beauty');
  assert.equal(report.products[115].offers[1].priceNgn, 20_000);
  assert.equal(report.products[115].offers[1].available, true);
  assert.equal(report.products[116].slug, 'keracare-dry-itchy-scalp-conditioner-950ml');
  ...
  assert.equal(report.products[116].offers[0].priceNgn, 38_485);
  assert.equal(report.products[116].offers[0].available, false);
  assert.equal(report.products[116].offers[1].retailer, 'Ediths Essentials');
  assert.equal(report.products[116].offers[1].priceNgn, 43_485);
  assert.equal(report.products[116].offers[1].available, true);
  assert.equal(report.products[107].slug, 'balance-salicylic-acid-zinc-clarifying-toner-200ml');
  ...
  assert.equal(report.products[107].step, 'Tone');
  assert.equal(report.products[107].offers[0].retailer, 'BuyBetter');
  assert.equal(report.products[107].offers[0].priceNgn, 8_400);
  assert.equal(report.products[107].offers[0].available, true);
  assert.equal(report.products[107].offers[1].retailer, '24Eleven');
  assert.equal(report.products[107].offers[1].priceNgn, 9_200);
  assert.equal(report.products[107].offers[1].available, true);
  assert.equal(report.products[103].slug, 'cerave-acne-foaming-cream-wash-10-150ml');
  assert.equal(report.products[103].category, 'Face');
  assert.equal(report.products[103].step, 'Cleanse');
  assert.equal(report.products[103].offers[0].retailer, 'Beauty by Daz');
  assert.equal(report.products[103].offers[0].priceNgn, 23_850);
  assert.equal(report.products[103].offers[0].available, true);
  assert.equal(report.products[103].offers[1].retailer, 'Teeka4');
  assert.equal(report.products[103].offers[1].priceNgn, 24_500);
  assert.equal(report.products[103].offers[1].available, true);
  assert.equal(report.products[102].slug, 'cerave-sa-smoothing-cleanser-473ml');
  assert.equal(report.products[102].category, 'Face');
  assert.equal(report.products[102].step, 'Cleanse');
  assert.equal(report.products[102].offers[0].retailer, 'Teeka4');
  assert.equal(report.products[102].offers[0].priceNgn, 20_900);
  assert.equal(report.products[102].offers[0].available, true);
  assert.equal(report.products[102].offers[1].retailer, '24Eleven');
  assert.equal(report.products[102].offers[1].priceNgn, 23_800);
  assert.equal(report.products[102].offers[1].available, true);
  assert.equal(report.products[108].slug, 'garnier-vitamin-c-brightening-day-cream-50ml');
  assert.equal(report.products[108].category, 'Face');
  assert.equal(report.products[108].step, 'Moisturize');
  assert.equal(report.products[108].offers[0].retailer, 'Care to Beauty');
  assert.equal(report.products[108].offers[0].priceNgn, 17_253.98);
  assert.equal(report.products[108].offers[0].available, true);
  assert.equal(report.products[108].offers[1].retailer, 'Teeka4');
  assert.equal(report.products[108].offers[1].priceNgn, 11_833);
  assert.equal(report.products[108].offers[1].available, true);
  assert.equal(report.products[119].slug, 'aqua-rich-ceramide-body-lotion-500ml');
  ...
  assert.equal(report.products[119].offers[0].priceNgn, 11_288);
  assert.equal(report.products[119].offers[0].available, false);
  assert.equal(report.products[119].offers[1].retailer, 'CSi Grocery');
  assert.equal(report.products[119].offers[1].priceNgn, 13_000);
  assert.equal(report.products[119].offers[1].available, true);
  assert.equal(report.products[112].slug, 'aqua-rich-turmeric-vitamin-c-body-lotion-500ml');
  ...
  assert.equal(report.products[112].offers[0].priceNgn, 12_800);
  assert.equal(report.products[112].offers[0].available, true);
  assert.equal(report.products[112].offers[1].retailer, 'Kadimez Essentials');
  assert.equal(report.products[112].offers[1].priceNgn, 12_000);
  assert.equal(report.products[112].offers[1].available, true);
  assert.equal(report.products[120].slug, 'balance-niacinamide-blemish-recovery-serum-30ml');
  ...
  assert.equal(report.products[120].offers[0].priceNgn, 8_400);
  assert.equal(report.products[120].offers[0].available, true);
  assert.equal(report.products[120].offers[1].retailer, 'CSi Grocery');
  assert.equal(report.products[120].offers[1].priceNgn, 10_700);
  assert.equal(report.products[120].offers[1].available, true);
  assert.equal(report.products[121].slug, 'nineless-a-control-10-azelaic-acid-serum-30ml');
  ...
  assert.equal(report.products[121].offers[1].priceNgn, 14_500);
  assert.equal(report.products[122].slug, 'nineless-mela-pro-rice-txa-toner-200ml');
  assert.equal(report.products[122].category, 'Face');
  assert.equal(report.products[122].step, 'Tone');
  assert.equal(report.products[122].offers[0].retailer, 'BuyBetter');
  assert.equal(report.products[122].offers[0].priceNgn, 15_500);
  assert.equal(report.products[122].offers[0].available, false);
  assert.equal(report.products[122].offers[1].retailer, 'Muna Cosmetics');
  assert.equal(report.products[122].offers[1].priceNgn, 19_000);
  assert.equal(report.products[122].offers[1].available, true);
  assert.equal(report.products[109].slug, 'facefacts-ceramide-oil-control-foaming-cleanser-400ml');
  assert.equal(report.products[109].category, 'Face');
  assert.equal(report.products[109].step, 'Cleanse');
  assert.equal(report.products[109].offers[0].retailer, 'CSi Grocery');
  assert.equal(report.products[109].offers[0].priceNgn, 7_500);
  assert.equal(report.products[109].offers[0].available, true);
  assert.equal(report.products[109].offers[1].retailer, '24Eleven');
  assert.equal(report.products[109].offers[1].priceNgn, 7_300);
  assert.equal(report.products[109].offers[1].available, true);
  assert.equal(report.products[98].slug, 'facefacts-ceramide-hydrating-gentle-cleanser-400ml');
  ...
  assert.equal(report.products[98].offers[0].priceNgn, 6_950);
  assert.equal(report.products[98].offers[1].retailer, 'Teeka4');
  assert.equal(report.products[98].offers[1].priceNgn, 7_200);
  assert.equal(report.products[98].offers[2].retailer, '24Eleven');
  assert.equal(report.products[98].offers[2].priceNgn, 7_300);
  assert.equal(report.products[110].slug, 'facefacts-ceramide-foaming-cleanser-400ml');
  ...
  assert.equal(report.products[110].offers[0].priceNgn, 6_450);
  assert.equal(report.products[110].offers[0].available, true);
  assert.equal(report.products[110].offers[1].retailer, '24Eleven');
  assert.equal(report.products[110].offers[1].priceNgn, 6_800);
  assert.equal(report.products[110].offers[1].available, true);
  assert.equal(report.products[123].slug, 'de-la-cruz-acne-treatment-10-sulfur-73-7g');
  ...
  assert.equal(report.products[123].offers[0].priceNgn, 12_500);
  assert.equal(report.products[123].offers[0].available, true);
  assert.equal(report.products[123].offers[1].retailer, 'BuyBetter');
  assert.equal(report.products[123].offers[1].priceNgn, 17_738);
  assert.equal(report.products[123].offers[1].available, false);
  assert.equal(report.products[124].slug, 'olay-super-serum-body-wash-normal-skin-547ml');
  ...
  assert.equal(report.products[124].offers[0].priceNgn, 20_963);
  assert.equal(report.products[124].offers[0].available, true);
  assert.equal(report.products[124].offers[1].retailer, 'Perona Beauty');
  assert.equal(report.products[124].offers[1].priceNgn, 21_200);
  assert.equal(report.products[124].offers[1].available, true);
  assert.equal(report.products[125].slug, 'sheamoisture-jamaican-black-castor-oil-shampoo-384ml');
  ...
  assert.equal(report.products[125].offers[0].priceNgn, 12_685);
  assert.equal(report.products[125].offers[0].available, false);
  assert.equal(report.products[125].offers[1].retailer, 'Perfect Trust Beauty');
  assert.equal(report.products[125].offers[1].priceNgn, 13_300);
  assert.equal(report.products[125].offers[1].available, true);
  assert.equal(report.products[126].slug, 'tresemme-keratin-smooth-weightless-conditioner-828ml');
  ...
  assert.equal(report.products[126].offers[0].priceNgn, 7_950);
  assert.equal(report.products[126].offers[0].available, false);
  assert.equal(report.products[126].offers[1].retailer, 'BuyBetter');
  assert.equal(report.products[126].offers[1].priceNgn, 9_138);
  assert.equal(report.products[126].offers[1].available, false);
  assert.equal(report.products[126].offers[2].retailer, 'Perfect Trust Beauty');
  assert.equal(report.products[126].offers[2].priceNgn, 9_900);
  assert.equal(report.products[126].offers[2].available, true);
  assert.equal(report.products[127].slug, 'laroche-posay-mela-b3-serum-30ml');
  assert.equal(report.products[127].category, 'Face');
  assert.equal(report.products[127].step, 'Treat');
  assert.equal(report.products[127].offers[0].retailer, 'Lux Beauty');
  assert.equal(report.products[127].offers[0].priceNgn, 48_500);
  assert.equal(report.products[127].offers[0].available, true);
  assert.equal(report.products[127].offers[1].retailer, 'Beauty by Daz');
  assert.equal(report.products[127].offers[1].priceNgn, 44_700);
  assert.equal(report.products[127].offers[1].available, false);
  assert.equal(report.products[128].slug, 'anua-azelaic-acid-10-hyaluron-redness-soothing-serum-30ml');
  assert.equal(report.products[128].category, 'Face');
  assert.equal(report.products[128].step, 'Treat');
  assert.equal(report.products[128].offers[0].retailer, 'BuyBetter');
  assert.equal(report.products[128].offers[0].priceNgn, 18_000);
  assert.equal(report.products[128].offers[0].available, true);
  assert.equal(report.products[128].offers[1].retailer, 'Teeka4');
  assert.equal(report.products[128].offers[1].priceNgn, 16_999);
  assert.equal(report.products[128].offers[1].available, false);
  assert.equal(report.products[111].slug, 'facefacts-vitamin-c-body-lotion-400ml');
  ...
  assert.equal(report.products[111].offers[0].priceNgn, 6_095);
  assert.equal(report.products[111].offers[0].available, true);
  assert.equal(report.products[111].offers[1].retailer, 'Allure Beauty');
  assert.equal(report.products[111].offers[1].priceNgn, 8_000);
