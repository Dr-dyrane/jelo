...
test('the checked-in publication manifest contains the verified neutral reference dossiers', () => {
  ...
  assert.equal(result.dossiers[127].candidateId, 'cerave-hydrating-cleanser-473ml');
  assert.equal(result.dossiers[127].nigeria.regulatoryStatus, 'pending');
  assert.equal(result.dossiers[128].candidateId, 'cerave-moisturising-cream-454g');
  ...
  assert.equal(result.dossiers[2].candidateId, 'dove-melanin-even-tone-body-wash-18-5oz');
  ...
  assert.equal(result.dossiers[3].candidateId, 'keracare-dry-itchy-scalp-conditioner-950ml');
  ...
  assert.equal(result.dossiers[121].candidateId, 'balance-salicylic-acid-zinc-clarifying-toner-200ml');
  ...
  assert.equal(result.dossiers[117].candidateId, 'cerave-acne-foaming-cream-wash-10-150ml');
  ...
  assert.equal(result.dossiers[8].candidateId, 'de-la-cruz-acne-treatment-10-sulfur-73-7g');
  assert.equal(result.dossiers[8].nigeria.exactOffers.length, 2);
  assert.equal(result.dossiers[9].candidateId, 'olay-super-serum-body-wash-normal-skin-547ml');
  assert.equal(result.dossiers[9].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[9].nigeria.exactOffers.map(offer => offer.priceNgn), [20_000, 21_200]);
  assert.equal(result.dossiers[9].rights.generationRecord?.outputSha256, result.dossiers[9].finalImage.sha256);
  assert.equal(result.dossiers[10].candidateId, 'dove-calming-moisture-body-wash-547ml');
  assert.equal(result.dossiers[10].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[10].nigeria.exactOffers.map(offer => offer.priceNgn), [22_600, 17_800]);
  assert.equal(result.dossiers[10].rights.generationRecord?.outputSha256, result.dossiers[10].finalImage.sha256);
  assert.equal(result.dossiers[116].candidateId, 'cerave-sa-smoothing-cleanser-473ml');
  ...
  assert.equal(result.dossiers[122].candidateId, 'garnier-vitamin-c-brightening-day-cream-50ml');
  ...
  assert.equal(result.dossiers[4].candidateId, 'aqua-rich-ceramide-body-lotion-500ml');
  assert.equal(result.dossiers[4].identity.name, 'Hydrate + Protect Body Lotion');
  assert.equal(result.dossiers[4].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[4].nigeria.exactOffers.map(offer => offer.priceNgn), [12_800, 13_000]);
  assert.equal(result.dossiers[4].nigeria.regulatoryStatus, 'pending');
  assert.equal(result.dossiers[4].rights.generationRecord?.outputSha256, result.dossiers[4].finalImage.sha256);
  assert.equal(result.dossiers[126].candidateId, 'aqua-rich-turmeric-vitamin-c-body-lotion-500ml');
  assert.equal(result.dossiers[126].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[126].nigeria.exactOffers.map(offer => offer.priceNgn), [12_800, 12_000]);
  ...
  assert.equal(result.dossiers[5].candidateId, 'balance-niacinamide-blemish-recovery-serum-30ml');
  assert.equal(result.dossiers[5].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[5].nigeria.exactOffers.map(offer => offer.priceNgn), [8_400, 10_700]);
  ...
  assert.equal(result.dossiers[6].candidateId, 'nineless-a-control-10-azelaic-acid-serum-30ml');
  assert.equal(result.dossiers[6].identity.packageVersion, 'Original green dropper bottle');
  assert.equal(result.dossiers[6].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[6].nigeria.exactOffers.map(offer => offer.priceNgn), [12_500, 14_500]);
  assert.equal(result.dossiers[6].rights.generationRecord?.outputSha256, result.dossiers[6].finalImage.sha256);
  assert.equal(result.dossiers[7].candidateId, 'nineless-mela-pro-rice-txa-toner-200ml');
  assert.equal(result.dossiers[7].identity.packageVersion, 'Original translucent bottle with orange cap');
  assert.equal(result.dossiers[7].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[7].nigeria.exactOffers.map(offer => offer.priceNgn), [15_500, 19_000]);
  assert.equal(result.dossiers[7].rights.generationRecord?.outputSha256, result.dossiers[7].finalImage.sha256);
  assert.equal(result.dossiers[123].candidateId, 'facefacts-ceramide-oil-control-foaming-cleanser-400ml');
  assert.equal(result.dossiers[123].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[123].nigeria.exactOffers.map(offer => offer.priceNgn), [7_500, 7_300]);
  assert.equal(result.dossiers[123].rights.generationRecord?.outputSha256, result.dossiers[123].finalImage.sha256);
  assert.equal(result.dossiers[112].candidateId, 'facefacts-ceramide-hydrating-gentle-cleanser-400ml');
  assert.equal(result.dossiers[112].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[112].nigeria.exactOffers.map(offer => offer.priceNgn), [6_950, 7_200, 7_300]);
  assert.equal(result.dossiers[112].rights.generationRecord?.outputSha256, result.dossiers[112].finalImage.sha256);
  assert.equal(result.dossiers[124].candidateId, 'facefacts-ceramide-foaming-cleanser-400ml');
  assert.equal(result.dossiers[124].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[124].nigeria.exactOffers.map(offer => offer.priceNgn), [6_450, 6_800]);
  assert.equal(result.dossiers[124].rights.generationRecord?.outputSha256, result.dossiers[124].finalImage.sha256);
  assert.equal(result.dossiers[125].candidateId, 'facefacts-vitamin-c-body-lotion-400ml');
  assert.equal(result.dossiers[125].nigeria.marketRoute, 'tier-a');
  assert.deepEqual(result.dossiers[125].nigeria.exactOffers.map(offer => offer.priceNgn), [6_095, 8_000]);
...
test('the checked-in release manifest explicitly publishes the verified neutral references', () => {
  ...
  assert.equal(report.products[127].slug, 'cerave-hydrating-cleanser-473ml');
  assert.equal(report.products[127].offers[0].priceNgn, 15_265);
  assert.equal(report.products[128].slug, 'cerave-moisturising-cream-454g');
  assert.equal(report.products[128].offers[0].priceNgn, 22_500);
  ...
  assert.equal(report.products[2].slug, 'dove-melanin-even-tone-body-wash-18-5oz');
  assert.equal(report.products[2].category, 'Body');
  assert.equal(report.products[2].step, 'Cleanse');
  assert.equal(report.products[2].offers[0].retailer, 'BuyBetter');
  assert.equal(report.products[2].offers[0].priceNgn, 22_600);
  assert.equal(report.products[2].offers[0].available, true);
  assert.equal(report.products[2].offers[1].retailer, 'Teeka4');
  assert.equal(report.products[2].offers[1].priceNgn, 17_800);
  assert.equal(report.products[2].offers[1].available, false);
  assert.equal(report.products[3].slug, 'keracare-dry-itchy-scalp-conditioner-950ml');
  assert.equal(report.products[3].category, 'Hair');
  assert.equal(report.products[3].step, 'Condition');
  assert.equal(report.products[3].offers[0].retailer, 'BuyBetter');
  assert.equal(report.products[3].offers[0].priceNgn, 38_485);
  assert.equal(report.products[3].offers[0].available, true);
  assert.equal(report.products[3].offers[1].retailer, 'Ediths Essentials');
  assert.equal(report.products[3].offers[1].priceNgn, 43_485);
  assert.equal(report.products[3].offers[1].available, true);
  assert.equal(report.products[121].slug, 'balance-salicylic-acid-zinc-clarifying-toner-200ml');
  ...
  assert.equal(report.products[121].offers[1].priceNgn, 9_200);
  assert.equal(report.products[121].offers[1].available, true);
  assert.equal(report.products[117].slug, 'cerave-acne-foaming-cream-wash-10-150ml');
  ...
  assert.equal(report.products[126].offers[0].priceNgn, 12_800);
  assert.equal(report.products[126].offers[0].available, true);
  assert.equal(report.products[126].offers[1].retailer, 'Kadimez Essentials');
  assert.equal(report.products[126].offers[1].priceNgn, 12_000);
  assert.equal(report.products[126].offers[1].available, true);
  assert.equal(report.products[5].slug, 'balance-niacinamide-blemish-recovery-serum-30ml');
  ...
  assert.equal(report.products[5].offers[0].priceNgn, 8_400);
  assert.equal(report.products[5].offers[0].available, true);
  assert.equal(report.products[5].offers[1].retailer, 'CSi Grocery');
  assert.equal(report.products[5].offers[1].priceNgn, 10_700);
  assert.equal(report.products[5].offers[1].available, true);
  assert.equal(report.products[6].slug, 'nineless-a-control-10-azelaic-acid-serum-30ml');
  ...
  assert.equal(report.products[6].offers[1].priceNgn, 14_500);
  assert.equal(report.products[7].slug, 'nineless-mela-pro-rice-txa-toner-200ml');
  assert.equal(report.products[7].category, 'Face');
  assert.equal(report.products[7].step, 'Tone');
  assert.equal(report.products[7].offers[0].retailer, 'BuyBetter');
  assert.equal(report.products[7].offers[0].priceNgn, 15_500);
  assert.equal(report.products[7].offers[0].available, true);
  assert.equal(report.products[7].offers[1].retailer, 'Muna Cosmetics');
  assert.equal(report.products[7].offers[1].priceNgn, 19_000);
  assert.equal(report.products[7].offers[1].available, true);
  assert.equal(report.products[123].slug, 'facefacts-ceramide-oil-control-foaming-cleanser-400ml');
  assert.equal(report.products[123].category, 'Face');
  assert.equal(report.products[123].step, 'Cleanse');
  assert.equal(report.products[123].offers[0].retailer, 'CSi Grocery');
  assert.equal(report.products[123].offers[0].priceNgn, 7_500);
  assert.equal(report.products[123].offers[0].available, true);
  assert.equal(report.products[123].offers[1].retailer, '24Eleven');
  assert.equal(report.products[123].offers[1].priceNgn, 7_300);
  assert.equal(report.products[123].offers[1].available, true);
  assert.equal(report.products[112].slug, 'facefacts-ceramide-hydrating-gentle-cleanser-400ml');
  ...
  assert.equal(report.products[112].offers[0].priceNgn, 6_950);
  assert.equal(report.products[112].offers[1].retailer, 'Teeka4');
  assert.equal(report.products[112].offers[1].priceNgn, 7_200);
  assert.equal(report.products[112].offers[2].retailer, '24Eleven');
  assert.equal(report.products[112].offers[2].priceNgn, 7_300);
  assert.equal(report.products[124].slug, 'facefacts-ceramide-foaming-cleanser-400ml');
  ...
  assert.equal(report.products[124].offers[0].priceNgn, 6_450);
  assert.equal(report.products[124].offers[0].available, true);
  assert.equal(report.products[124].offers[1].retailer, '24Eleven');
  assert.equal(report.products[124].offers[1].priceNgn, 6_800);
  assert.equal(report.products[124].offers[1].available, true);
  assert.equal(report.products[8].slug, 'de-la-cruz-acne-treatment-10-sulfur-73-7g');
  assert.equal(report.products[8].category, 'Face');
  assert.equal(report.products[8].step, 'Treat');
  assert.equal(report.products[8].offers[0].retailer, 'Beauty by Daz');
  assert.equal(report.products[8].offers[0].priceNgn, 12_500);
  assert.equal(report.products[8].offers[0].available, true);
  assert.equal(report.products[8].offers[1].retailer, 'BuyBetter');
  assert.equal(report.products[8].offers[1].priceNgn, 17_738);
  assert.equal(report.products[8].offers[1].available, false);
  assert.equal(report.products[9].slug, 'olay-super-serum-body-wash-normal-skin-547ml');
  ...
  assert.equal(report.products[9].offers[0].priceNgn, 20_000);
  assert.equal(report.products[9].offers[0].available, true);
  assert.equal(report.products[9].offers[1].retailer, 'Perona Beauty');
  assert.equal(report.products[9].offers[1].priceNgn, 21_200);
  assert.equal(report.products[9].offers[1].available, true);
  assert.equal(report.products[10].slug, 'dove-calming-moisture-body-wash-547ml');
  assert.equal(report.products[10].category, 'Body');
  assert.equal(report.products[10].step, 'Cleanse');
  assert.equal(report.products[10].offers[0].retailer, 'BuyBetter');
  assert.equal(report.products[10].offers[0].priceNgn, 22_600);
  assert.equal(report.products[10].offers[0].available, true);
  assert.equal(report.products[10].offers[1].retailer, 'Teeka4');
  assert.equal(report.products[10].offers[1].priceNgn, 17_800);
  assert.equal(report.products[10].offers[1].available, false);
  assert.equal(report.products[125].slug, 'facefacts-vitamin-c-body-lotion-400ml');
  ...
  assert.equal(report.products[125].offers[0].priceNgn, 6_095);
  assert.equal(report.products[125].offers[0].available, true);
  assert.equal(report.products[125].offers[1].retailer, 'Allure Beauty');
  assert.equal(report.products[125].offers[1].priceNgn, 8_000);
