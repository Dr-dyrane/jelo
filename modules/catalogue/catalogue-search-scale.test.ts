...
function record(
  overrides: Partial<CatalogueSearchRecord> = {},
...
test("server search ranking reaches products beyond the former 24-item client ceiling", () => {
  ...
  assert.ok(
    lateProduct,
    "no product beyond index 24 ranks first for its own name+size query",
  );
...
test("catalogue search matches name, brand, size and exact barcode with deterministic priority", () => {
  const records = [
    record({
      name: "Barcode prefix cleanser",
      href: "/products/prefix",
      barcode: "850068103000",
    }),
    record({
      brand: "KeraCare",
      name: "Dry & Itchy Conditioner",
      size: "32 oz",
      href: "/products/keracare",
      barcode: "850068103058",
    }),
    record({
      brand: "KeraCare",
  ...
  assert.equal(
    rankCatalogueSearchRecords(records, "dry itchy")[0]?.href,
    "/products/keracare",
  );
  assert.deepEqual(
  ...
  assert.equal(
    rankCatalogueSearchRecords(records, "32 oz")[0]?.href,
    "/products/keracare",
  );
  assert.equal(
    rankCatalogueSearchRecords(records, "850068103058")[0]?.href,
    "/products/keracare",
  );
  assert.deepEqual(
    rankCatalogueSearchRecords(records, "850068103", 1).map(
      (item) => item.href,
    ),
    ["/products/prefix"],
  );
...
test("suggestion requests are bounded, normalized and market aware", () => {
  const parsed = parseCatalogueSearchRequest(
    "https://www.jelocare.com/api/products/suggestions?q=%20%20CeraVe%20%20cleanser%20&market=US",
  );
  ...
  assert.equal(
    parseCatalogueSearchRequest(
      "https://www.jelocare.com/api/products/suggestions?q=a",
    ).searchable,
    catalogueSuggestionMinimumQueryLength <= 1,
  );
  assert.equal(
    parseCatalogueSearchRequest(
      `https://www.jelocare.com/api/products/suggestions?q=${"x".repeat(200)}`,
    ).query.length,
    120,
  );
});
...
test("products page ships compact start suggestions and fetches product matches on demand", async () => {
  ...
  assert.match(client, /fetch\([\s\S]*?`\/api\/products\/suggestions\?/);
