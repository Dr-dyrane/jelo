...
test("0033 backfills one deterministic immutable version for each reviewed public product row", async () => {
  const migration = await source(
    "db/migrations/0033_catalogue_product_identity_versions.sql",
  );
  ...
  assert.match(
    migration,
    /catalogue-product-identity-version:v1:' \|\| product\.id::text/,
  );
  assert.match(
    migration,
    /catalogue-product-identity:v1:' \|\| product\.id::text/,
  );
  assert.match(migration, /digest\([\s\S]*'sha256'/);
  assert.match(migration, /where product\.is_published = true/);
  assert.match(
    migration,
    /product\.source_version in \('static-v1', 'published-intake-v1'\)/,
  );
...
test("0033 preserves tombstones and rejects silent size, lifecycle, or transition rewrites", async () => {
  ...
  assert.match(
    migration,
    /A material size change requires a new catalogue identity version/,
  );
  assert.match(
    migration,
    /old\.is_published = true and new\.is_published = false/,
  );
  assert.match(
    migration,
  ...
  assert.match(
    migration,
    /Catalogue identity transition cycles are prohibited/,
  );
...
test("0033 and the seed admit only the reviewed public projection and reconcile repeat runs", async () => {
  ...
  assert.match(
    migration,
    /Catalogue identity versions require a public catalogue product/,
  );
  assert.match(migration, /catalogue_identity_public_authority_check/);
  assert.match(seed, /catalogueIdentityIdForProductId\(\s*savedProduct\.id/);
  assert.match(
    seed,
    /catalogueIdentityVersionIdForProductId\([\s\S]*savedProduct\.id/,
  );
...
test("the complete current reviewed catalogue remains a one-to-one projection", () => {
  ...
  assert.equal(
    new Set(
      publicCatalogueSearchArtifact.products.map((product) => product.slug),
    ).size,
