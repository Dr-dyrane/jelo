...
export type PersistedOfferEvidence = {
  ...
  inventoryStatus?:
    "in_stock" | "low_stock" | "out_of_stock" | "unknown" | null;
...
export function materializeOfferEvidence(
  ...
  return {
    ...offer,
    retailerEvidence:
      offer.retailerEvidence ?? retailerEvidenceFor(offer.retailer),
  };
}
...
export function materializePersistedOfferEvidence(
  ...
  const basis =
    persisted.verificationMethod === "retailer_page" ||
  ...
  const listingEvidence =
    basis && offer.match !== "search" && observedAt
      ? {
          observedAt,
          sourceUrl,
          basis,
      ...
      : undefined;
  const hasPrice =
    (typeof offer.priceNgn === "number" && offer.priceNgn > 0) ||
    (typeof offer.priceUsd === "number" && offer.priceUsd > 0);
  const stock = persisted.inventoryStatus?.replaceAll("_", "-") as
    NonNullable<Offer["priceObservation"]>["stock"] | undefined;
  ...
  return {
    ...
    listingEvidence,
    priceObservation:
      listingEvidence && hasPrice && observedTitle && observedSize
        ? {
...
export function hasSellerIdentityEvidence(offer: Offer) {
  return Boolean(
    ...
    ].includes(offer.sellerIdentityEvidence.basis) &&
...
export function hasBrandAuthorizationEvidence(
  offer: Offer,
  ...
  return Boolean(
    ...
    evidence.brand.trim() &&
    (!expectedBrand ||
      evidence.brand.localeCompare(expectedBrand, undefined, {
        sensitivity: "base",
      }) === 0) &&
...
/**
 * Same as `observedMarketPrice` but does NOT require the offer to be fresh.
 *
 * Trend computation uses historical price points — a stale offer's price is
 * still a valid data point for charting market movements over time. The
 * freshness gate is a shopper-facing concern (is this price still actionable
 * today?), not a trend-data concern.
 *
 * Availability is still required: an out-of-stock offer has no observable
 * price for the "current" snapshot.
 */
export function observedMarketPriceForTrends(offer: Offer, market: Market) {
  if (
    !offer.available ||
    !hasListingEvidence(offer) ||
    !hasCompletePriceObservation(offer)
...
/**
 * Same as `comparableMarketPrice` but does NOT require freshness.
 * Used for trend computation where historical price points matter
 * regardless of whether the offer is still actionable today.
 */
export function comparableMarketPriceForTrends(offer: Offer, market: Market) {
  if (offer.priceComparison === "exclude") return null;
  return observedMarketPriceForTrends(offer, market);
}

export function landedCostCaveat(offer: Offer) {
  if (offer.priceObservation?.landedCost === "included")
...
export function observedStockLabel(offer: Offer, fresh: boolean) {
  ...
  if (offer.inventoryQuantity && offer.inventoryQuantity > 0)
    return `${offer.inventoryQuantity} left`;
  return "In stock";
}
