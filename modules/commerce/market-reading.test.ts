...
function makeOffer(overrides: Partial<Offer> & { retailer: string }): Offer {
  return {
    ...
    listingEvidence: {
      ...
      sourceUrl: `https://example.com/${overrides.retailer.toLowerCase()}`,
      basis: "retailer-page",
    },
    priceObservation: {
      observedAt: overrides.checkedAt ?? "2026-07-21",
...
test("stale offers produce unavailable state when freshness is required", () => {
  const reading = buildMarketReading(
    [
      makeOffer({
        retailer: "Store A",
...
test("stale offers show observed price when freshness is not required", () => {
  const reading = buildMarketReading(
    [
      makeOffer({
        retailer: "Store A",
  ...
  assert.equal(reading.state, "priced");
  if (reading.state !== "priced") return;
  assert.equal(reading.lowestPrice, 9_850);
  assert.equal(reading.storeCount, 1);
  // Freshness label should reflect the actual observation date, not hide it
  assert.equal(reading.freshnessLabel, "Checked 1 Jul");
});
...
test("deterministic yesterday label", () => {
  assert.equal(
    freshnessLabelFor("2026-07-21T08:00:00Z", NOW),
    "Checked yesterday",
  );
});
...
test("deterministic days-ago label", () => {
  assert.equal(
    freshnessLabelFor("2026-07-19T08:00:00Z", NOW),
    "Checked 3 days ago",
  );
});
...
test("deterministic date label beyond 7 days", () => {
  assert.equal(
    freshnessLabelFor("2026-07-10T08:00:00Z", NOW),
    "Checked 10 Jul",
  );
});
