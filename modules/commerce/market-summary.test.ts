...
test("removes stale observations from every current market claim when freshness is required", () => {
...
test("includes stale observations when freshness is not required", () => {
  const summary = summarizeMarket(
    offers,
    "NG",
    new Date("2026-07-30T12:00:00Z"),
  ...
  // Stale offers should still show prices — they're evidence-backed, just not fresh
  assert.equal(summary.retailerCount, 3);
  assert.equal(summary.pricedRetailerCount, 2);
  assert.equal(summary.lowestPrice, 14_500);
  assert.equal(summary.priceBasis, "multi-source");
