...
test("checkout keeps a valid retailer choice and otherwise uses the ranked default", () => {
  const product = productBySlug("aqua-rich-licorice-mulberry-body-wash-1000ml");
  assert.ok(product);
  const options = findRetailerBasketOptions(
    [product],
  ...
  assert.equal(
    chooseRetailerBasketOption(options, options[1].retailer)?.retailer,
    options[1].retailer,
  );
  assert.equal(
    chooseRetailerBasketOption(options, "missing-store"),
    options[0],
  );
});
