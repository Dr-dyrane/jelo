...
test("selectShareGaps keeps a real two-store spread with the right figures", () => {
  const gaps = selectShareGaps(
    [product("wide", [ngOffer("one", 14_500), ngOffer("two", 17_500)])],
    now,
  );
...
test("selectShareGaps drops a spread below the naira floor", () => {
  const gaps = selectShareGaps(
    [product("narrow", [ngOffer("one", 14_500), ngOffer("two", 14_900)])],
    now,
  );
...
test("selectShareGaps ignores a single-source product (no spread to claim)", () => {
  const gaps = selectShareGaps(
    [product("single", [ngOffer("one", 14_500)])],
    now,
  );
  assert.equal(gaps.length, 0);
});
...
test("selectRecentDrops keeps a notable fall and reports it as positive naira", () => {
  const drops = selectRecentDrops(
    [
      {
        product: product("p", [ngOffer("one", 14_500), ngOffer("two", 17_500)]),
        trends: movement({ amountMinor: -1_200, percent: -8 }),
      },
    ],
    now,
  );
...
test("selectRecentDrops ignores rises, flat moves and sub-threshold falls", () => {
  const rise = {
    product: product("rise", [ngOffer("one", 14_500)]),
    trends: movement({ direction: "up", percent: 6, amountMinor: 1_000 }),
  };
  const flat = {
    product: product("flat", [ngOffer("one", 14_500)]),
    trends: movement({ direction: "flat", percent: 0.2, amountMinor: 30 }),
  };
  const tiny = {
    product: product("tiny", [ngOffer("one", 14_500)]),
    trends: movement({ direction: "down", percent: -2, amountMinor: -300 }),
  };
...
test("selectRecentDrops falls through weak 30-day evidence to a valid seven-day drop", () => {
  ...
  const drops = selectRecentDrops(
    [
      {
        product: product("valid-seven-day", [
          ngOffer("one", 14_500),
          ngOffer("two", 17_500),
        ]),
...
test("selectRecentDrops ignores a product with no shareable offer", () => {
  const searchOnly = product("search", [
    observed({
      retailer: "s",
      url: "https://x",
      trust: 100,
  ...
  assert.equal(
    selectRecentDrops([{ product: searchOnly, trends: movement({}) }], now)
      .length,
    0,
  );
});
...
test("selectRecentDrops ranks by percentage movement before absolute naira change", () => {
  const drops = selectRecentDrops(
    [
      ...
      {
        product: product("stronger-movement", [
          ngOffer("three", 10_000),
          ngOffer("four", 12_000),
        ]),
  ...
  assert.deepEqual(
    drops.map((drop) => drop.slug),
    ["stronger-movement", "larger-naira"],
  );
});
...
test("selectRecentDrops uses wider retailer evidence to break equal trend ties", () => {
  const drops = selectRecentDrops(
    [
      {
        product: product("narrow-evidence", [
          ngOffer("one", 10_000),
          ngOffer("two", 12_000),
        ]),
...
test("the canonical read model preserves strict lanes and dedupes a product that qualifies for both", () => {
  ...
  assert.deepEqual(
    model.recentDrops.map((signal) => signal.slug),
    ["drop-and-gap"],
  );
  assert.deepEqual(
    model.priceGaps.map((signal) => signal.slug),
    ["gap-only"],
  );
  assert.deepEqual(
  ...
  assert.equal(
    new Set(model.rankedPool.map((signal) => signal.slug)).size,
    model.rankedPool.length,
  );
});
...
test("deduping a drop does not consume one of the eight strict gap slots", () => {
  const items = Array.from({ length: 10 }, (_, index) => {
    ...
    const item = product(`gap-${index}`, [
      ngOffer(`low-${index}`, 10_000),
      ngOffer(`high-${index}`, 10_000 + spread),
    ]);
    return {
      product: item,
      trends:
        index === 0 ? movement({ percent: -10, amountMinor: -1_500 }) : {},
    };
  });
...
test("fallback ranking uses freshness, retailer breadth, then actionable price context", () => {
  const newest = product("newest", [
    ngOffer("new", 12_000, "2026-07-22T10:00:00Z"),
  ]);
  ...
  const twoStores = product("two-stores", [
    ngOffer("two-a", 11_000),
    ngOffer("two-b", 11_000),
  ]);
  const oneStore = product("one-store", [ngOffer("one-a", 8_000)]);
  const stale = product("stale", [ngOffer("stale-a", 7_000, "2026-06-01")]);
  const search = product("search", [
    { ...ngOffer("search-a", 6_000), match: "search" },
  ]);
  const outsideNg = product("outside-ng", [
    { ...ngOffer("us-a", 5_000), location: ["US"] },
  ]);
  const model = buildShareSignalReadModel(
    [newest, wide, twoStores, oneStore, stale, search, outsideNg].map(
      (item) => ({ product: item, trends: {} }),
    ),
    now,
  );
...
test("aggregate interest is optional, neutral when absent, and only breaks an evidence tie", () => {
  ...
  const withInterest = buildShareSignalReadModel(
    items,
    now,
    new Map([["beta", 4]]),
  );
  ...
  assert.deepEqual(
    neutral.rankedPool.map((signal) => signal.slug),
    ["alpha", "beta"],
  );
  assert.deepEqual(
    empty.rankedPool.map((signal) => signal.slug),
    ["alpha", "beta"],
  );
  assert.deepEqual(
    withInterest.rankedPool.map((signal) => signal.slug),
    ["beta", "alpha"],
  );
  ...
  const newer = product("newer", [
    ngOffer("newer-store", 10_000, "2026-07-22T10:00:00Z"),
  ]);
  ...
  assert.deepEqual(
    evidenceBeforeInterest.rankedPool.map((signal) => signal.slug),
    ["newer", "older"],
  );
});
...
test("detail recommendations share the global pool and category only breaks an evidence tie", () => {
  ...
  const duplicatedPool = [
    ...model.rankedPool,
    model.rankedPool.find((signal) => signal.slug === "tied-body")!,
  ];
  ...
  assert.equal(
    new Set(recommendations.map((signal) => signal.slug)).size,
...
test("the server read has no live or private interest dependency and both routes consume it", async () => {
  ...
  assert.match(
    serverRead,
    /No live aggregate-interest provider is wired today/,
  );
...
test("share index and detail cards keep deterministic mobile reflow contracts", async () => {
  ...
  const [indexRoute, indexCss, detailCard, detailCss] = await Promise.all([
    readFile(path.join(root, "app/(site)/share/page.tsx"), "utf8"),
    readFile(
      path.join(root, "app/(site)/share/share-index.module.css"),
      "utf8",
    ),
    readFile(path.join(root, "app/(site)/share/[slug]/share-card.tsx"), "utf8"),
    readFile(
      path.join(root, "app/(site)/share/[slug]/share-card.module.css"),
      "utf8",
    ),
  ]);
  ...
  assert.match(
    indexRoute,
    /className=\{styles\.grid\}[\s\S]*priceDrops\.map[\s\S]*className=\{styles\.card\}/,
  );
  assert.match(
    compactIndexCss,
    /\.card\{[^}]*grid-template-columns:5\.5remminmax\(0,1fr\)auto/,
  );
  assert.match(compactIndexCss, /@media\(max-width:720px\)\{\.grid,\.topics/);
  assert.match(
    compactIndexCss,
    /@media\(max-width:720px\)[\s\S]*\.card\{grid-template-columns:4\.75remminmax\(0,1fr\)auto/,
  );
  assert.match(
    detailCard,
    /<ul[\s\S]*?className=\{styles\.alternativeList\}[\s\S]*?data-count=\{Math\.min\(items\.length, 3\)\}/,
  );
  assert.match(
    compactDetailCss,
    /\.alternativeList\{[^}]*repeat\(3,minmax\(0,1fr\)\)/,
  );
  assert.match(
    compactDetailCss,
    /@media\(max-width:760px\)[\s\S]*\.alternativeList\[data-count\]\{grid-template-columns:1fr;max-width:none/,
  );
  assert.match(
    compactDetailCss,
    /@media\(max-width:430px\)[\s\S]*grid-template-columns:4\.75remminmax\(0,1fr\)auto/,
  );
});
