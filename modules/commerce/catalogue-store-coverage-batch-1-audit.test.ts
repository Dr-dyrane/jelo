...
test('batch 1 deterministically records the retailer grouping and accepted score reasons', () => {
  ...
  assert.deepEqual(audit.selection.rankedReasons.map(product => product.slug), selectedSlugs);
  ...
  for (const product of audit.selection.rankedReasons) {
    ...
    assert.deepEqual(
      ...
      { exactStoreGap: 3, freshPriceGap: 3, staleOrUnverifiedOffers: 2, blockedExactOffers: 1 },
      product.slug,
    );
  }
...
test('only evidence-qualified Batch 1 observations are admitted', () => {
  ...
  for (const product of audit.products) {
    assert.equal(product.retailer, audit.selection.retailerGrouping, product.slug);
    assert.equal(product.retailerDirectoryStatus, 'directory-listed', product.slug);
    assert.ok(registered.has(product.retailer), product.slug);
    assert.equal(new URL(product.requestedUrl).protocol, 'https:', product.slug);
    assert.equal(new URL(product.finalUrl).protocol, 'https:', product.slug);

    const offer = verifiedRetailOffers[product.slug]?.find(candidate => candidate.retailer === product.retailer);
    if (product.disposition === 'verified-exact-offer') {
      ...
      assert.equal(offer.priceObservation?.stock, product.stock, product.slug);
