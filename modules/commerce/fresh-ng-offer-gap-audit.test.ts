...
test('the fresh Nigerian offer-gap audit covers the four unresolved products', () => {
  ...
  for (const product of audit.products) {
    ...
    assert.equal(product.disposition, 'withheld', product.slug);
    assert.ok(product.reasons.length > 0, product.slug);
  }
});

test('directory status is explicit and agrees with the retailer registry', () => {
  const registeredRetailers = new Set(nigeriaRetailers.map(retailer => retailer.name));
...
test('identity or landed-cost ambiguity cannot leak into verified offers', () => {
  for (const product of audit.products) {
    assert.equal(verifiedRetailOffers[product.slug], undefined, `${product.slug} must remain withheld`);
  }
});
