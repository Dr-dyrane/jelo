...
const observations = products.flatMap(product => product.offers.map(offer => ({ product, offer })));
const asOf = new Date(audit.reviewedAt);
const visualRevisionManifest = verifyCatalogueProductVisualRevisionManifest(visualRevisionSource);
const packageEquivalenceManifest = verifyCataloguePackageRevisionEquivalenceManifest(
...
function authoritiesFor(
  ...
  const retailer = nigeriaRetailers.find(record => record.name === offer.retailer.displayName);
  return {
...
test('the pharmacist offer batch preserves every lead and its independent disposition', () => {
  ...
  for (const { product, offer } of observations) {
    ...
    assert.ok(offer.status === 'admitted' || offer.reasons.length > 0, offer.observationId);
  }
});
...
test('only the three complete fresh exact offers pass the admission contract', () => {
  ...
  for (const { product, offer } of observations) {
    ...
    if (offer.status === 'admitted') assert.deepEqual(blockers, [], offer.observationId);
    else assert.ok(blockers.length > 0, `${offer.observationId} must fail closed`);
  }
});

test('admission is bound to the exact published release identity, fingerprint and route', () => {
  const original = products.find(item => item.candidateId === 'naturium-the-perfector-salicylic-acid-body-wash-500ml');
  const offer = original?.offers.find(item => item.retailer.displayName === 'Rhema Beauty Shop');
  assert.ok(original && offer);

  for (const mutate of [
    (product: BatchProduct) => { product.canonicalIdentity.value = '850010792935'; },
    (product: BatchProduct) => { product.releaseFingerprint = 'a'.repeat(64); },
    (product: BatchProduct) => { product.publicRoute = '/products/lookalike'; },
  ]) {
    const product = structuredClone(original);
    mutate(product);
    assert.ok(
      catalogueOfferAdmissionBlockers(product, offer, asOf, authoritiesFor(product, offer))
...
test('exact identity fields reject suffix variants and incomplete store text', () => {
  ...
  suffixVariant.observedVariant = 'The Perfector Salicylic Acid Body Wash Counterfeit Suffix';
  assert.ok(
    catalogueOfferAdmissionBlockers(product, suffixVariant, asOf, authoritiesFor(product, suffixVariant))
  ...
  assert.ok(
    catalogueOfferAdmissionBlockers(product, incompleteText, asOf, authoritiesFor(product, incompleteText))
  ...
  for (const storeIdentityText of [
    ...
    assert.ok(
      catalogueOfferAdmissionBlockers(product, widened, asOf, authoritiesFor(product, widened))
        .includes('store-identity-text-incomplete'),
    );
  }
});
...
test('the admitted evidence projects exactly once and rejected or pending stores cannot leak', () => {
  ...
  for (const { product, offer } of observations) {
    const projected = verifiedRetailOffers[product.candidateId]?.filter(candidate => (
    ...
    assert.equal(projected[0]?.priceObservation?.variant, offer.observedVariant);
...
test('retailer hosts fail closed against subdomain, suffix and credential spoofing', () => {
  ...
  for (const finalUrl of [
    ...
    assert.ok(
      catalogueOfferAdmissionBlockers(product, spoofed, asOf, authoritiesFor(product, spoofed))
...
test('structured price and stock must reconcile to the retained retailer text', () => {
  ...
  assert.ok(
    catalogueOfferAdmissionBlockers(product, wrongPrice, asOf, authoritiesFor(product, wrongPrice))
  ...
  assert.ok(
    catalogueOfferAdmissionBlockers(product, wrongStock, asOf, authoritiesFor(product, wrongStock))
  ...
  for (const sourceText of ['Not in stock', 'Only 0 left in stock']) {
    ...
    assert.ok(
      catalogueOfferAdmissionBlockers(product, negativeStock, asOf, authoritiesFor(product, negativeStock))
        .includes('offer-unavailable'),
    );
  }
});
...
test('historical packaging uses only the verified official exact-size authority', () => {
  ...
  crossSize.storeText = "L'Occitane en Provence Almond (Amande) Shower Oil — Almond (Amande) Softening Shower Oil — 250 ml";
...
test('the Jumia lead is textually 250 ml and no 500 ml authority can bind it', () => {
  const product = products.find(item => item.candidateId === 'loccitane-almond-softening-shower-oil-250ml');
