...
test("retailer partnerships stay private until a deliberate catalogue review", async () => {
  ...
  const migration = await readFile(
    path.join(root, "db/migrations/0016_retailer_partnership_intake.sql"),
    "utf8",
  );
  const repository = await readFile(
    path.join(root, "lib/retailer-partnership/repository.ts"),
    "utf8",
  );
  const page = await readFile(
    path.join(root, "app/(site)/retailers/page.tsx"),
    "utf8",
  );
  const experience = await readFile(
    path.join(root, "components/retailers/retailer-partnership-experience.tsx"),
    "utf8",
  );
  ...
  const magicRoute = await readFile(
    path.join(root, "app/api/retailers/magic/route.ts"),
    "utf8",
  );
  ...
  assert.doesNotMatch(
    repository,
    /insert into (products|retailers|offers|retail_offers)/i,
  );
