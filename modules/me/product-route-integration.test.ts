...
test("readMeProduct does not import or call findCatalogueProduct", () => {
  ...
  assert.doesNotMatch(
    source,
    /import\s+\{[^}]*findCatalogueProduct/,
    "route-read-models must not import findCatalogueProduct",
  );
  ...
  const fnBody = source.slice(
    ...
    source.indexOf(
      ...
      source.indexOf("export async function readMeProduct") + 1,
    ),
  );
  assert.doesNotMatch(
    fnBody,
    /findCatalogueProduct\(/,
    "readMeProduct must not call findCatalogueProduct",
  );
  assert.match(
    source,
    /readMeProduct\(identity: CustomerAccessIdentity, product: Product/,
    "readMeProduct must accept a Product",
  );
});
...
test("route page passes the resolved product to readMeProduct", () => {
  ...
  assert.match(
    page,
    /readMeProduct\(customer, selectedProduct/,
    "route must pass the resolved product, not a slug",
  );
});
...
function makeOffer(
  overrides: Partial<Offer> & { retailer?: string } = {},
): Offer {
...
test("market snapshot and inline reading agree on listing-only state", () => {
  ...
  if (
    snapshot.NG.reading.state === "listing-only" &&
    inline.state === "listing-only"
  ) {
...
test("duplicate retailer offers do not inflate store count in snapshot or inline", () => {
  ...
  if (snapshot.NG.reading.state === "priced" && inline.state === "priced") {
    assert.equal(snapshot.NG.reading.storeCount, inline.storeCount);
    assert.equal(
      snapshot.NG.reading.storeCount,
      1,
      "inline must deduplicate to 1 store",
    );
  }
  assert.equal(
    snapshot.NG.extras.uniquePricedStoreCount,
    1,
    "panel must deduplicate to 1 store",
  );
  assert.equal(
    snapshot.NG.extras.uniqueListingStoreCount,
    1,
    "panel listing count must deduplicate to 1 store",
  );
});
...
test("retailer names differing by case and whitespace deduplicate", () => {
  ...
  if (snapshot.NG.reading.state === "priced" && inline.state === "priced") {
    assert.equal(
      inline.storeCount,
      1,
      "inline must deduplicate case/whitespace variants",
    );
  }
  assert.equal(
    snapshot.NG.extras.uniquePricedStoreCount,
    1,
    "panel must deduplicate case/whitespace variants",
  );
});
...
function makeShelfItem(
  overrides: Partial<CustomerPortalShelfItem> = {},
...
test("shelf context prefers saved-current over saved-changed when both match", () => {
  ...
  if (ctx.state === "saved-current") {
    assert.equal(
      ctx.shelfItem.identityVersionId,
...
test("RetailerList accepts a marketSnapshot prop and does not call summarizeMarket", () => {
  ...
  assert.match(
    ...
    "RetailerList must accept marketSnapshot",
  );
  assert.doesNotMatch(
    source,
    /summarizeMarket\(/,
    "RetailerList must not call summarizeMarket",
  );
  assert.doesNotMatch(
    source,
    /from.*market-summary/,
    "RetailerList must not import from market-summary",
  );
});
...
test("RetailerList does not parse priceLabel back into a number", () => {
  ...
  assert.doesNotMatch(
    ...
    "RetailerList must not parse priceLabel",
  );
  assert.doesNotMatch(
    source,
    /priceLabel\.replace.*\d/,
    "RetailerList must not strip characters from priceLabel",
  );
  assert.match(
    source,
    /extras\?\.lowestPrice/,
    "RetailerList must use numeric lowestPrice from extras",
  );
});
...
test("public product hero consumes the panel snapshot and never animates market facts from zero", () => {
  const page = readFileSync("app/(site)/products/[slug]/page.tsx", "utf8");
  const hero = readFileSync(
    "components/products/product-hero-motion.tsx",
    "utf8",
  );
  assert.match(page, /panelData\.marketSnapshot\?\.NG\.reading/);
  assert.doesNotMatch(page, /buildMarketReading\(product\.offers/);
  assert.doesNotMatch(hero, /<Counter|components\/motion\/counter/);
  assert.match(hero, /\{priceLabel\} · \{storeCount\}/);
});
...
test("readMeProduct uses narrow reads, not full list", () => {
  ...
  assert.doesNotMatch(
    productFn,
    /customerShelfService\.read\(/,
    "readMeProduct must not call customerShelfService.read",
  );
  assert.doesNotMatch(
    productFn,
    /customerRoutineService\.read\(/,
    "readMeProduct must not call customerRoutineService.read",
  );
});
