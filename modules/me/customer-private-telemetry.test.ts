...
test('private telemetry accepts only its four enum fields', () => {
  ...
  assert.equal(
    parseCustomerPrivateTelemetryEvent({ ...validEvent, operation: 'add:item-42' }),
    null,
  );
  assert.equal(
  ...
  assert.equal(
    parseCustomerPrivateTelemetryEvent({ ...validEvent, latencyBucket: '412ms' }),
    null,
  );
  assert.equal(
    parseCustomerPrivateTelemetryEvent({
...
test('recording writes one hourly counter with a fixed 35-day bucket expiry', async () => {
  ...
  assert.equal(await recordCustomerPrivateTelemetry(validEvent, dependencies), true);
  assert.equal(await recordCustomerPrivateTelemetry(validEvent, dependencies), true);
...
test('failed operations schedule failure and rethrow the original signal', async () => {
  ...
  const unchanged = await measureCustomerPrivateOperation(
    ...
    { schedule: () => { throw new Error('after unavailable'); } },
  );
...
test('all current authenticated Me reads and mutations use the private aggregate helper', () => {
  ...
  for (const [surface, operation] of [
    ...
    assert.match(
      actions,
      new RegExp(
        `measureMeMutation\\(\\s*"${surface}",\\s*"${operation}"`,
      ),
      `${surface}:${operation}`,
    );
  }
  ...
  assert.equal(
    notificationActions.match(/operation: 'update'/g)?.length,
    2,
  );
...
test('telemetry remains server-only, aggregate-only, and operator-readable only by command', () => {
  ...
  assert.doesNotMatch(telemetry, /cookies\(|headers\(|console\.|userAgent|referrer/i);
  assert.match(script, /environment: CustomerPrivateTelemetryEnvironment = 'production'/);
