...
describe("private missing-product presentation model", () => {
  ...
  it("accepts only an exact canonical identity alias before opening the request path", () => {
    ...
    assert.equal(
      findExactCanonicalIdentity(products, "anua niacinamide serum"),
      undefined,
    );
    assert.deepEqual(
      searchCanonicalIdentities(products, "anua 30 ml"),
      products,
    );
  });
  ...
  it("offers every partial canonical match as a Shelf add before a missing-product request", () => {
    ...
    assert.deepEqual(
      searchCanonicalIdentities(products, "not in catalogue"),
      [],
    );

    const fields = source(
      "components/me/product-requests/product-request-fields.tsx",
    );
    ...
    assert.match(
      fields,
      /matches\.map\([\s\S]*<ShelfActionButton[\s\S]*productSlug=\{product\.slug\}/,
    );
  ...
  it("defaults photo-identification consent off and validates required identity fields", () => {
    ...
    assert.equal(
      validateProductRequestFields(empty),
      "Enter the brand printed on the pack.",
    );
    assert.equal(
      validateProductRequestFields({
        ...empty,
  ...
  it("reuses a retry key only for the same mutation fingerprint", () => {
    ...
    const changed = retryKeyFor(
      retry,
      { revision: 3, submit: true },
      createKey,
    );
...
describe("private request /me contract", () => {
  it("prepares the reviewed five plus nine preview on the development server boundary", () => {
    ...
    const shelf = source(
      "components/me/product-requests/product-request-shelf.tsx",
    );
    const detail = source(
      "components/me/product-requests/use-product-request-detail.ts",
    );
  ...
  it("keeps the legacy manifest and private fixture outside the production client graph", () => {
    ...
    assert.ok(
      !relativePaths.includes("lib/customer/legacy-product-request-fixture.ts"),
    );
    assert.ok(
      !relativePaths.includes("lib/customer/legacy-shelf-import-manifest.ts"),
    );
  ...
  it("keeps add and request detail as Shelf-owned stack routes", () => {
    ...
    assert.match(
      routes,
      /parts\[0\] === ['"]shelf['"][\s\S]*parts\[1\] === ['"]add['"]/,
    );
    assert.match(
      routes,
      /parts\[0\] === ['"]shelf['"][\s\S]*parts\[1\] === ['"]request['"]/,
    );
    assert.match(
      shell,
      /route\.kind === ['"]shelf-add['"] \|\| route\.kind === ['"]shelf-request['"]/,
    );
    assert.match(
      shell,
  ...
  it("uses one shell FAB registry and keeps the barrel component thin", () => {
    ...
    const barrel = source(
      "components/me/product-requests/product-request-experience.tsx",
    );
  ...
  it("keeps upload and consent separate and exposes photo replacement and removal", () => {
    ...
    const detail = source(
      "components/me/product-requests/product-request-detail-sections.tsx",
    );
  ...
  it("confirms destructive deletion with identity, consequences, and modal focus semantics", () => {
    const detail = source(
      "components/me/product-requests/product-request-delete-dialog.tsx",
    );
    const controller = source(
      "components/me/product-requests/use-product-request-detail.ts",
    );
    ...
    assert.match(
      detail,
      /<dialog[\s\S]*aria-labelledby="delete-product-request-title"/,
    );
    ...
    assert.match(
      detail,
      /Printed size or variant[\s\S]*request\.printedSizeVariant/,
    );
    ...
    assert.doesNotMatch(
      controller,
      /window\.confirm\('Delete this private product request/,
    );
  ...
  it("limits identity and photo edits while allowing consent-only revocation through published", () => {
    ...
    assert.deepEqual(
      states.filter((lifecycleState) =>
        canEditProductRequestIdentity({ ...base, lifecycleState }),
      ),
      ["draft", "pending", "needs_info"],
    );
    assert.deepEqual(
      states.filter((lifecycleState) =>
        canManageProductRequestPhoto({ ...base, lifecycleState }),
      ),
      ["draft", "pending", "in_review", "needs_info", "matched"],
    );
    assert.deepEqual(
      states.filter((lifecycleState) =>
        canRevokeProductRequestPhotoConsent({ ...base, lifecycleState }),
      ),
    ...
    assert.doesNotMatch(
      revocation,
      /brand|fullPackName|printedSizeVariant|sourceUrl/,
    );
  });
  ...
  it("keeps request API calls on the private owner-scoped endpoints", () => {
    ...
    assert.match(
      api,
      /\/api\/me\/product-requests\/\$\{encodeURIComponent\(id\)\}\/image/,
    );
