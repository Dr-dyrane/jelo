...
import { createCustomerShelfService } from "../../lib/customer/shelf-policy";
import type {
  CustomerShelfRecord,
...
function productionClientGraph() {
  ...
  const pending = sourceFiles.filter((path) =>
...
function record(
  overrides: Partial<CustomerShelfRecord> = {},
): CustomerShelfRecord {
  return {
...
function memoryRepository() {
  ...
  const repository: CustomerShelfRepository = {
    ...
    async remove(owner, identityVersionId) {
      calls += 1;
      return ownerRows(owner).delete(identityVersionId)
        ? "removed"
        : "already_removed";
    },
    async clear(owner) {
...
test("two owners stay isolated and add/remove retries are idempotent", async () => {
  ...
  assert.equal(
    (await service.remove(bea, versionId)).status,
    "already_removed",
  );
  assert.equal((await service.read(ada)).items.length, 1);
  assert.equal((await service.remove(ada, versionId)).status, "removed");
  assert.equal(
    (await service.remove(ada, versionId)).status,
    "already_removed",
  );
...
test("missing owners and synthetic development fail closed without repository work", async () => {
  ...
  assert.equal(
    (await service.read(identity("synthetic", "synthetic-development"))).status,
    "unavailable",
  );
  assert.equal(
    (await service.add(identity("synthetic", "synthetic-development"), slug))
      .status,
    "error",
  );
...
test("active identities resolve while retired, changed, and unmatched snapshots stay removable", () => {
  ...
  assert.equal(
    resolveCustomerPortalShelfItem(record(), catalogue).product,
    product,
  );
  const retired = resolveCustomerPortalShelfItem(
  ...
  const changed = resolveCustomerPortalShelfItem(
    record({ lifecycleState: "superseded" }),
    catalogue,
  );
  assert.equal(changed.availability, "changed");
  assert.match(changed.message ?? "", /changed/i);
  const unmatched = resolveCustomerPortalShelfItem(
    record({ currentSlug: "missing" }),
    catalogue,
  );
...
test("migration and repository enforce RLS plus explicit owner predicates", () => {
  ...
  assert.match(
    migration,
    /primary key \(owner_subject, product_identity_version_id\)/i,
  );
  assert.match(
    migration,
    /references catalogue_product_identity_versions\(identity_version_id\) on delete restrict/i,
  );
  ...
  assert.match(
    repository,
    /set_config\('app\.customer_subject', \$\{owner\}, true\)/,
  );
  assert.match(repository, /where item\.owner_subject = \$\{owner\}/);
  assert.match(
    repository,
    /where owner_subject = \$\{owner\}[\s\S]*product_identity_version_id = \$\{versionId\}/,
  );
  assert.match(
    repository,
  ...
  assert.match(
    repository,
    /pg_catalog\.set_config\('search_path', 'pg_catalog, public', true\)/g,
  );
  ...
  assert.match(
    roleMigration,
    /not runtime_role\.rolcanlogin[\s\S]*runtime_role\.rolinherit[\s\S]*runtime_role\.rolbypassrls/,
  );
  ...
  assert.match(
    roleMigration,
    /grant select \([\s\S]*identity_version_id[\s\S]*formula_version_at_review[\s\S]*\) on table public\.catalogue_product_identity_versions/,
  );
  assert.match(
    roleMigration,
    /grant select \([\s\S]*id,[\s\S]*slug,[\s\S]*is_published[\s\S]*\) on table public\.products/,
  );
  assert.doesNotMatch(
    roleMigration,
...
test("Shelf role attestation rejects every elevated or indirect authority path", () => {
  ...
  for (const key of [
    ...
    assert.equal(
      isCustomerShelfRoleAttestationSafe({ ...safe, [key]: false }),
      false,
      key,
    );
  }
  ...
  const auditSource = readFileSync(
    "scripts/audit-customer-shelf-rls.ts",
    "utf8",
  );
  const runtimeSource = readFileSync("lib/customer/shelf-database.ts", "utf8");
  const attestationQuery = (source: string) =>
    source
      .match(
...
test("named actions derive the customer and keep the workspace FAB contract unchanged", () => {
  ...
  const buttonStyles = readFileSync(
    "components/me/shelf/shelf-action-button.module.css",
    "utf8",
  );
...
test("the reviewed legacy manifest reconciles all 14 hashed source records exactly once", () => {
  assert.equal(
    LEGACY_SHELF_IMPORT_MANIFEST.source.commit,
    "04c45c87db839d516d0dc91cf93ac690445a9949",
  );
  assert.equal(
    LEGACY_SHELF_IMPORT_MANIFEST.source.products.sha256,
    "17d6d7173dc2a724eaad873afbc43b5b1b325ea87baa3e4faa922214c73b89f3",
  );
  assert.equal(
    LEGACY_SHELF_IMPORT_MANIFEST.source.routine.sha256,
  ...
  for (const binding of LEGACY_SHELF_IMPORT_MANIFEST.accepted) {
    const product = products.find(
      (candidate) => candidate.slug === binding.identityVersion.slugAtReview,
    );
    assert.deepEqual(
      product && {
  ...
  const classified = [
    ...
    ...LEGACY_SHELF_IMPORT_MANIFEST.pendingRequests.map(
      (item) => item.legacyId,
    ),
  ];
  ...
  assert.deepEqual(
    [...classified].sort(),
    [...LEGACY_SHELF_IMPORT_MANIFEST.source.products.legacyIds].sort(),
  );
...
test("the immutable legacy snapshot verifies without Git history and stays outside every client graph", () => {
  const verifierPath = join(
    repositoryRoot,
    "lib/customer/legacy-shelf-import-source.ts",
  );
  ...
  assert.doesNotMatch(
    `${verifierSource}\n${snapshotSource}`,
    /node:child_process|git fetch|git show/,
  );
...
test("the one-off importer is dry-run by default, redacted, and apply-confirmed", () => {
  ...
  assert.throws(
    () => parseLegacyShelfImportOptions(["--apply"], environment),
    /receipt hash/,
  );
  const confirmation = targetImportReceiptSha256(ownerSubject);
  assert.equal(
    parseLegacyShelfImportOptions(
      ["--apply", `--confirm-receipt-sha256=${confirmation}`],
      environment,
    ).apply,
    true,
  );
  ...
  assert.match(
    script,
    /version\.slug_at_review = \$\{item\.identityVersion\.slugAtReview\}/,
  );
  assert.match(
    script,
    /version\.brand_at_review = \$\{item\.identityVersion\.brandAtReview\}/,
  );
  assert.match(
    script,
    /version\.variant_at_review = \$\{item\.identityVersion\.variantAtReview\}/,
  );
  assert.match(
    script,
    /version\.size_at_review = \$\{item\.identityVersion\.sizeAtReview\}/,
  );
  assert.match(
    script,
    /version\.provenance = \$\{manifest\.requiredIdentity\.provenance\}/,
  );
  assert.match(
    script,
    /version\.public_eligibility_basis = \$\{manifest\.requiredIdentity\.publicEligibilityBasis\}/,
  );
  assert.match(
    script,
    /version\.lifecycle_state = \$\{manifest\.requiredIdentity\.lifecycleState\}/,
  );
  ...
  assert.match(
    script,
    /lock table public\.customer_shelf_items in share row exclusive mode/,
  );
  ...
  const shelfLock = script.indexOf(
    "lock table public.customer_shelf_items in share row exclusive mode",
  );
  const existingRead = script.indexOf(
    "select product_identity_version_id\n    from public.customer_shelf_items",
  );
  const finalSetCheck = script.indexOf(
    "hasExactSet(acceptedIdentityIds, acceptedFinal)",
  );
  const receiptWrite = script.indexOf(
...
test("the real Shelf role and owner isolation audit is explicit and rolls writes back", () => {
  ...
  assert.match(
    packageSource,
    /"customer:shelf:audit": "tsx scripts\/audit-customer-shelf-rls\.ts"/,
  );
