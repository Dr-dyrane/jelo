...
test('routine inputs normalize bounded line-based steps', () => {
  ...
  assert.equal(serializeCustomerRoutineSteps(parsed.steps), 'Cleanse | Brief, gentle cleanse.\nMoisturize | Light hydration.');
  assert.throws(() => parseCustomerRoutineInput('', 'Cleanse'));
  assert.throws(() => parseCustomerRoutineInput('Routine', ''));
  assert.throws(() => parseCustomerRoutineInput('Routine', Array.from({ length: 21 }, () => 'Step').join('\n')));
});
...
test('routine create, list, update, and delete remain owner-isolated', async () => {
  ...
  assert.equal((await service.update(ownerB, created.id, 0, input)).status, 'conflict');
  assert.equal((await service.update(ownerA, created.id, 0, {
    name: 'Evening',
    steps: [{ label: 'Cleanse', instruction: 'Remove sunscreen.' }],
  })).status, 'updated');
  assert.equal((await service.update(ownerA, created.id, 0, input)).status, 'conflict');
  assert.equal((await service.remove(ownerB, created.id)).status, 'already_removed');
...
test('synthetic and missing identities fail closed', async () => {
  ...
  assert.equal((await service.create(identity('synthetic', 'synthetic-development'), input)).status, 'error');
  assert.equal(memory.rows.size, 0);
});

test('routine migration forces owner RLS and grants only exact runtime CRUD', () => {
  const migration = readFileSync('db/migrations/0037_customer_routines.sql', 'utf8');
  for (const table of ['customer_routines', 'customer_routine_steps']) {
    ...
    assert.match(migration, new RegExp(`revoke all privileges on table public\\.${table} from public`));
    assert.match(migration, new RegExp(`revoke all privileges on table public\\.${table} from jelocare_app_runtime`));
    assert.match(migration, new RegExp(`grant select, insert, update, delete on table public\\.${table} to jelocare_shelf_runtime`));
  }
  assert.equal(migration.match(/current_setting\('app\.customer_subject', true\)/g)?.length, 4);
...
test('the pinned legacy routine manifest contains exactly three routines and eleven ordered steps', () => {
  ...
  assert.equal(LEGACY_SHELF_IMPORT_MANIFEST.routines.flatMap(routine => routine.steps).length, 11);
  assert.deepEqual(LEGACY_SHELF_IMPORT_MANIFEST.routines[0]?.steps.map(step => [
    step.position, step.label, step.instruction,
  ]), [
  ...
  assert.deepEqual(unresolved.map(step => step.label), ['Cleanse', 'Treat', 'Moisturize', 'Kuza + OGX']);
});
...
test('routine actions derive the owner and the importer commits receipt counts atomically', () => {
  ...
  for (const action of ['createRoutineAction', 'updateRoutineAction', 'deleteRoutineAction']) {
    assert.match(actions, new RegExp(`export async function ${action}`));
  }
  assert.match(actions, /requireCustomer\('\/me\/routine'\)/g);
  assert.doesNotMatch(actions, /formData\.get\(['"]owner/);
  assert.match(importer, /lock table public\.customer_routines in share row exclusive mode/);
  assert.match(importer, /lock table public\.customer_routine_steps in share row exclusive mode/);
  ...
  assert.doesNotMatch(importer, /console\.(?:log|error)\([^\n]*(?:ownerSubject|routineId|stepId)/);
});
