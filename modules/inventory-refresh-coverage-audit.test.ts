...
test("coverage freshness uses governed exact evidence and the inclusive UTC-day boundary", () => {
  ...
  assert.equal(
    offerCoverageState(
      "150 ml",
      { ...exactOffer, verificationMethod: "import" },
      now,
    ),
    "unverified",
  );
...
test("coverage fails closed on a mass-versus-volume size conflict", () => {
  assert.equal(
    offerCoverageState("150 ml", { ...exactOffer, observedSize: "150 g" }, now),
    "conflict",
  );
});
...
test("coverage classifies adapters, search blockers, and the smallest next action", () => {
  assert.equal(
    offerRefreshCapability(exactOffer),
    "automation:beauty-by-daz+manual-fallback",
  );
  assert.equal(
    offerRefreshCapability({
      ...exactOffer,
    ...
    "automation:structured-generic+manual-fallback",
  );
  assert.equal(
    offerRefreshCapability({ ...exactOffer, matchKind: "search" }),
    "blocked-search",
  );
...
test("coverage priority applies the accepted score and breaks ties by oldest observation", () => {
  const blocked = {
    ...
    latestJobError:
      "Retailer canonical URL redirected off the exact product route.",
  };
  const report = productCoverage({
  ...
  const queue = [
    {
      ...
      priority: {
        ...report.priority,
        tieBreakObservation: "2026-08-02T00:00:00.000Z",
      },
    },
    {
      slug: "never-observed",
      priority: { ...report.priority, tieBreakObservation: null },
    },
    {
      slug: "older",
      priority: {
        ...report.priority,
        tieBreakObservation: "2026-07-30T00:00:00.000Z",
      },
    },
  ...
  assert.deepEqual(
    ...
    ["never-observed", "older", "newer", "lower-score"],
  );
});
  ...
  const report = productCoverage({
    ...
    offers: [
      exactOffer,
      {
        ...exactOffer,
        retailer: "Lux Beauty",
        url: "https://luxbeautyng.com/product/example-cleanser/",
      },
      {
        ...exactOffer,
        retailer: "Care to Beauty",
        url: "https://caretobeauty.com/ng/example-cleanser/",
      },
      {
        ...exactOffer,
        url: "https://beautybydaz.com/shop/face/example-cleanser-two/",
      },
    ],
    now,
  });
...
test("tiered coverage target keeps default for non-limited products", () => {
  const report = productCoverage({
    slug: "some-other-product",
    size: "150 ml",
    databasePublished: true,
    offers: [exactOffer],
    now,
  });
  assert.equal(report.storeChoice.target, 3);
  assert.equal(report.storeChoice.gapToTarget, 2);
});
