...
test("Next skips its duplicate TypeScript pass only after production verification", async () => {
  ...
  assert.match(
    buildSource,
    /phase\.includes\('verify-release'\)[\s\S]*releaseVerificationPassed = true/,
  );
  assert.match(
    buildSource,
    /step === 'build-next'[\s\S]*releaseVerificationPassed/,
  );
  assert.match(nextConfigSource, /process\.env\.VERCEL === "1"/);
  assert.match(nextConfigSource, /process\.env\.VERCEL_ENV === "production"/);
  assert.match(
    nextConfigSource,
    /JELO_VERCEL_RELEASE_TYPECHECK_PASSED === "1"/,
  );
  assert.match(
    nextConfigSource,
...
test("staged asset checks use bounded concurrency", async () => {
  const source = await readFile(
    "scripts/promote-staged-product-assets.ts",
    "utf8",
  );
...
test("the explicit operator reconciliation is ordered and external discovery is opt-in", async () => {
  const source = await readFile(
    "scripts/reconcile-production-database.ts",
    "utf8",
  );
  assert.match(source, /requireAdminDatabaseUrl\(\)/);
  assert.match(
    source,
    /'db:migrate',[\s\S]*'db:seed',[\s\S]*'db:seed:external'[\s\S]*'assets:product:seed',[\s\S]*'assets:editorial:seed'/,
  );
  assert.match(source, /--include-external-discovery/);
  assert.match(source, /options\.includes\(INCLUDE_EXTERNAL_DISCOVERY\)/);
});

test("Market Finder release operations hard-delete only the reviewed market cache tag", async () => {
  const runbook = await readFile("docs/operations/RUNBOOKS.md", "utf8");

  assert.match(
    runbook,
    /vercel cache dangerously-delete \\\s*\n\s*--tag market-finder:market:trade-fair \\\s*\n\s*--project jelo \\\s*\n\s*--yes/,
  );
  assert.match(
    runbook,
    /no cache purge is\s+needed:[\s\S]*return before cached reads/,
  );
  assert.match(runbook, /Do\s+not use `vercel cache invalidate`/);
  assert.match(
    runbook,
    /Do\s+not use the global `market-finder` tag or `vercel cache purge`/,
  );
  assert.doesNotMatch(runbook, /CLI cannot invalidate a remote/);
});
