...
import { requireAdminDatabaseUrl } from '../../scripts/lib/admin-database';
...
test('production application configuration requires the exact runtime login', () => {
  ...
  assert.equal(applicationDatabaseUrl({ NODE_ENV: 'production', DATABASE_URL: exact }), exact);
  assert.equal(applicationDatabaseUrl({
    NODE_ENV: 'production',
  ...
  assert.equal(applicationDatabaseUrl({
    ...
    DATABASE_URL: 'postgresql://owner:secret@ep-safe.example/db',
    POSTGRES_URL: exact,
  }), undefined, 'an unsafe preferred URL cannot fall through to a second credential');
  assert.equal(applicationDatabaseUrl({
    NODE_ENV: 'production',
    POSTGRES_URL: exact,
  }), exact);
...
test('development and tests remain usable with an ordinary local PostgreSQL role', () => {
  ...
  assert.equal(applicationDatabaseUrl({ NODE_ENV: 'development', DATABASE_URL: local }), local);
  assert.equal(applicationDatabaseUrl({ NODE_ENV: 'test', POSTGRES_URL: local }), local);
  assert.equal(applicationDatabaseUrl({ NODE_ENV: 'production', DATABASE_URL: 'not-a-url' }), undefined);
});
...
test('administrative operators accept only the named direct admin credential', () => {
  ...
  assert.equal(requireAdminDatabaseUrl({ MIGRATION_DATABASE_URL: direct }), direct);
  for (const rejected of [
    {},
    { DATABASE_URL_UNPOOLED: direct },
    { POSTGRES_URL_NON_POOLING: direct },
    { MIGRATION_DATABASE_URL: 'postgresql://jelocare_app_runtime:secret@ep-safe.example/jelocare' },
    { MIGRATION_DATABASE_URL: 'postgresql://jelocare_shelf_runtime:secret@ep-safe.example/jelocare' },
    { MIGRATION_DATABASE_URL: 'postgresql://migration_admin:secret@ep-safe-pooler.example/jelocare' },
  ]) {
    assert.throws(() => requireAdminDatabaseUrl(rejected), /MIGRATION_DATABASE_URL/);
  }
});

test('application runtime audit tables remain append-only after the bulk grant', async () => {
  const migration = await readFile('db/migrations/0035_runtime_database_roles.sql', 'utf8');
...
test('every database mutation step in reconciliation uses the admin-only resolver', async () => {
  ...
  const legacyAssetImporter = await readFile('scripts/import-product-assets.mjs', 'utf8');
  ...
  const inventoryRuntime = await readFile('lib/inventory/refresh-worker.ts', 'utf8');
