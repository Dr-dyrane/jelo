...
// The production build verifies before it mutates, so a brand-new promotion's
// public URL does not exist yet at verification time. A 404 is acceptable only
// when an active staged promotion binds the same URL and content hash and the
// staged local bytes pass the identical decode, alpha, edge and surface checks.
// assets:promote:staged still verifies the uploaded bytes remotely afterwards.
async function verifyStagedFallback(
  expectation: Parameters<typeof verifyRemoteCataloguePublicationImage>[0],
) {
  const promotion = (promotions as PromotionRecord[]).find(record => (
    record.active === true
  ...
  if (!promotion?.localPath || promotion.contentHash !== expectation.sha256) {
    throw new Error(`${expectation.candidateId}: final image is missing and no matching staged promotion binds it.`);
  }
  ...
  if (localSha !== expectation.sha256) {
    throw new Error(`${expectation.candidateId}: staged bytes do not match the promoted content hash.`);
  }
  ...
  return { ...result, transport: 'staged-local-bytes-pending-promotion' as const };
}

async function verifyOne(expectation: Parameters<typeof verifyRemoteCataloguePublicationImage>[0]) {
  try {
    return await verifyRemoteCataloguePublicationImage(expectation);
  } catch (error) {
    if (error instanceof Error && error.message.endsWith('final image returned 404, not 200.')) {
...
async function main() {
  ...
  for (let offset = 0; offset < structural.dossiers.length; offset += concurrency) {
    ...
    verified.push(...await Promise.all(batch.map(dossier => verifyOne({
  ...
  const staged = verified.filter(result => (result as { transport?: string }).transport === 'staged-local-bytes-pending-promotion');
  ...
  if (staged.length) {
    console.log(`${staged.length} verified from staged local bytes awaiting promotion in this deployment.`);
  }
