...
import { assistedOrderCookieName, hashOrderSecret } from '@/lib/commerce/assisted-procurement-security';
...
export default async function OrderPage({
  ...
  const order = secret ? await readAssistedOrderBySession(hashOrderSecret(secret)) : null;
  const { recovery } = await searchParams;
  return (
    <main className={styles.page}>
      {order ? <OrderStatus order={toAssistedOrderCustomerView(order)} /> : (
        <section className={styles.missing}>
          <p className="eyebrow">Private order status</p>
          <h1>{recovery === 'expired' ? 'That link has expired.' : 'No order on this device.'}</h1>
          <p>Open the newest private link in your JeloCare email, or begin a guest basket. Recovery links work once; your clean status page then stays on this device.</p>
          <OrderRecoveryForm />
