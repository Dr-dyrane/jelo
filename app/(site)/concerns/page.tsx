...
export default async function ConcernsPage({
  ...
  const products = productsWithReviewedConcernLinks(
  ...
  const raw = Array.isArray(params.concerns)
    ? params.concerns[0]
    : params.concerns;
  const initialSelected = (raw ?? "")
    .split(",")
    ...
    <main className="page-shell">
      <EditorialEntry asset={storyAsset} priority>
        <header className="page-heading">
          ...
          <h1>What do you notice?</h1>
          <p>Choose one. Or a few.</p>
        </header>
      </EditorialEntry>
      <ConcernSelector
        concerns={concerns}
