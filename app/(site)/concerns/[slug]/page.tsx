...
import { ReviewedOn, SourceList, type SourceEntry } from '@/components/clinical/clinical-primitives';
...
export default async function ConcernPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const sources: SourceEntry[] = concern.sources.map(s => ({ title: s.title, url: s.url }));

  return <main className="page-shell">
    {/* 1. Plain-language summary */}
    <header className="page-heading">
      <p className="eyebrow">{concern.area} · {concern.kind === 'condition-pattern' ? 'Pattern guide' : 'Concern guide'}</p>
      <h1>{concern.name}</h1>
      <p>{concern.summary} Guidance, not a diagnosis.</p>
      <Link className="concern-combine" href={concern.kind === 'concern' ? `/concerns?concerns=${concern.slug}` : '/concerns'}>
        <Layers3 size={16} aria-hidden="true"/> {concern.kind === 'concern' ? 'Add another concern' : 'Browse concerns'}
      </Link>
    </header>

    {/* 2. Why it matters — urgent escalation content prominent and visually distinct */}
    {concern.urgentAction ? (
      <section className={`concern-urgent-action concern-urgent-action-${concern.urgentAction.urgency}`} aria-labelledby="concern-action-heading">
        <div>
          <p>What to do now</p>
          <h2 id="concern-action-heading">{concern.urgentAction.urgency === 'emergency' ? 'Get emergency help.' : 'Get medical help today.'}</h2>
        </div>
    ...
    <section className="concern-detail-grid">
      <div className="concern-detail-panel">
        ...
        <div className="concern-detail-chips">
          {concern.signals.map(item => <span key={item}>{item}</span>)}
        </div>
      </div>
      <div className="concern-detail-panel">
        ...
        <div className="concern-detail-chips">
          {concern.ingredients.map(item => {
            ...
            return (
              <span className={source ? 'concern-detail-chip-sourced' : undefined} key={item}>
                {item}
    ...
    {concern.kind === 'concern' ? <section className="concern-matches">
      ...
      <div className="section-heading"><h2>{matches.length ? 'Catalogue matches.' : 'Care first.'}</h2></div>
      {matches.length ? <ProductRail products={matches}/> : <p className="concern-no-match">Start with the care options above.</p>}
    </section> : null}
