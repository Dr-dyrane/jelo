...
import {
  offerFreshnessDays,
  offerPriceRank,
} from "@/modules/commerce/price-rank";
 ...
 * "Continue to retailer". An exact-offer continuation records the canonical
 * store_click event once, then redirects.
 *
...
export async function GET(request: Request) {
  ...
  const product = productSlug
    ? await findCatalogueProduct(productSlug)
    : undefined;
  ...
  if (offer) {
    ...
    const position = Math.min(
      ...
      Math.max(
        1,
        ranked.findIndex(
          (item) => item.retailer === offer.retailer && item.url === offer.url,
        ) + 1,
      ),
    );
  ...
  const searchUrl = retailerSearchUrl(
    retailerName,
