...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  if (!await allowCommunityAction(request, 'submit')) return NextResponse.json({ error: 'Please try again later.' }, { status: 429 });
  const secret = editSecretFromRequest(request, id);
  if (!secret) return NextResponse.json({ error: 'Draft access expired.' }, { status: 401 });

  try {
    const result = await submitCommunityDraft({ id, editSecretHash: hashEditSecret(secret) });
    ...
    if (error instanceof z.ZodError) {
      return NextResponse.json({ error: 'A few answers are still missing.', fields: error.issues.map(issue => issue.path[0]) }, { status: 422 });
    }
    return NextResponse.json({ error: 'Could not submit yet.' }, { status: 500 });
  }
}
