...
function QueueIcon({ queue }: { queue: ActivityDecision['queue'] }) {
  const Icon = queue === 'community_contribution'
    ? FileText
      ...
      {items.map(item => (
        <li key={item.label}>
          ...
          <span className={styles.rankTrack} aria-hidden="true">
            <span style={{ width: `${denominator > 0 ? Math.min((item.count / denominator) * 100, 100) : 0}%` }} />
          </span>
        </li>
      ))}
    </ol>
      ...
      <div className={styles.detailScroll}>
        <section className={styles.detailIdentity}>
          ...
          <div>
            ...
            <p>{actionLabels[decision.action]} {queueLabels[decision.queue].toLowerCase()}</p>
          </div>
        </section>
        ...
        <details className={styles.metadata}>
          ...
          <dl>
            ...
            <div>
              <dt>Source record</dt>
              <dd><code>{decision.targetRef}</code></dd>
            </div>
          </dl>
        </details>
      </div>
...
export function ActivityInsights({ inference }: { inference: ActivityInference }) {
  ...
  const decisionCountLabel = decisions.length < inference.audit.totalDecisions
    ...
    <div className={styles.surface}>
      ...
      <section className={styles.patterns} aria-labelledby="patterns-heading">
        <header className={styles.sectionHeading}>
          ...
          <span>Early sample · {number.format(community.approvedNotes)} approved notes</span>
        </header>
        ...
        <p className={styles.boundary}>
          Community reported. These mentions guide research; they do not prove results or retailer trust.
        </p>
      </section>
      ...
      <section className={styles.evidence} aria-labelledby="evidence-heading">
        <header className={styles.sectionHeading}>
          ...
          <span>Updated <RelativeTime iso={inference.generatedAt} /></span>
        </header>
        <div className={styles.evidenceRows}>
          <div>
            <span>Relationships</span>
            <strong>{number.format(evidence.approvedRelationships)} approved</strong>
            <small>{number.format(evidence.pendingRelationships)} waiting</small>
          </div>
          <div>
            <span>Observations</span>
            <strong>{number.format(evidence.approvedObservations)} approved</strong>
          ...
          <div>
            ...
            <small>{number.format(research.pendingRetailerResearch)} waiting</small>
          </div>
          <div>
            <span>Notes include</span>
            <strong>{number.format(community.experienceNotes)} experiences</strong>
                ...
                <span className={styles.decisionRow}>
                  ...
                  <span className={styles.decisionCopy}>
                    <span className={styles.decisionAction}>
                      {actionLabels[decision.action]} {queueLabels[decision.queue].toLowerCase()}
                    </span>
                    <strong>{decision.targetLabel}</strong>
                    <span className={styles.decisionMeta}>
                      {decision.operatorName} · <RelativeTime iso={decision.createdAt} />
                    </span>
                  </span>
                  <ChevronRight size={16} className={styles.decisionCaret} aria-hidden="true" />
                </span>
              )}
              renderItemDetails={decision => <DecisionDetail decision={decision} />}
            />
          </div>
        ) : (
