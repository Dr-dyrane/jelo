...
import { inviteOperatorAction, mutateOperatorAccessAction } from "./actions";
...
const accessActionLabels: Record<
  Exclude<
    ConsoleOperatorRecord["recentAccessActivity"][number]["action"],
    "send"
  >,
  string
> = {
...
function QueueIcon({ queue }: { queue: ConsoleOperatorActivity["queue"] }) {
  const Icon: ComponentType<{ size?: number; strokeWidth?: number }> =
...
function OperatorInspector({
  ...
  const feedback =
    mutationState?.targetId === row.id
      ...
      : null;
  const feedbackTone =
    mutationState?.tone ?? (mutationState?.ok ? "success" : "warning");
      ...
      <div className={styles.detailScroll}>
        ...
        <section className={styles.detailSection}>
          ...
          <dl className={styles.facts}>
            ...
            <div>
              <dt>Role</dt>
              <dd>{roleLabels[row.role]}</dd>
            </div>
            <div>
              <dt>Can review</dt>
              <dd>{roleScope[row.role]}</dd>
            </div>
            {row.kind === "operator" ? (
              <div>
                <dt>Added</dt>
                <dd>{exactDate.format(new Date(row.createdAt))}</dd>
              </div>
            ) : (
              <>
                <div>
                  <dt>Invited by</dt>
                  <dd>{row.invitedByName}</dd>
                </div>
                <div>
                  <dt>Delivery</dt>
                ...
                {row.lastSentAt ? (
                  <div>
                    <dt>Last sent</dt>
                    <dd>
                      <RelativeTime iso={row.lastSentAt} />
                    </dd>
                  </div>
                ) : null}
                {row.expiresAt && row.status !== "revoked" ? (
                  <div>
                    <dt>Expires</dt>
                    <dd>{exactDate.format(new Date(row.expiresAt))}</dd>
                  </div>
                ) : null}
              </>
            )}
        ...
        {row.kind === "operator" ? (
          <section className={styles.detailSection}>
            ...
            <dl className={styles.facts}>
              <div>
                <dt>Today</dt>
                <dd>{number.format(row.decisionsToday)}</dd>
              </div>
              <div>
                <dt>Last 7 days</dt>
                <dd>{number.format(row.decisionsLast7Days)}</dd>
              </div>
              <div>
                <dt>All time</dt>
                <dd>{number.format(row.decisionsTotal)}</dd>
              </div>
              <div>
                <dt>Last decision</dt>
                <dd>
                  {row.lastActionAt ? (
                    <RelativeTime iso={row.lastActionAt} />
                  ) : (
                    "None yet"
                  )}
                </dd>
                ...
                <li key={activity.id}>
                  ...
                  <span>
                    ...
                    <small>
                      ...
                      {queueLabels[activity.queue].toLocaleLowerCase("en-NG")}
                      {" · "}
        ...
        {accessLifecycleReady ? (
          ...
          <section className={styles.detailSection}>
            ...
            <p className={styles.emptyCopy}>
              Updates will be available after setup.
            </p>
          </section>
        )}
      </div>
        ...
        <section className={styles.detailDecision}>
          ...
          <div className={styles.actionStack}>
            {row.kind === "invitation" && row.status !== "revoked" ? (
                ...
                {invitationSendLabel ? (
                  <form action={mutationAction}>
                    ...
                    >
                      ...
                      {isMutationPending
                        ? invitationSendLabel === "Renew invitation"
                          ? "Renewing…"
                          : "Sending…"
                        : invitationSendLabel}
                    </button>
              ...
              <p className={styles.quietNote}>
                Ask another admin to pause your access.
              </p>
            ) : (
  ...
  const sections = useMemo<InboxCollectionSection<ConsoleOperatorRecord>[]>(
    () => [
      ...
      },
      {
        ...
        itemIds: rows
          .filter(
            (row) => row.status === "inactive" || row.status === "revoked",
          )
          .map((row) => row.id),
      },
    ],
    ...
    <>
      ...
      <div className={styles.directory}>
        <InboxContainer
          ...
          renderItemRow={(row, _active, context) =>
            context?.presentation === "feature-shelf" ? (
              ...
              <span className={styles.compactRow}>
                ...
                <span className={styles.compactCopy}>
                  <strong>{operatorName(row)}</strong>
                  <span>
                    {roleLabels[row.role]} · {statusLabel(row)}
                  </span>
                </span>
          ...
          renderItemDetails={(row) =>
            isSelectionPending && selectedId === row.id ? (
        ...
        <section className={styles.dialogSheet}>
          ...
          {accessLifecycleReady ? (
            ...
            <p className={styles.setupMessage}>
              Access updates will be available after setup.
            </p>
          )}
        </section>
      </dialog>
          ...
          {confirmAction ? (
            <form action={mutationAction} className={styles.confirmActions}>
              ...
              <input
                type="hidden"
                name="targetKind"
                value={confirmAction.targetKind}
              />
              <input
                type="hidden"
                name="targetId"
                value={confirmAction.targetId}
              />
              {mutationState?.targetId === confirmAction.targetId &&
              !mutationState.ok ? (
                <p className={styles.errorFeedback} role="alert">
                  {mutationState.error}
                </p>
