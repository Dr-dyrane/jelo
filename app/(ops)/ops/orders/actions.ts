...
async function deliverOrderUpdate(orderId: string): Promise<OrderActionResult & { ok: true }> {
  try {
    const delivery = await deliverPendingAssistedOrderNotifications({ orderId, limit: 5 });
    if (delivery.sent > 0) return { ok: true, delivery: 'sent' };
    if (delivery.failed > 0) return { ok: true, delivery: 'failed' };
    if (delivery.pending > 0 || delivery.unavailable) return { ok: true, delivery: 'pending' };
...
export async function retryOrderNotificationAction(input: unknown): Promise<OrderActionResult> {
  try {
    const operator = await requireConsoleOperator();
    assertCan(operator, 'orders.manage');
    ...
    if (!admitted) return { ok: false, error: 'This delivery is no longer retryable.' };
