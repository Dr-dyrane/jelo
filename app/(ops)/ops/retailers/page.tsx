  ...
  return (
    <>
      ...
      {rows.length === 0 ? <Empty label="retailer application" /> : (
        <Table head={<tr><th>Store</th><th>Email</th><th>Verified</th><th>Submitted</th><th>Decision</th></tr>}>
          {rows.map(row => (
            <tr key={row.id}>
              <td>{row.storeName}</td>
              ...
              <td><DecideForm action={decideRetailerApplicationAction} id={row.id} approveLabel="Approve" rejectLabel="Decline" /></td>
            </tr>
          ))}
        </Table>
      )}
