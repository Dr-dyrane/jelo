...
test('Me modal chrome uses neutral dark scrims without changing light scrims', async () => {
  ...
  assert.match(accountSheet, /\.dialog::backdrop\s*{\s*background: rgb\(54 29 26 \/ 34%\);/);
  assert.match(accountSheet, /background: rgb\(54 29 26 \/ 58%\);/);
  assert.equal(occurrences(accountSheet, 'background: var(--glass-scrim);'), 2);
  assert.equal(occurrences(accountSheet, 'box-shadow: 0 30px 100px rgb(0 0 0 / 72%);'), 2);

  assert.match(meHome, /\.filterDialog::backdrop \{ background: rgb\(52 32 29 \/ 38%\);/);
  assert.equal(occurrences(meHome, 'background: var(--glass-scrim);'), 2);
  assert.equal(occurrences(meHome, 'box-shadow: 0 -30px 90px rgb(0 0 0 / 72%);'), 2);
});
...
test('Ops dropdowns and modal backdrops use the neutral dark foundation', async () => {
  ...
  assert.equal(occurrences(opsShell, 'border-color: rgba(255, 255, 255, 0.08);'), 2);

  assert.match(vocabulary, /\.targetDialog::backdrop\s*{\s*background: rgba\(31, 23, 22, \.32\);/);
  assert.equal(occurrences(vocabulary, 'background: var(--glass-scrim);'), 2);

  assert.match(operators, /\.dialog::backdrop\s*{\s*background: rgba\(31, 23, 22, \.32\);/);
...
test('shared workspace dock retains its light elevation and neutralizes dark elevation', async () => {
  const dock = await readSource('components/workspace-shell/adaptive-workspace-dock.module.css');
