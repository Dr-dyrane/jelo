...
describe('Observation list visual integrity', () => {
  it('keeps rows transparent while giving packshots one restrained media stage', () => {
    assert.match(shareStyles, /packshot renders straight onto the card, no nested container/i);
    ...
    for (const selector of [
      ...
      assert.doesNotMatch(rule, /\bbackground\s*:/, `${selector} must stay transparent`);
    }
    ...
    for (const selector of [
      ...
      const rule = cssRule(observationStyles, selector);
      assert.match(rule, /background:\s*color-mix\(in srgb,\s*var\(--ops-surface-subtle\)/);
      assert.match(rule, /border-radius:/);
      assert.doesNotMatch(rule, /\bbox-shadow\s*:/, `${selector} must not become an independent card`);
      assert.doesNotMatch(rule, /\bborder\s*:/, `${selector} must stay borderless`);
    }
  });
  ...
  it('lets the shared interactive row own selected, hover, and focus state', () => {
    ...
    assert.match(activeRule, /background:\s*var\(--ops-accent-subtle\)/);
    assert.doesNotMatch(observationSource, /selectedSurface/);
    assert.match(inboxSource, /className=\{`\$\{styles\.sectionItemButton\} \$\{isActive \? styles\.sectionItemButtonActive/);
    assert.match(inboxSource, /aria-current=\{isActive \? 'true' : undefined\}/);
  ...
  it('keeps compact packshots legible inside the same restrained media-stage grammar', () => {
    ...
    assert.match(stageRule, /background:\s*color-mix\(in srgb,\s*var\(--ops-surface-subtle\)/);
  ...
  it('keeps loading geometry paired with each ready-state observation presentation', () => {
    const loadingSurfaceRule = cssRule(inboxStyles, '.skeletonSectionItemButton');
