...
test("reserved public dark overrides consume the shared black-cherry foundation", async () => {
  ...
  assert.equal(
    occurrences(productExperience, "--product-peach: var(--peach);"),
    2,
  );
  assert.equal(
    occurrences(productExperience, "--product-pink: var(--card-3);"),
    2,
  );
  assert.equal(
    occurrences(productExperience, "--product-blush: var(--surface-3);"),
    2,
  );
  ...
  assert.match(
    platform,
    /html\[data-theme="dark"\] \.site-header\{background:rgba\(0,0,0,\.74\)\}/,
  );

  assert.equal(
    occurrences(
      trust,
      "linear-gradient(135deg, var(--peach), var(--cream) 58%)",
    ),
    2,
  );
  ...
  assert.equal(
    occurrences(recommendation, "background:rgba(255,255,255,.1)"),
    2,
  );

  assert.doesNotMatch(concernDetail, /rgba\((?:36,29,27|122,86,72),/);
  assert.equal(
    occurrences(concernDetail, "background: var(--state-warning-bg)"),
    2,
  );
  assert.equal(
    (
      concernDetail.match(
        /\.concern-detail-chip-sourced a\s*\{\s*color:\s*var\(--muted\)/g,
      ) ?? []
    ).length,
    2,
  );
