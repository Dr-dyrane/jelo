...
test("every public page family has explicit contextual social metadata", () => {
  ...
  const sitePages = appFiles
    .filter(
      (file) =>
        file.startsWith("app/(site)/") && /\/page\.(?:ts|tsx)$/.test(file),
    )
    .sort();
  const covered = PUBLIC_SOCIAL_ROUTE_COVERAGE.map(
    (route) => route.source,
  ).sort();
  ...
  const imageFiles = appFiles.filter((file) =>
    /\/(?:opengraph|twitter)-image\.(?:ts|tsx)$/.test(file),
  );
  assert.deepEqual(
    imageFiles,
    [],
    "route-local image files must not bypass the shared renderer",
  );
...
test("the full page tree is classified as contextual public or intentionally non-indexable", () => {
  ...
  const publicFiles = new Set(
    PUBLIC_SOCIAL_ROUTE_COVERAGE.map((route) => route.source),
  );
  const classified = pageFiles.filter(
    (file) =>
      publicFiles.has(
        file as (typeof PUBLIC_SOCIAL_ROUTE_COVERAGE)[number]["source"],
      ) ||
      ...
      MARKET_FIXTURE_PAGES.includes(
        file as (typeof MARKET_FIXTURE_PAGES)[number],
      ) ||
      file.startsWith("app/(auth)/") ||
      file.startsWith("app/(campaign)/campaign-review/") ||
...
test("private campaign reviews are noindex, noarchive, no-referrer, dynamic, and have no social payload", () => {
  const layout = readFileSync(
    path.join(root, "app/(campaign)/campaign-review/layout.tsx"),
    "utf8",
  );
  const page = readFileSync(
    path.join(root, "app/(campaign)/campaign-review/[reportId]/page.tsx"),
    "utf8",
  );
  ...
  assert.match(layout, /openGraph:\s*null/);
  assert.match(layout, /twitter:\s*null/);
...
test("product and share routes preserve exact SKU identity and exact packshot binding", async () => {
  ...
  assert.match(
    productCard.eyebrow,
    new RegExp(shortest.brand.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")),
  );
  ...
  const resolved = await resolveSocialCard(
    new URL(socialImageUrl(longShareCard)),
  );
...
test("missing product media produces a truthful text fallback", () => {
  ...
  const markup = renderToStaticMarkup(
    createElement(SocialCard, { card, packshotSrc: null }),
  );
...
test("canonical URLs are absolute and Open Graph/Twitter stay in parity", () => {
  const filtered = catalogueSocialCard({
    category: "Hair",
    market: "US",
    sort: "newest",
  });
...
test("social image URLs change when route or product content changes", () => {
  ...
  const renamed = productSocialCard(
    { ...product, name: `${product.name} Updated` },
    "product",
  );
  const newPackshot = productSocialCard(
    { ...product, image: `${product.image}?revision=2` },
    "product",
  );
...
test("private customer and operations layouts are noindex, no-store, and have no social payload", () => {
  for (const sourcePath of [
    ...
    assert.doesNotMatch(
      source,
      /operator\.|customer\.|session\.|email.*openGraph/i,
    );
  }
