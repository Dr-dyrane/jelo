...
export const storeClickEventSchema = z.object({
  productSlug: z.string().min(1).max(160),
  retailer: z.string().min(1).max(160),
  market: z.enum(['NG', 'US']),
  priceNgn: z.number().int().min(0).max(100_000_000).nullable(),
...
export async function recordStoreClick(event: StoreClickEvent): Promise<void> {
  ...
  const { productSlug, retailer, market, priceNgn, priceRank, position, freshnessDays } = parsed.data;
  try {
