...
export const retailerPartnershipDraftSchema = z
  .object({
    ...
    email: z
      .email()
      .max(254)
      .transform((value) => value.trim().toLocaleLowerCase("en-NG")),
    phone: optionalText,
    ...
    contactConsent: z
      .boolean()
      .refine((value) => value, "Contact permission is required."),
    currentStep: z.number().int().min(1).max(8),
  })
  .strict();

export type RetailerPartnershipDraft = z.infer<
  typeof retailerPartnershipDraftSchema
>;

export const createRetailerPartnershipSchema = retailerPartnershipDraftSchema
  .pick({
    storeName: true,
    channels: true,
    state: true,
...
export const saveRetailerPartnershipSchema = z
  .object({
