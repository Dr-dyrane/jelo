...
export function retailerPartnershipSecretFromRequest(
  request: NextRequest,
  applicationId: string,
) {
  const parsed = parseRetailerPartnershipToken(
    request.cookies.get(retailerPartnershipCookieName)?.value ?? null,
  );
...
function networkKey(request: NextRequest) {
  ...
  const secret =
    process.env.RETAILER_PARTNERSHIP_RATE_LIMIT_SECRET ??
...
export async function allowRetailerPartnershipAction(
  ...
  const result = await limiter.limit(
    `${networkKey(request)}${applicationId ? `:${applicationId}` : ""}`,
  );
