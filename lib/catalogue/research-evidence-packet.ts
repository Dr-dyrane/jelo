...
export const catalogueResearchEvidencePacketPolicy =
  "private-evidence-packet-only" as const;
export const catalogueResearchEvidencePacketPublicationStatus =
...
export type StaticResearchEvidencePacket = {
  ...
  research: {
    ...
    knownProduct?: NonNullable<CatalogueResearchQueueItem["knownProduct"]>;
  };
  productLead: {
    discoveryId: string;
    title: string;
    ...
    identityStatus: "unverified-retailer-lead" | "known-product-review-binding";
...
export type ResearchEvidencePacket =
  StaticResearchEvidencePacket | CommunityResearchEvidencePacket;
...
export type CommunityResearchPacketRequest =
  { mode: "single"; label: string } | { mode: "batch"; count: number };
...
function packetId(source: string, identity: string) {
  return sha256(
    `jelocare-private-research-evidence-packet-v1\n${source}\n${identity}\n`,
  ).slice(0, 24);
}
...
function assertRequestedCount(value: number) {
  if (
    ...
    value < 1 ||
    value > maximumCatalogueResearchPacketBatch
  ) {
    throw new Error(
      `Research packet batch must be between 1 and ${maximumCatalogueResearchPacketBatch}.`,
    );
  }
}
...
function assertQueueOffset(value: number, queueLength: number) {
  if (!Number.isSafeInteger(value) || value < 0 || value >= queueLength) {
    throw new Error(
      "Research packet queue offset is outside the static priority queue.",
    );
  }
}
...
function staticQueueItem(
  queue: CatalogueResearchQueue,
  request: StaticResearchPacketRequest,
) {
  if (queue.policy !== catalogueResearchQueuePolicy)
    throw new Error("Static research queue is not private research only.");
  if (request.mode === "single") {
    if (!discoveryIdPattern.test(request.discoveryId))
      throw new Error("Static packet discovery id is invalid.");
    const item = queue.items.find(
      (candidate) => candidate.discoveryId === request.discoveryId,
    );
    if (!item)
      throw new Error(
        `Static research priority ${request.discoveryId} was not found.`,
      );
    return [item];
  }
  ...
  if (offset + request.count > queue.items.length) {
    throw new Error(
      "Research packet batch extends beyond the static priority queue.",
    );
  }
...
function sourceResponse(snapshot: CatalogueDiscoverySnapshot, id: string) {
  ...
  if (!response)
    throw new Error(`Discovery observation references unknown response ${id}.`);
...
function staticPacket(
  ...
  if (
    ...
    throw new Error(
      `${candidate.discoveryId} is not a private discovery lead.`,
    );
  }
  if (
    ...
    throw new Error(
      `${candidate.discoveryId} has an unexpected research state.`,
    );
  }
  if (!item.offerTargets.length) {
    throw new Error(
      `${candidate.discoveryId} has no exact offer review targets.`,
    );
  }
  const productRef =
    item.knownProduct?.productRef ?? `discovery:${candidate.discoveryId}`;
  const targetKeys = new Set<string>();
  const selected = item.offerTargets.map((target) => {
    const expectedKey = catalogueResearchOfferKey(
      productRef,
      target.retailer,
      target.listingUrl,
    );
    if (target.offerKey !== expectedKey || targetKeys.has(target.offerKey)) {
      throw new Error(
        `${candidate.discoveryId} has an invalid or duplicated exact offer target.`,
      );
    }
    targetKeys.add(target.offerKey);
    const matches = candidate.retailerObservations.filter(
      (observation) =>
        catalogueResearchOfferKey(
          productRef,
    ...
    if (matches.length !== 1) {
      throw new Error(
        `${candidate.discoveryId}/${target.retailer} exact offer target has ${matches.length} immutable snapshot bindings.`,
      );
    }
    return { target, observation: matches[0]! };
  });
  const observations = selected.map(({ target, observation }) => {
    ...
    if (
      ...
      throw new Error(
        `${candidate.discoveryId} has an untraceable retailer observation.`,
      );
    }
    return {
      offerKey: target.offerKey,
      retailer: target.retailer,
      retailerStatus: target.retailerStatus,
  ...
  const retailerCodeLeads = selected.flatMap(({ target, observation }) =>
    observation.retailerSku
      ? [
          {
            retailer: target.retailer,
  ...
  return {
    ...
    research: {
      ...
      ...(item.knownProduct
        ? { knownProduct: structuredClone(item.knownProduct) }
        : {}),
    },
    productLead: {
      ...
      canonicalSlug: item.knownProduct?.canonicalProductSlug ?? null,
      identityStatus:
        item.researchKind === "known-product-additional-offer"
          ? "known-product-review-binding"
...
export function buildStaticResearchEvidencePacketManifest(
  ...
  if (sourceSnapshotSha256 !== snapshotSha256) {
    throw new Error(
      "Static research queue does not match the supplied discovery snapshot bytes.",
    );
  }
  const packets = staticQueueItem(queue, request).map((item) => {
    const candidate = snapshot.candidates.find(
      (value) => value.discoveryId === item.discoveryId,
    );
    if (!candidate)
      throw new Error(
        `Research priority ${item.discoveryId} is absent from discovery evidence.`,
      );
  ...
  return {
    ...
    source: {
      ...
      discoverySnapshotSha256: sourceSnapshotSha256,
    },
    packets,
  };
}
...
function communityTaskIdentity(task: CommunityAggregateResearchTask) {
  return [
    task.taskKind,
    task.entitySource,
    task.identity?.canonicalSlug ?? "",
    task.identity?.brand ?? "",
    task.identity?.name ?? task.entityLabel,
    task.identity?.size ?? "",
  ].join("|");
}
...
function hasForbiddenCommunityIdentifier(value: unknown): boolean {
  ...
  return Object.entries(value).some(
    ([key, nested]) =>
      /^(?:contributor|contribution|submission|draft|email|phone|contact|user|session)/i.test(
        key,
...
function assertCommunityTask(task: CommunityAggregateResearchTask) {
  if (hasForbiddenCommunityIdentifier(task))
    throw new Error("Community report task contains a contributor identifier.");
  if (
    !["product-identity", "product-retail-refresh"].includes(task.taskKind) ||
    task.entityKind !== "product"
  ) {
    throw new Error("Community report task is not a product research task.");
  }
  if (
    !["canonical", "custom"].includes(task.entitySource) ||
    task.priorityLane !== "community-first"
  ) {
    throw new Error("Community report task has an invalid research source.");
  }
  if (
    task.publicationStatus !== "private-research-only" ||
    !Number.isSafeInteger(task.signalCount) ||
    task.signalCount < 1
  ) {
    throw new Error(
      "Community report task is not a private aggregate research signal.",
    );
  }
  if (!task.entityLabel.trim() || !validIso(task.lastSeenAt))
    throw new Error("Community report task is malformed.");
}
...
function communityTasks(
  ...
  if (!validIso(report.generatedAt) || !Array.isArray(report.researchQueue))
    throw new Error("Community aggregate report is invalid.");
  const tasks = report.researchQueue.filter(
    (task) => task.entityKind === "product",
  );
  ...
  if (request.mode === "single") {
    ...
    if (!task)
      throw new Error(
        `Community aggregate research task ${request.label} was not found.`,
      );
    return [task];
  }
  assertRequestedCount(request.count);
...
export function buildCommunityResearchEvidencePacketManifest(
  ...
  const packets = communityTasks(report, request).map(
    (task): CommunityResearchEvidencePacket => ({
...
function allProofSlotsUncollected(slots: ResearchEvidenceProofSlots) {
  return (
    slots.officialIdentity.status === "uncollected" &&
...
export function assertPrivateResearchEvidencePacketManifest(
  manifest: CatalogueResearchEvidencePacketManifest,
) {
  if (
    ...
    throw new Error(
      "Research evidence packet manifest is invalid or publishable.",
    );
  if (manifest.selection.requestedCount !== manifest.packets.length) {
    throw new Error(
      "Research evidence packet manifest count does not match its packets.",
    );
  }
  if (manifest.selection.source === "static-priority") {
    assertHash(manifest.source.staticQueueSha256, "Static packet queue");
    assertHash(
      manifest.source.discoverySnapshotSha256,
      "Static packet discovery snapshot",
    );
  } else
    assertHash(
      manifest.source.communityReportSha256,
      "Community packet report",
    );

  const ids = new Set<string>();
  for (const packet of manifest.packets) {
    if (!discoveryIdPattern.test(packet.id) || ids.has(packet.id))
      throw new Error("Research packet id is invalid or duplicated.");
    ids.add(packet.id);
    if (
      packet.publicationStatus !==
        catalogueResearchEvidencePacketPublicationStatus ||
      !allProofSlotsUncollected(packet.proofSlots)
    ) {
      throw new Error(
        "Research evidence packets must remain uncollected and non-publishable.",
      );
    }
    if (packet.source === "static-priority") {
      if (
        manifest.selection.source !== "static-priority" ||
        !discoveryIdPattern.test(packet.productLead.discoveryId)
      ) {
        throw new Error("Static research packet source is invalid.");
      }
      if (!packet.discoveryEvidence.observations.length)
        throw new Error("Static research packet lacks source observations.");
      const research =
        packet.research as StaticResearchEvidencePacket["research"] & {
          kind?: CatalogueResearchKind;
        };
      if (research.kind !== undefined) {
        if (research.kind === "known-product-additional-offer") {
          const known = research.knownProduct;
          if (
            !known?.productRef.trim() ||
            known.authority !== "private-review-target-only" ||
            ![
              "retailer-code-crosswalk-review",
              "reviewed-title-size-alias-review",
            ].includes(known.bindingBasis) ||
            research.nextAction !==
              "review-additional-offer-for-known-product" ||
            packet.productLead.identityStatus !==
              "known-product-review-binding" ||
            packet.productLead.canonicalSlug !==
              (known.catalogueStatus === "public-catalogue"
                ? known.productRef
                : null)
          )
            throw new Error(
              "Known-product research packet lacks its private canonical binding.",
            );
        } else if (
          research.kind !== "new-product-identity" ||
          research.knownProduct !== undefined ||
          packet.productLead.canonicalSlug !== null ||
          packet.productLead.identityStatus !== "unverified-retailer-lead" ||
          research.nextAction === "review-additional-offer-for-known-product"
        )
          throw new Error(
            "New-product research packet contains a canonical product claim.",
          );
      }
      const productRef =
        research.knownProduct?.productRef ??
        `discovery:${packet.productLead.discoveryId}`;
      ...
      for (const observation of packet.discoveryEvidence.observations) {
        if (
          ...
          throw new Error(
            "Static research packet contains an invalid exact retailer retrieval locator.",
          );
        }
        if (
          research.kind !== undefined &&
          (!discoveryIdPattern.test(observation.offerKey) ||
            offerKeys.has(observation.offerKey) ||
            observation.offerKey !==
              catalogueResearchOfferKey(
                productRef,
          ...
          throw new Error(
            "Static research packet exact offer binding is invalid or duplicated.",
          );
        if (observation.offerKey) offerKeys.add(observation.offerKey);
        ...
        if (
          ...
          throw new Error(
            "Static research packet exact retailer retrieval locator is not scoped to its listing.",
          );
        }
      }
      for (const lead of packet.discoveryEvidence.retailerCodeLeads) {
        if (
          lead.treatment !== "retailer-local-code-not-manufacturer-identity"
        ) {
          throw new Error(
            "Retailer code lead was promoted to product identity.",
          );
        }
      }
    } else if (
      manifest.selection.source !== "community-aggregate" ||
      packet.aggregateEvidence.privacy !==
        "aggregate-task-only-no-contributor-identifiers"
    ) {
