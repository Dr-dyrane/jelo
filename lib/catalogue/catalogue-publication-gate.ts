...
export const catalogueApprovalScope =
  "identity-source-rights-packaging-and-catalogue-fit" as const;
...
export type CatalogueQualitySignals = {
  ...
  backgroundTreatment:
    "none" | "styled-composite" | "automated-removal" | "unknown";
...
export type ExternalCatalogueApproval = {
  ...
  asset: {
    ...
    rightsBasis:
      "cc-by-sa-3.0" | "official-brand-permission" | "owned-editorial";
    rightsUrl: string;
...
export type ExternalCatalogueGateDecision<
  T extends ExternalCatalogueGateCandidate,
> = {
  candidate: T;
...
export type ApprovedExternalCatalogueProduct<
  T extends ExternalCatalogueGateCandidate,
> = T & {
  canonicalImageUrl: string;
  imageSha256: string;
  ...
  imageHeight: number;
  imageTreatment:
    "untouched-publication-asset" | "identity-verified-styled-composite";
...
function httpsUrl(value: string, hostname?: string) {
  try {
    ...
    return (
      url.protocol === "https:" && (!hostname || url.hostname === hostname)
    );
  } catch {
    return false;
  }
...
function reviewedBrandEvidenceHost(brand: string, value: string | undefined) {
  ...
  return (
    reviewedBrandEvidenceHosts[brandKey]?.includes(
      new URL(value).hostname.toLowerCase(),
    ) ?? false
  );
}

export function externalCandidateFingerprint(
...
export function assessCatalogueQuality(
  signals: CatalogueQualitySignals,
): CatalogueQualityAssessment {
  const identity = signals.identityTraceable ? 20 : 0;
  const formula =
    signals.formulaEvidence === "complete"
      ? 15
      : signals.formulaEvidence === "partial"
        ? 8
        : 0;
  const evidence =
    signals.productEvidence === "high"
      ? 15
  ...
  const allowedBackground =
    signals.backgroundTreatment === "none" ||
    signals.backgroundTreatment === "styled-composite";
  const presentation =
    (allowedAssetOrigins.has(signals.assetOrigin) ? 4 : 0) +
  ...
  const concerns = [
    ...
    signals.formulaEvidence === "missing" ||
    signals.formulaEvidence === "unknown"
      ? "formula-evidence-incomplete"
      : null,
    signals.productEvidence === "not-reviewed"
      ? "product-evidence-not-reviewed"
      : null,
    !signals.exactNigeriaOffer ? "no-exact-nigeria-offer" : null,
    signals.imageRights !== "documented"
      ? `image-rights-${signals.imageRights}`
      : null,
    !allowedAssetOrigins.has(signals.assetOrigin)
      ? "asset-origin-not-approved"
      : null,
    ...
    !signals.labelVariantSizeUnchanged
      ? "label-variant-or-size-not-confirmed"
      : null,
    signals.packagingInvented === true ? "packaging-invented" : null,
    signals.packagingInvented === "unknown"
      ? "packaging-originality-not-confirmed"
      : null,
    !signals.manualSourceOutputQa ? "manual-source-output-qa-missing" : null,
    signals.presentationQuality !== "magazine-ready"
      ? "final-image-not-magazine-ready"
      : null,
...
function freshNigeriaOffers(
  ...
  return approval.nigeria.exactOffers.filter((offer) => {
    ...
    return (
      offer.retailer.trim().length >= 2 &&
      httpsUrl(offer.listingUrl) &&
      offer.variant
        .trim()
        .localeCompare(candidate.name.trim(), undefined, {
          sensitivity: "base",
        }) === 0 &&
      offer.size
        .trim()
        .localeCompare(candidate.quantity.trim(), undefined, {
          sensitivity: "base",
        }) === 0 &&
      isValidGtin(offer.observedGtin) &&
      canonicalGtin(offer.observedGtin) === canonicalGtin(candidate.barcode) &&
      ["explicit-gtin", "explicit-ean", "explicit-upc"].includes(
...
function validApproval(approval: ExternalCatalogueApproval, asOf: number) {
  ...
  return (
    approval.scope === catalogueApprovalScope &&
...
function hardGateReason(
  ...
  if (
    !Number.isFinite(approvedAt) ||
    causalEvidenceTimes.some(
      (timestamp) => !Number.isFinite(timestamp) || timestamp > approvedAt,
    )
  )
  ...
  const independentRetailers = new Set(
    directoryOffers.map(({ retailer }) => normalized(retailer.name)),
  );
  const independentHosts = new Set(
    directoryOffers.map(({ offer }) => normalizedHost(offer.listingUrl)),
  );
  const tierARoute =
    sameUrl(
      approval.nigeria.tierAIdentityEvidenceUrl,
      approval.sku.evidenceUrl,
    ) &&
    independentRetailers.size >= 2 &&
    independentHosts.size >= 2;
  const brandAuthorizationEvidenceUrl =
    approval.nigeria.brandAuthorizationEvidenceUrl;
  ...
  const brandRoute =
    reviewedBrandEvidenceHost(candidate.brand, brandAuthorizationEvidenceUrl) &&
    reviewedBrandEvidenceHost(candidate.brand, approval.sku.evidenceUrl);
  ...
  if (
    ...
    brandAuthorizedOffers.some(({ retailer }) => {
      ...
      return (
        ...
        [evidence.observedAt, evidence.retrievedAt, evidence.reviewedAt].some(
          (value) => Date.parse(value) > approvedAt,
        )
      );
    })
  )
    return "invalid-approval";
  if (!tierARoute && !sellerBoundBrandRoute)
    return "nigeria-market-evidence-insufficient";
  return undefined;
}
...
function externalSignals(
  ...
  asOf: number,
): CatalogueQualitySignals {
  const identityTraceable =
    isValidGtin(candidate.barcode) &&
  ...
  const rightsDocumented =
    candidate.imageLicense === "CC BY-SA 3.0" &&
  ...
  return {
    ...
    source: "community",
    identityTraceable,
    formulaEvidence: approval?.careReview.formulaArchetype.trim()
      ? "complete"
      : "unknown",
    productEvidence: "not-reviewed",
    exactNigeriaOffer: approval
      ? freshNigeriaOffers(candidate, approval, asOf).length > 0
      : false,
    imageRights:
      rightsDocumented ||
      approval?.asset.rightsBasis === "official-brand-permission" ||
      approval?.asset.rightsBasis === "owned-editorial"
        ? "documented"
        : "missing",
    ...
    labelVariantSizeUnchanged:
      approval?.asset.labelVariantSizeUnchanged ?? false,
...
export function evaluateExternalCatalogueCandidate<
  T extends ExternalCatalogueGateCandidate,
>(
  candidate: T,
  approval: ExternalCatalogueApproval | undefined,
  ...
  const quality = assessCatalogueQuality(
    externalSignals(candidate, approval, asOf),
  );
  if (!approval)
    return {
      candidate,
  ...
  if (!validApproval(approval, asOf))
    return {
      candidate,
      status: "private-candidate",
  ...
  const candidateIsCutout =
    candidate.imageTreatment === "source-faithful-background-extraction" ||
  ...
  if (
    ...
    return {
      candidate,
      status: "private-candidate",
      reason: "automated-background-removal",
      quality,
      approval,
    };
  ...
  if (
    !allowedAssetOrigins.has(
      quality.dimensions.presentation === 20
        ? approval.asset.origin
        : "unknown",
    ) ||
    ...
    return {
      candidate,
      status: "private-candidate",
      reason: "asset-not-publication-ready",
      quality,
      approval,
    };
  return {
...
export function gateExternalCatalogue<T extends ExternalCatalogueGateCandidate>(
  ...
  if (manifest.approvals.length > 0) {
    throw new Error(
      "External catalogue publication is disabled; move exact-SKU research through the private intake dossier gate.",
    );
  }
  const approvals = new Map<string, ExternalCatalogueApproval>();
  for (const approval of manifest.approvals) {
    if (approvals.has(approval.barcode))
      throw new Error(`Duplicate catalogue approval for ${approval.barcode}.`);
  ...
  for (const barcode of approvals.keys()) {
    if (!candidateIds.has(barcode))
      throw new Error(
        `Catalogue approval ${barcode} has no current candidate.`,
      );
  }
  ...
  const decisions = candidates.map((candidate) =>
    evaluateExternalCatalogueCandidate(
      candidate,
      approvals.get(candidate.barcode),
      asOf,
    ),
  );
  const approved = decisions.flatMap((decision) =>
    decision.status === "approved" && decision.approval
      ? [
          {
            ...decision.candidate,
            ...
            imageTreatment:
              decision.approval.asset.backgroundTreatment === "styled-composite"
                ? "identity-verified-styled-composite"
