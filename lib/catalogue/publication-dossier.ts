import { stableJson, fingerprint } from "@/lib/crypto/hashing";
...
export const cataloguePublicationApprovalScope =
  "exact-identity-source-care-retail-rights-and-final-image" as const;
export const catalogueReferencePublicationApprovalScope =
  "exact-identity-source-care-rights-and-final-image-reference-only" as const;
...
export type CataloguePublicationDossier = {
  ...
  finalImage: {
    ...
    packaging: "intact";
    backgroundTreatment:
      "none" | "source-pixel-isolation" | "identity-verified-render";
...
function cloneOfficialIdentityExtraction(
      ...
      ...(extraction.fields.manufacturerBrandAliases
        ? {
            manufacturerBrandAliases:
              extraction.fields.manufacturerBrandAliases.map((alias) => ({
                ...alias,
              })),
          }
        : {}),
  ...
  if (extraction.schemaVersion === 6) {
    return {
      ...
      supplementalResponses: extraction.supplementalResponses.map(
        (response) => ({ ...response }),
      ),
      browserCapture: { ...extraction.browserCapture },
      identifierCorroborations: extraction.identifierCorroborations.map(
        (corroboration) => ({
          ...corroboration,
          fields: {
    ...
    return {
      ...
      fields: {
        ...
        manufacturerIdentifierStatus: {
          ...extraction.fields.manufacturerIdentifierStatus,
        },
        packageVersion: { ...extraction.fields.packageVersion },
      },
      supplementalResponses: extraction.supplementalResponses.map(
    ...
    return [
      {
        ...
        listingHost: new URL(offer.listingUrl).hostname
          .replace(/^www\./, "")
          .toLowerCase(),
        evidence: {
...
export function catalogueIntakeCandidateFingerprint(
  candidate: CatalogueIntakeCandidate,
) {
...
function createCataloguePublicationDossierCore(
  ...
  const referenceOnlyReady =
    ...
    decision.blockers.every((blocker) =>
      referenceOnlyMarketBlockers.has(blocker),
    );
  if (!decision.approvalDraftReady && !referenceOnlyReady) {
    throw new Error(
      `${candidate.id} is not approval-ready (${decision.stage}: ${decision.blockers.join(", ")}).`,
    );
  }
  if (
  ...
  if (referenceOnly && decision.approvalDraftReady) {
    throw new Error(
      `${candidate.id} has verified market evidence and must use the full publication scope.`,
    );
  }
  if (approval.reviewer.trim().length < 2)
    throw new Error(`${candidate.id} approval reviewer is missing.`);

  const approvedAt = parsedDate(
    approval.approvedAt,
    `${candidate.id} approval timestamp`,
    asOf,
  );
  const officialIdentity = required(
    candidate.identity.officialEvidence,
    `${candidate.id} official identity evidence`,
  );
  const canonicalIdentifier = required(
    catalogueCanonicalIdentifierFor(candidate.identity),
    `${candidate.id} canonical identifier`,
  );
  const identityCheckedAt = required(
    candidate.identity.checkedAt,
    `${candidate.id} identity timestamp`,
  );
  const identityEvidenceRetrievedAt = required(
    officialIdentity.retrievedAt,
    `${candidate.id} official identity retrieval timestamp`,
  );
  const careReviewedAt = required(
    candidate.care.reviewedAt,
    `${candidate.id} care review timestamp`,
  );
  const sourceAssetRetrievedAt = required(
    candidate.asset.sourceAssetRetrievedAt,
    `${candidate.id} source asset retrieval timestamp`,
  );
  const artReviewedAt = required(
    candidate.asset.artReviewedAt,
    `${candidate.id} art review timestamp`,
  );
  const identityCheckedTimestamp = parsedDate(
    identityCheckedAt,
    `${candidate.id} identity timestamp`,
    asOf,
  );
  ...
  const artReviewedTimestamp = parsedDate(
    artReviewedAt,
    `${candidate.id} art review timestamp`,
    asOf,
  );
  ...
  const nigeriaMarketRoute = referenceOnly
    ...
    : required(
        decision.nigeriaMarketRoute,
        `${candidate.id} Nigeria market route`,
      );
  const brandSellerAuthorizationEvidence =
  ...
  if (identityEvidenceRetrievedTimestamp > identityCheckedTimestamp) {
    throw new Error(
      `${candidate.id} identity review predates its official evidence snapshot.`,
    );
  }
  if (sourceAssetRetrievedTimestamp > artReviewedTimestamp) {
    throw new Error(
      `${candidate.id} art review predates its source asset retrieval.`,
    );
  }
  const generatedTimestamp = candidate.asset.generationRecord
    ? parsedDate(
        candidate.asset.generationRecord.generatedAt,
        `${candidate.id} generation timestamp`,
        asOf,
      )
    : undefined;
  if (
    ...
    throw new Error(
      `${candidate.id} generation must follow source retrieval and precede art review.`,
    );
  ...
  if (
    ...
    !(
      parsedDate(
        isolationRecord.audit.generatedAt,
        `${candidate.id} isolation run timestamp`,
        asOf,
    ...
    throw new Error(
      `${candidate.id} isolation must follow source retrieval and precede art review.`,
    );
  const evidenceTimes = [
    ...
    ...(regulatoryEvidence
      ? [
          parsedDate(
            regulatoryEvidence.retrievedAt,
            `${candidate.id} regulatory retrieval timestamp`,
            asOf,
          ),
          parsedDate(
            regulatoryEvidence.observedAt,
            `${candidate.id} regulatory observation timestamp`,
            asOf,
          ),
          parsedDate(
            regulatoryEvidence.reviewedAt,
            `${candidate.id} regulatory review timestamp`,
            asOf,
          ),
        ]
      : []),
    ...
    ...(isolationRecord
      ? [
          ...
          parsedDate(
            isolationRecord.review.identityReviewedAt,
            `${candidate.id} isolation identity review timestamp`,
            asOf,
          ),
          parsedDate(
            isolationRecord.review.artReviewedAt,
            `${candidate.id} isolation art review timestamp`,
            asOf,
          ),
        ]
      : []),
    ...decision.freshExactOffers.flatMap((offer) => [
      ...
      parsedDate(
        required(
          ...
          `${candidate.id} offer evidence retrieval timestamp`,
        ),
        `${candidate.id} offer evidence retrieval timestamp`,
        asOf,
      ),
      parsedDate(
        required(
          ...
          `${candidate.id} offer evidence review timestamp`,
        ),
        `${candidate.id} offer evidence review timestamp`,
        asOf,
      ),
    ...
    ...brandSellerAuthorizationEvidence.map((item) =>
      parsedDate(
        ...
        `${candidate.id} seller authorization observation timestamp`,
        asOf,
      ),
    ),
    ...brandSellerAuthorizationEvidence.flatMap((item) => [
      parsedDate(
        ...
        `${candidate.id} seller authorization retrieval timestamp`,
        asOf,
      ),
      parsedDate(
        item.evidence.reviewedAt,
        `${candidate.id} seller authorization review timestamp`,
        asOf,
      ),
    ]),
  ];
  if (approvedAt < Math.max(...evidenceTimes))
    throw new Error(`${candidate.id} approval predates its bound evidence.`);
  ...
  const payload = {
    ...
    sourceEvidence: {
      basis: required(
        ...
        `${candidate.id} identity basis`,
      ),
      officialProductUrl: required(
        candidate.identity.officialProductUrl,
        `${candidate.id} official source`,
      ),
      officialIdentity: {
    ...
    care: {
      ...
      formulaArchetype: required(
        candidate.care.formulaArchetype,
        `${candidate.id} formula archetype`,
      ),
      careTier: required(candidate.care.careTier, `${candidate.id} care tier`),
      reviewScope: required(
        candidate.care.reviewScope,
        `${candidate.id} care review scope`,
      ),
      advisoryBoundary: required(
        candidate.care.advisoryBoundary,
        `${candidate.id} advisory boundary`,
      ),
      ...
      reviewer: required(
        candidate.care.reviewer,
        `${candidate.id} care reviewer`,
      ),
    },
    nigeria: {
      ...
      ...(nigeriaMarketRoute === "reference-only"
        ...
        : nigeriaMarketRoute === "tier-a"
          ? {
              tierAIdentityEvidenceUrl: required(
                candidate.nigeria.tierAIdentityEvidenceUrl,
                `${candidate.id} Tier-A evidence`,
              ),
            }
          : {
    ...
    rights: {
      ...
      ...(candidate.asset.origin !== "owned-identity-verified-render"
        ? {
            evidenceUrl: required(
              candidate.asset.rightsUrl,
              `${candidate.id} rights evidence`,
            ),
          }
        : {}),
      sourceAsset: {
        url: required(
          ...
          `${candidate.id} source asset`,
        ),
        sha256: required(
          candidate.asset.sourceAssetSha256,
          `${candidate.id} source asset hash`,
        ),
        mimeType: required(
          candidate.asset.sourceAssetMimeType,
          `${candidate.id} source asset MIME type`,
        ),
        byteSize: required(
          candidate.asset.sourceAssetByteSize,
          `${candidate.id} source asset byte size`,
        ),
        width: required(
          candidate.asset.sourceAssetWidth,
          `${candidate.id} source asset width`,
        ),
        height: required(
          candidate.asset.sourceAssetHeight,
          `${candidate.id} source asset height`,
        ),
      ...
      ...(candidate.asset.generationRecord
        ? {
            generationRecord: {
              ...
              inputs: candidate.asset.generationRecord.inputs.map((input) => ({
                ...input,
              })),
        ...
        : {}),
      ...(isolationRecord
        ? { isolationRecord: structuredClone(isolationRecord) }
        : {}),
    },
    finalImage: {
      ...
      url: required(
        ...
        `${candidate.id} final image`,
      ),
      sha256: required(
        candidate.asset.publicImageSha256,
        `${candidate.id} final image hash`,
      ),
      mimeType: required(
        candidate.asset.publicImageMimeType,
        `${candidate.id} final image MIME type`,
      ),
      byteSize: required(
        candidate.asset.publicImageByteSize,
        `${candidate.id} final image byte size`,
      ),
      width: required(
        candidate.asset.width,
        `${candidate.id} final image width`,
      ),
      height: required(
        candidate.asset.height,
        `${candidate.id} final image height`,
      ),
      packaging: candidate.asset.packaging as "intact",
      backgroundTreatment: candidate.asset.backgroundTreatment as
        "none" | "source-pixel-isolation" | "identity-verified-render",
      labelVariantSizeUnchanged: candidate.asset
        .labelVariantSizeUnchanged as true,
      ...
      presentationQuality: candidate.asset
        .presentationQuality as "magazine-ready",
      reviewedAt: artReviewedAt,
      reviewer: required(
        candidate.asset.artReviewer,
        `${candidate.id} art reviewer`,
      ),
    },
    approval: {
  ...
  const dossierFingerprint = fingerprint(
    "jelocare-catalogue-publication-dossier-v8",
    payload,
  );
...
function approvalFromStoredDossier(
...
function verifyCataloguePublicationDossierManifestCore(
  ...
  const source = objectRecord(
    manifest,
    "Catalogue publication dossier manifest",
  );
  if (source.schemaVersion !== cataloguePublicationDossierSchemaVersion)
    throw new Error("Unsupported catalogue publication dossier schema.");
  if (source.exposure !== cataloguePublicationExposure)
    throw new Error("Catalogue publication dossiers must remain private-only.");
  if (!Array.isArray(source.dossiers))
    throw new Error(
      "Catalogue publication dossier manifest must contain a dossiers array.",
    );
  ...
  const dossiers = source.dossiers.map((value, index) => {
    const stored = objectRecord(
      value,
      `Catalogue publication dossier ${index}`,
    );
    if (typeof stored.candidateId !== "string" || !stored.candidateId)
      throw new Error(
        `Catalogue publication dossier ${index} has no candidate ID.`,
      );
    const candidateId = stored.candidateId;
    if (seenCandidates.has(candidateId))
      throw new Error(
        `Duplicate catalogue publication dossier: ${candidateId}`,
      );
    ...
    if (!candidate)
      throw new Error(
        `Catalogue publication dossier ${candidateId} has no current intake candidate.`,
      );
    const currentCandidateFingerprint =
      catalogueIntakeCandidateFingerprint(candidate);
    if (stored.candidateFingerprint !== currentCandidateFingerprint) {
      throw new Error(
        `${candidateId} candidate fingerprint changed; approval is invalid.`,
      );
    }
    ...
    if (
      typeof stored.dossierFingerprint !== "string" ||
      !hashPattern.test(stored.dossierFingerprint)
    ) {
      throw new Error(`${candidateId} dossier fingerprint is invalid.`);
    }
    if (seenFingerprints.has(stored.dossierFingerprint))
      throw new Error(
        `Duplicate catalogue publication fingerprint: ${stored.dossierFingerprint}`,
      );
    seenFingerprints.add(stored.dossierFingerprint);
    if (stableJson(stored) !== stableJson(expected))
      throw new Error(
        `${candidateId} dossier content or fingerprint changed; approval is invalid.`,
      );
    if (seenImageHashes.has(expected.finalImage.sha256)) {
      throw new Error(
        `${candidateId} reuses another catalogue publication image hash.`,
      );
    }
...
export function verifyCataloguePublicationDossierManifest(
  ...
  return verifyCataloguePublicationDossierManifestCore(
    candidates,
    manifest,
    asOf,
  );
