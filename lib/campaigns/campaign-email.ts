...
const roleCopy: Record<PacketRole, { number: string; label: string }> = {
  proof: {
    number: "01",
    label: "Market",
  },
...
function verifiedActionUrl(value: string) {
  let parsed: URL;
  try {
  ...
  if (
    ...
    parsed.search ||
    parsed.hash ||
    (!allowed.includes(parsed.pathname) &&
      !/^\/share\/[a-z0-9]+(?:-[a-z0-9]+)*$/.test(parsed.pathname))
  ) {
    throw new Error("campaign_email_action_url_invalid");
  }
  return value;
...
export function dailyCampaignEmail(input: {
  ...
  const subject = `${prefix}Today’s JeloCare packet · Market, Useful, Relatable`;
  ...
  const coverage = `${input.draft.selection.catalogueProductCount} public products checked · ${input.draft.selection.freshPriceCandidateCount} fresh price candidates`;
  verifiedActionUrl(input.draft.actionUrl);
  ...
  const text = [
    ...
    ...images.flatMap((image, index) => {
      ...
      const pillar = input.draft.creativePlan.packet[index]!.pillar;
      const pillarActionUrl = verifiedActionUrl(pillar.actionUrl);
      return [
        `${role.number} ${role.label}`,
        pillar.headline,
        pillar.body,
        `Caption: ${pillar.caption}`,
        `Review note: ${pillar.evidenceNote}`,
        `Download story: ${image.downloadUrl}`,
        `${pillar.action}: ${pillarActionUrl}`,
  ...
  const sections = images
    .map((image, index) => {
      ...
      const safeAlt = escapeHtml(
        `JeloCare ${role.label} campaign: ${pillar.headline}`,
      );
      return `
        <tr>
            ...
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#fffdf9" class="section-card" style="width:100%;background-color:#fffdf9;border-radius:24px;border-collapse:separate">
              <tr>
                <td class="section-copy" style="padding:28px 28px 22px">
                  ...
                  <h2 class="section-title" style="margin:0 0 10px;color:#2d211f;font-family:Georgia,'Times New Roman',serif;font-size:30px;line-height:1.08;font-weight:400">${escapeHtml(pillar.headline)}</h2>
                  <p class="section-context" style="margin:0;color:#7a6b66;font-size:15px;line-height:1.55">${escapeHtml(pillar.body)}</p>
                </td>
              </tr>
              <tr>
              ...
              <tr>
                <td class="section-copy" style="padding:24px 28px 30px">
                  <p class="caption" style="margin:0 0 20px;color:#514945;font-size:15px;line-height:1.65">${escapeHtml(pillar.caption)}</p>
                  <table role="presentation" cellpadding="0" cellspacing="0" border="0" class="action-table" style="border-collapse:separate">
                    <tr>
                      ...
                      <td class="action-cell secondary-cell" bgcolor="#f4d4c5" style="border-radius:999px;background-color:#f4d4c5">
                        <a href="${escapeHtml(pillarActionUrl)}" class="secondary-action" style="display:inline-block;padding:13px 18px;color:#2d211f;font-size:14px;font-weight:600;text-decoration:none">${escapeHtml(pillar.action)}</a>
                      </td>
                    </tr>
                  </table>
                  <p class="evidence-note" style="margin:22px 0 0;color:#7a6b66;font-size:12px;line-height:1.55">${escapeHtml(pillar.evidenceNote)} Checked ${safe.observed}. Draft creative for internal review.</p>
                </td>
              </tr>
            </table>
          </td>
          ...
          <table role="presentation" width="620" cellpadding="0" cellspacing="0" border="0" bgcolor="#f8ece7" class="email-card" style="width:100%;max-width:620px;margin:0 auto;background-color:#f8ece7;border-radius:30px;border-collapse:separate">
            <tr>
              <td class="header-cell" style="padding:38px 30px 34px">
                ...
                <p class="email-lead" style="margin:0;color:#6f625e;font-size:16px;line-height:1.6">${safe.greeting} A market check, one useful story and one relatable story are ready for review.</p>
                <p class="evidence-note" style="margin:14px 0 0;color:#7a6b66;font-size:12px;line-height:1.55">${safe.coverage}. Selection is brand-neutral and scans the full public catalogue.</p>
              </td>
            </tr>
            ${sections}
            <tr>
