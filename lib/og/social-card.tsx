...
type CatalogueFilterKind = 'query' | 'category' | 'brand' | 'concern' | 'step' | 'market';
...
const staticCards = {
  ...
  catalogue: {
    ...
    metaTitle: 'Products',
    metaDescription: 'Browse exact skincare, haircare, and body-care products with clear context.',
  ...
  concerns: {
    ...
    metaDescription: 'Browse calm, educational guides for skin and hair concerns.',
  ...
  retailers: {
    ...
    description: 'A guide to the Nigerian beauty-store sources JeloCare checks.',
    detail: 'A listing is not proof of authenticity.',
    metaTitle: 'Retailers',
    metaDescription: 'The Nigerian beauty-store sources JeloCare checks, plus retailer partnerships.',
  ...
  consult: {
    ...
    metaDescription: 'Describe what you notice about your skin and receive educational guidance.',
  ...
  contribute: {
    ...
    metaDescription: 'Tell JeloCare about one skincare product. Anonymous and usually under a minute.',
  ...
  'share-index': {
    ...
    metaDescription: 'Exact products and source-checked guides worth passing on.',
...
export const PUBLIC_SOCIAL_ROUTE_COVERAGE = [
  ...
  { family: '/share/[slug]', source: 'app/(site)/share/[slug]/page.tsx', context: 'exact shareable SKU and packshot' },
  { family: '/share/ingredient/[slug]', source: 'app/(site)/share/ingredient/[slug]/page.tsx', context: 'exact source-checked ingredient' },
] as const;
...
export function staticSocialCard(kind: keyof typeof staticCards): SocialCardModel {
...
function canonicalCatalogueQuery(params: SearchParams) {
  ...
  const category = ['Face', 'Hair', 'Body'].find(value => value === first(params, 'category'));
  const concern = concernBySlug(first(params, 'concern'));
  const brand = products
    .map(product => product.brand)
    .find(value => value.toLocaleLowerCase() === first(params, 'brand').toLocaleLowerCase());
  const step = products
    .map(product => product.step)
    .find(value => value.toLocaleLowerCase() === first(params, 'step').toLocaleLowerCase());
...
export function catalogueSocialCard(params: SearchParams): { card: SocialCardModel; canonicalPath: string } {
  ...
  if (resolved.q) {
    ...
    const label = resolved.category === 'Hair' ? 'Hair & scalp' : `${resolved.category} care`;
...
export function productSocialCard(
  product: Pick<Product, 'slug' | 'brand' | 'name' | 'size' | 'category' | 'image'>,
  surface: ProductSurface,
): SocialCardModel {
  const exactName = `${product.brand} ${product.name}`;
  return {
    request: { kind: 'product', slug: product.slug, productSurface: surface },
    eyebrow: surface === 'share' ? `Worth sharing · ${product.brand}` : `${product.brand} · JeloCare`,
    title: product.name,
    description: `${product.size} · ${product.category}`,
    detail: surface === 'share' ? 'Exact product. Clear context.' : 'Product profile at jelocare.com',
    metaTitle: exactName,
...
export function hasPublishableProductImage(image: string | null | undefined) {
  return Boolean(image && !/\/(?:product-)?(?:placeholder|fallback)(?:[./-]|$)/i.test(image));
}
...
export function socialImageUrl(card: SocialCardModel) {
  ...
  if (card.request.productSurface) url.searchParams.set('surface', card.request.productSurface);
  if (card.request.filterKind) url.searchParams.set('filter', card.request.filterKind);
  if (card.request.filterValue) url.searchParams.set('value', card.request.filterValue);
  url.searchParams.set('v', socialCardVersion(card));
  return url.toString();
}

export function publicSocialMetadata(card: SocialCardModel, canonicalPath: string): Metadata {
...
type ProductCardIdentity = Pick<Product, 'slug' | 'brand' | 'name' | 'size' | 'category' | 'image'>;
...
export async function resolveSocialCard(
  ...
  if (kind === 'catalogue') {
    const filterKind = url.searchParams.get('filter') as CatalogueFilterKind | null;
    const filterValue = url.searchParams.get('value') ?? '';
    return catalogueSocialCard(catalogueRequestParams({
      kind,
  ...
  if (kind === 'product') {
    ...
    return productSocialCard(product, url.searchParams.get('surface') === 'share' ? 'share' : 'product');
  }
  if (kind === 'retailer') {
...
export function SocialCard({ card, packshotSrc }: { card: SocialCardModel; packshotSrc?: string | null }): ReactElement {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        boxShadow: card.theme === 'light' ? '0 28px 80px rgba(112,71,61,.14)' : '0 28px 80px rgba(0,0,0,.52)',
      }}>
        <div style={{
          position: 'absolute',
          width: 360,
        ...
        {isProduct ? (
          <div style={{
            ...
            {hasPackshot ? (
              ...
              <img src={packshotSrc as string} width={340} height={440} style={{ objectFit: 'contain' }} alt="" />
            ) : (
              <div style={{
                width: '100%',
                height: '100%',
                ...
                <span style={{ fontFamily: 'Italiana', fontSize: 108, lineHeight: 1, color: palette.ink }}>J</span>
                <span style={{ display: 'flex', fontSize: 18, lineHeight: 1.4, marginTop: 20 }}>Exact packshot unavailable</span>
              </div>
            )}
          </div>
        ) : null}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ display: 'flex', width: 52, height: 1, background: palette.hairline, marginRight: 16 }} />
            {card.detail}
          </div>
          <div style={{
            display: 'flex',
