...
import { hasInsufficientData } from "@/lib/clinical/care-evidence-bridge";

export const communityResearchPriorityLane = "community-first" as const;
export const communityResearchPublicationStatus =
  "private-research-only" as const;
...
export type CommunityResearchTask = {
  ...
  context: {
    ...
    products: string[];
    retailers: string[];
    needsCareEvidence?: boolean;
  };
};

export function communityResearchTasks(
  draft: ContributionDraft,
): CommunityResearchTask[] {
  const context = {
  ...
  const tasks: CommunityResearchTask[] = [
    ...draft.products.map((product) => {
      const productSlug =
        product.source === "canonical"
          ? product.id.replace(/^product:/, "")
          : product.id;
      const needsCareEvidence =
        product.source === "canonical"
          ? hasInsufficientData(productSlug)
          : false;
      return {
        taskKind:
          product.source === "custom"
            ? ("product-identity" as const)
            : ("product-retail-refresh" as const),
        ...
        context: needsCareEvidence
          ? { ...context, needsCareEvidence: true }
          : context,
      };
    }),
    ...draft.retailers.map((retailer) => ({
      taskKind:
        retailer.source === "custom"
          ? ("retailer-identity" as const)
          : ("retailer-refresh" as const),
  ...
  const uniqueTasks = [
    ...new Map(
      tasks.map((task) => [`${task.taskKind}:${task.entityRef}`, task]),
    ).values(),
  ];
