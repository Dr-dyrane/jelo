...
import { adaptiveValueSchema, type AdaptiveValue } from '@/lib/community-intake/schema';
...
const optionalUrl = z
  ...
  .refine(value => value === null || /^https?:\/\//i.test(value), { message: 'Use an http or https link.' })
  .catch(null);

const ngnAmount = z
  .union([z.number().int().min(100).max(100_000_000), z.null(), z.undefined()])
  .transform(value => value ?? null);
...
function normalizePhone(value: string | null): { display: string | null; href: string | null } {
  if (!value) return { display: null, href: null };
  const cleaned = value.replace(/[^0-9+]/g, '');
  return cleaned ? { display: value, href: `tel:${cleaned}` } : { display: value, href: null };
}

function normalizeWhatsApp(value: string | null): { display: string | null; href: string | null } {
  if (!value) return { display: null, href: null };
  const digits = value.replace(/[^0-9]/g, '');
  return digits ? { display: value, href: `https://wa.me/${digits}` } : { display: value, href: null };
}

function normalizeInstagram(value: string | null): { display: string | null; href: string | null } {
  if (!value) return { display: null, href: null };
  const cleaned = value.replace('@', '').trim();
  return cleaned ? { display: `@${cleaned}`, href: `https://instagram.com/${cleaned}` } : { display: value, href: null };
}

function normalizeWebsite(value: string | null): { display: string | null; href: string | null } {
  if (!value) return { display: null, href: null };
...
function locationDisplay(city: string | null, state: string | null, address: string | null): string {
...
export function retailerApplicationReviewItem(
  ...
  return {
    ...
    facebook: { display: payload.facebook, href: payload.facebook ? `https://facebook.com/${payload.facebook}` : null },
    sample: {
