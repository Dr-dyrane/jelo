...
function fixtureStore() {
  const scope = globalThis as typeof globalThis & {
    __jelocareAssistedOrders?: {
      ...
      recoveries: Map<string, { orderId: string; expiresAt: number; consumed: boolean }>;
...
export async function createAssistedOrder(record: CreateAssistedOrderRecord) {
  ...
  const orderId = await sql.begin(async transaction => {
    const identities = await transaction<{ identity_version_id: string; slug_at_review: string }[]>`
    ...
    `;
    const identityBySlug = new Map(identities.map(row => [row.slug_at_review, row.identity_version_id]));
    ...
    const [order] = createdOrder ? [createdOrder] : await transaction<{ id: string; request_fingerprint: string }[]>`
      ...
      for update
    `;
    if (!order || ('request_fingerprint' in order && order.request_fingerprint !== record.requestFingerprint)) {
...
export async function readAssistedOrderForOwner(orderId: string, ownerSubject: string) {
...
export async function exchangeAssistedOrderRecovery(recoveryHash: string, sessionHash: string) {
  if (assistedOrderFixtureEnabled()) {
    const store = fixtureStore();
    const recovery = store.recoveries.get(recoveryHash);
    if (!recovery || recovery.consumed || recovery.expiresAt <= Date.now()) return null;
    recovery.consumed = true;
    for (const [hash, orderId] of store.sessions) if (orderId === recovery.orderId) store.sessions.delete(hash);
...
export async function replaceAssistedOrderRecovery(input: {
  ...
  if (assistedOrderFixtureEnabled()) {
    const store = fixtureStore();
    const order = Array.from(store.orders.values()).find(candidate =>
      candidate.reference === input.reference
      && candidate.contactEmail.toLocaleLowerCase('en') === input.contactEmail.toLocaleLowerCase('en'),
    );
    if (!order) return null;
    for (const [hash, recovery] of store.recoveries) if (recovery.orderId === order.id) store.recoveries.delete(hash);
    store.recoveries.set(input.recoveryHash, { orderId: order.id, expiresAt: Date.now() + assistedOrderRecoveryMaxAge * 1000, consumed: false });
    return { reference: order.reference, contactEmail: order.contactEmail, contactName: order.contactName };
  }
  const sql = getPostgresClient();
  return sql.begin(async transaction => {
    const [order] = await transaction<{ id: string; public_reference: string; contact_email: string; contact_name: string }[]>`
...
export async function decideAssistedOrderQuote(input: {
  ...
  const updatedId = await sql.begin(async transaction => {
    ...
    const nextState = input.decision === 'approve' ? 'payment_pending' : 'needs_response';
    await transaction`
...
export async function listAssistedOrderQueue() {
  await expireAssistedOrderQuotes();
  if (assistedOrderFixtureEnabled()) {
    return Array.from(fixtureStore().orders.values()).sort((a, b) => a.updatedAt.localeCompare(b.updatedAt));
  }
  const sql = getPostgresClient();
  const rows = await sql.unsafe<OrderRow[]>(`${orderSelect}
...
export async function transitionAssistedOrderForOperator(input: {
  ...
  const id = await sql.begin(async transaction => {
    const allowedFrom = input.toState === 'quoting' ? ['requested', 'needs_response'] : ['requested', 'quoting', 'awaiting_approval', 'needs_response', 'payment_pending'];
    const [order] = await transaction<{ id: string; state: AssistedOrderState }[]>`
...
function createFixtureOrder(record: CreateAssistedOrderRecord): AssistedOrderPrivateView {
  const store = fixtureStore();
  const existingRequest = store.requests.get(record.requestKeyHash);
  if (existingRequest) {
    if (existingRequest.fingerprint !== record.requestFingerprint) throw new Error('order_idempotency_conflict');
    for (const [hash, orderId] of store.sessions) if (orderId === existingRequest.orderId) store.sessions.delete(hash);
    for (const [hash, recovery] of store.recoveries) if (recovery.orderId === existingRequest.orderId) store.recoveries.delete(hash);
  ...
  store.requests.set(record.requestKeyHash, { orderId: id, fingerprint: record.requestFingerprint });
...
function transitionFixtureOrder(input: {
  ...
  const allowed = input.toState === 'quoting'
    ? order?.state === 'requested' || order?.state === 'needs_response'
    : Boolean(order && ['requested', 'quoting', 'awaiting_approval', 'needs_response', 'payment_pending'].includes(order.state));
...
function submitFixtureQuote(input: {
  ...
  const order = fixtureStore().orders.get(input.orderId);
  if (!order || order.revision !== input.revision || order.state !== 'quoting') return null;
  const version = (order.quote?.version ?? 0) + 1;
  const totalNgn = Object.values(input.components).reduce<number>((sum, value) => sum + (value ?? 0), 0);
  order.quote = { id: randomUUID(), version, status: 'awaiting_approval', components: input.components, totalNgn, evidenceReference: input.evidenceReference, notes: input.notes, issuedAt: new Date().toISOString(), expiresAt: input.expiresAt, approvedAt: null };
...
function decideFixtureQuote(input: {
  ...
  const sessionOwns = input.sessionHash && store.sessions.get(input.sessionHash) === input.orderId;
  const ownerOwns = input.ownerSubject && order?.ownerSubject === input.ownerSubject;
  if (!order || (!sessionOwns && !ownerOwns) || order.revision !== input.revision
      || order.state !== 'awaiting_approval' || order.quote?.version !== input.quoteVersion
      || order.quote.status !== 'awaiting_approval' || new Date(order.quote.expiresAt) <= new Date()) return null;
  const nextState = input.decision === 'approve' ? 'payment_pending' : 'needs_response';
  order.quote.status = input.decision === 'approve' ? 'approved' : 'declined';
  order.quote.approvedAt = input.decision === 'approve' ? new Date().toISOString() : null;
  order.state = nextState;
  order.revision += 1;
  order.updatedAt = new Date().toISOString();
  order.events.push({ id: randomUUID(), action: input.decision === 'approve' ? 'quote_approved' : 'quote_declined', fromState: 'awaiting_approval', toState: nextState, reason: input.reason, createdAt: order.updatedAt });
  return order;
}
