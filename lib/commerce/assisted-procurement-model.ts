...
export const CUSTOMER_VISIBLE_ORDER_STATES: Record<AssistedOrderState, {
  label: string;
  detail: string;
}> = {
  requested: {
    label: 'Request received',
    detail: 'JeloCare will verify the exact products, retailer terms, and delivery cost.',
  },
  quoting: {
    label: 'Checking your basket',
    detail: 'A staff member is confirming each exact item and every cost component.',
  },
  awaiting_approval: {
    label: 'Quote ready',
    detail: 'Review the complete quote. Nothing proceeds until you approve this exact version.',
  },
  needs_response: {
    label: 'Your response is needed',
    detail: 'The quote changed, expired, or needs a decision before work can continue.',
  },
  payment_pending: {
    label: 'Quote approved',
    detail: 'Your approval is recorded. Payment instructions are not yet available in JeloCare.',
  },
  paid: { label: 'Payment confirmed', detail: 'Governed payment evidence has been recorded.' },
  procurement: { label: 'Being purchased', detail: 'The retailer order is being placed for the exact approved items.' },
  retailer_confirmed: { label: 'Retailer confirmed', detail: 'The retailer has accepted the exact order.' },
  out_for_delivery: { label: 'Out for delivery', detail: 'Dispatch evidence has been recorded.' },
  delivered: { label: 'Delivered', detail: 'Delivery has been recorded.' },
  cancelled: { label: 'Cancelled', detail: 'No further procurement will proceed.' },
  refund_pending: { label: 'Refund pending', detail: 'A governed refund is being reconciled.' },
...
export function canTransitionAssistedOrder(from: AssistedOrderState, to: AssistedOrderState) {
...
export function quoteTotal(components: AssistedOrderQuoteComponents): number | null {
  const values = Object.values(components);
  if (values.some(value => value == null || !Number.isInteger(value) || value < 0)) return null;
...
export function quoteIsComplete(input: {
  ...
  return quoteTotal(input.components) != null
...
export type AssistedOrderQuoteView = {
  id: string;
  version: number;
  status: 'awaiting_approval' | 'approved' | 'declined' | 'expired' | 'superseded';
  components: AssistedOrderQuoteComponents;
