export const ASSISTED_ORDER_NOTIFICATION_ACTIONS = {
  ...
  quote_expired: {
    ...
    message: 'The previous quote expired. JeloCare will verify a fresh version before anything proceeds.',
  },
  order_cancelled: {
    kind: 'cancelled',
  ...
  out_for_delivery: {
    ...
    message: 'Dispatch evidence has been recorded for the exact approved order.',
  },
  delivered: {
    kind: 'delivered',
...
export type AssistedOrderNotificationAction = keyof typeof ASSISTED_ORDER_NOTIFICATION_ACTIONS;
export type AssistedOrderNotificationKind = typeof ASSISTED_ORDER_NOTIFICATION_ACTIONS[AssistedOrderNotificationAction]['kind'];
export type AssistedOrderNotificationEmailStatus = 'pending' | 'sending' | 'sent' | 'failed' | 'suppressed';
...
export function notificationCopyForAction(action: string) {
  return Object.hasOwn(ASSISTED_ORDER_NOTIFICATION_ACTIONS, action)
    ? ASSISTED_ORDER_NOTIFICATION_ACTIONS[action as AssistedOrderNotificationAction]
    : null;
}
