...
export function formatBaselinePrice(slug: string, preferred: Market = 'NG') {
  ...
  const locale = price.currency === 'NGN' ? 'en-NG' : 'en-US';
  return new Intl.NumberFormat(locale, { style: 'currency', currency: price.currency, maximumFractionDigits: price.currency === 'NGN' ? 0 : 2 }).format(price.amount);
}
