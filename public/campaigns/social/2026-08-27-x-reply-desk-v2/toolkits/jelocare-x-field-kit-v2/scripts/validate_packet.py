...
def main() -> int:
    ...
    asset_manifest = directory / "examples/visual/ASSETS.json"
    allowed_media = {}
    if asset_manifest.is_file():
        declared = json.loads(asset_manifest.read_text())
        if declared.get("synthetic") is not True:
            errors.append("fixture assets must have original synthetic provenance")
        allowed_media = {item["path"]: item["sha256"] for item in declared.get("assets", [])}
    ...
    for path in directory.rglob("*"):
        ...
        if path.stat().st_size > 25_000_000:
            errors.append(f"unexpected file larger than 25 MB: {relative}")
        if path.suffix.lower() in media_extensions:
            if not relative.startswith("examples/visual/") or allowed_media.get(relative) != hashlib.sha256(path.read_bytes()).hexdigest():
                errors.append(f"unregistered or changed fixture media: {relative}")
