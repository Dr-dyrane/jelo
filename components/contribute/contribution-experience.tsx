...
import {
  AdaptiveSelector,
  type AdaptiveOption,
} from "@/components/ui/adaptive-selector";
import {
...
const kindOptions: AdaptiveOption[] = [
  {
    id: "product",
    label: "One product",
    detail: "What you used and where you found it",
  },
  {
    id: "routine",
    label: "My routine",
    detail: "The products you use together",
  },
  {
    id: "store",
    label: "A trusted store",
    detail: "A place others should know",
  },
];
...
function flowFor(
  ...
  hasKind: boolean,
  catalogueHandoff: boolean,
): FlowStep[] {
  if (!hasKind) return ["kind"];
  if (draft.kind === "routine")
    return ["kind", "purposes", "products", "outcome", "review"];
  if (draft.kind === "store") return ["kind", "purposes", "retailer", "review"];
  const needsBrand =
    draft.products.length === 0 || draft.products[0]?.source === "custom";
  if (catalogueHandoff) {
    return [
      "products",
      ...(needsBrand ? ["brand" as const] : []),
      "purposes",
      "retailer",
      "purchase",
      "outcome",
...
export function ContributionExperience({
  purposes,
  products,
  brands,
  retailers,
  initialProduct = null,
}: ContributionExperienceProps) {
  ...
  const [catalogueHandoff, setCatalogueHandoff] = useState(
  ...
  const flow = useMemo(
    () => flowFor(draft, hasKind, catalogueHandoff),
    [catalogueHandoff, draft, hasKind],
  );
    ...
    const frame = window.requestAnimationFrame(() => {
      ...
      if (restored?.draft) {
        ...
        if (partial) {
          ...
          revisionRef.current = Number.isInteger(restored.revision)
            ? (restored.revision ?? 0)
            : 0;
  ...
  const recordEvent = useCallback(
    (
      type: IntakeEvent["type"],
      inputMode: IntakeEvent["inputMode"],
  ...
  function patchDraft(patch: Partial<ContributionDraft>) {
    setDraft((current) => ({
      ...current,
      ...patch,
      currentStep: Math.min(stepIndex + 1, 9),
    }));
    setError("");
  }
  ...
  function chooseProducts(values: AdaptiveValue[]) {
    ...
    const knownBrand = option?.detail
      ? brands.find((brand) => brand.label === option.detail)
      : undefined;
    patchDraft({
      products: values,
      brands: knownBrand ? [selection(knownBrand.id, knownBrand.label)] : [],
    });
  }
  ...
  async function submit() {
    ...
    const events = pendingEventsRef.current.splice(0, 50);
    saveQueueRef.current = saveQueueRef.current.then(() =>
      saveSnapshot(validated.data, events),
    );
  ...
  function startAnother() {
    ...
    window.localStorage.removeItem(localDraftKey);
    if (catalogueHandoff)
      window.history.replaceState(null, "", "/contribute#contribution-form");
  }
  ...
  if (complete)
    return (
      <section
        ...
        aria-labelledby="contribution-complete-title"
      >
        <span>
        ...
        <div>
          <button type="button" onClick={startAnother}>
            <RotateCcw size={16} aria-hidden="true" /> Add another
          </button>
  ...
  const kindValue = hasKind
    ? [
        selection(
          draft.kind,
          kindOptions.find((option) => option.id === draft.kind)?.label ??
            draft.kind,
        ),
      ]
    : [];
  const outcomeValue = draft.outcome
    ? [
        selection(
          draft.outcome,
          outcomeOptions.find((option) => option.id === draft.outcome)?.label ??
            draft.outcome,
        ),
      ]
    : [];
  const progress =
    flow.length > 1 ? Math.round((stepIndex / (flow.length - 1)) * 100) : 0;
  const interaction = (
    mode: "tap" | "search" | "type" | "custom",
    count: number,
  ) => recordEvent("selection_changed", mode, count);

  return (
    <section
      id="contribution-form"
      className={styles.experience}
      aria-labelledby="contribution-question"
    >
      ...
      <div className={styles.question} key={currentStep}>
        ...
        {currentStep === "purposes" ? (
          <AdaptiveSelector
            ...
            selectionLabel="Answers added"
            mode="multiple"
            value={draft.purposes}
            suggestions={purposes}
            onChange={(values) => patchDraft({ purposes: values })}
            onInteraction={interaction}
          />
        ) : null}
        {currentStep === "products" ? (
          <AdaptiveSelector
            ...
            hint={draft.kind === "routine" ? "Add one or more." : undefined}
            selectionLabel={
              draft.kind === "routine" ? "Products added" : "Product added"
            }
            mode={draft.kind === "routine" ? "multiple" : "single"}
        ...
        {currentStep === "retailer" ? (
          <AdaptiveSelector
            label={
              draft.kind === "store"
                ? "Which store do you trust?"
                : "Where did you buy it?"
            }
            selectionLabel="Store added"
            mode="single"
            value={draft.retailers}
            suggestions={retailers}
            onChange={(values) => patchDraft({ retailers: values })}
            onInteraction={interaction}
          />
        ) : null}
        {currentStep === "purchase" ? (
          <div className={styles.purchase}>
            <div>
              <p>Optional</p>
              <h2 id="contribution-question">What did you pay?</h2>
            </div>
            <label>
              <span>Price</span>
            ...
            <label>
              ...
              <input
                ...
                onChange={(event) =>
                  patchDraft({ purchaseDate: event.target.value || null })
                }
              />
            </label>
          </div>
        ...
        {currentStep === "outcome" ? (
          <AdaptiveSelector
            label="How was it?"
            hint="Your experience, not a medical claim."
            mode="single"
            value={outcomeValue}
            suggestions={outcomeOptions}
            allowCustom={false}
            searchable={false}
            onChange={(values) =>
              patchDraft({
                outcome:
                  (values[0]?.id as ContributionDraft["outcome"]) ?? null,
              })
            }
            onInteraction={interaction}
          />
        ...
        {currentStep === "review" ? (
          <div className={styles.review}>
            <div>
              <p>Almost done</p>
              <h2 id="contribution-question">Ready to share?</h2>
            ...
            <dl>
              <div>
                ...
                <dd>
                  {
                    kindOptions.find((option) => option.id === draft.kind)
                      ?.label
                  }
                </dd>
              </div>
              <div>
                <dt>For</dt>
                <dd>{joined(draft.purposes)}</dd>
              </div>
              {draft.products.length ? (
                <div>
                  <dt>{draft.kind === "routine" ? "Routine" : "Product"}</dt>
                  <dd>{joined(draft.products)}</dd>
                </div>
              ) : null}
              {draft.retailers.length ? (
                <div>
                  <dt>Store</dt>
                  <dd>{joined(draft.retailers)}</dd>
                </div>
              ) : null}
              {draft.priceNgn ? (
                <div>
                  <dt>Paid</dt>
                  <dd>₦{draft.priceNgn.toLocaleString("en-NG")}</dd>
                </div>
              ) : null}
            </dl>
          </div>
      ...
      <div className={styles.actions}>
        {stepIndex > 0 ? (
          <button type="button" className={styles.back} onClick={previousStep}>
        ...
        {currentStep === "review" ? (
          <button
            ...
            disabled={submitting}
          >
            {submitting ? "Sharing…" : "Share with JeloCare"}{" "}
