...
export function ContributionExperience({ purposes, products, brands, retailers }: ContributionExperienceProps) {
  ...
  return <section id="contribution-form" className={styles.experience} aria-labelledby="contribution-question">
    ...
    <div className={styles.question} key={currentStep}>
      ...
      {currentStep === 'purposes' ? <AdaptiveSelector label="What is it for?" hint="Choose every answer that fits." selectionLabel="Answers added" mode="multiple" value={draft.purposes} suggestions={purposes} onChange={values => patchDraft({ purposes: values })} onInteraction={interaction}/> : null}
      {currentStep === 'products' ? <AdaptiveSelector label={draft.kind === 'routine' ? 'What is in your routine?' : 'Which product?'} hint={draft.kind === 'routine' ? 'Add one or more.' : undefined} selectionLabel={draft.kind === 'routine' ? 'Products added' : 'Product added'} mode={draft.kind === 'routine' ? 'multiple' : 'single'} value={draft.products} suggestions={products} onChange={chooseProducts} onInteraction={interaction}/> : null}
      {currentStep === 'brand' ? <AdaptiveSelector label="Who makes it?" selectionLabel="Brand added" mode="single" value={draft.brands} suggestions={brands} onChange={values => patchDraft({ brands: values })} onInteraction={interaction}/> : null}
      {currentStep === 'retailer' ? <AdaptiveSelector label={draft.kind === 'store' ? 'Which store do you trust?' : 'Where did you buy it?'} selectionLabel="Store added" mode="single" value={draft.retailers} suggestions={retailers} onChange={values => patchDraft({ retailers: values })} onInteraction={interaction}/> : null}
