...
export function ProductRequestAddPage({
  ...
  const controller = useProductRequestAdd(viewModel, initialSearch);
  return (
    <section
      className={styles.requestPage}
      aria-labelledby="product-request-add-title"
    >
      <header className={styles.pageHeading}>
        <p>My Shelf · Private</p>
        <h1 id="product-request-add-title">Find it first.</h1>
        <span>
          A missing-product request stays private and never becomes a public or
          canonical product by itself.
        </span>
      </header>
      ...
      {controller.canOpenRequest && !controller.requestOpen ? (
        <div className={styles.requestGate}>
          ...
          <div>
            ...
            <p>
              Describe only what is printed or shown at the retailer.
              Researchers will review it privately.
            </p>
          </div>
          <button type="button" onClick={controller.openRequest}>
            Request this missing product
          </button>
        </div>
      ) : null}
        ...
        >
          <div className={sharedStyles.stepHeading}>
            ...
            <div>
              ...
              <p>
                These bounded fields preserve your wording as the original
                request.
              </p>
            </div>
          </div>
          ...
          controller.fields.printedSizeVariant.trim() ? (
            <section
              ...
              aria-labelledby="new-request-photo-title"
            >
              <div className={sharedStyles.stepHeading}>
                <span>03</span>
                <div>
                  <h2 id="new-request-photo-title">
                    A photo can stay private too.
                  </h2>
                  <p>
                    The upload and identification permission are deliberately
                    separate choices.
                  </p>
                </div>
              </div>
              ...
              <p className={styles.privacyNote}>
                ...
                Your request, retailer evidence, and photo remain private
                customer data. Publication requires a separate reviewed
                catalogue record.
              </p>
            </section>
          ) : null}
          ...
          <div className={styles.formFeedback} role="status" aria-live="polite">
            {controller.synthetic ? (
              <p>Development preview · request creation is read-only.</p>
            ) : null}
            ...
            {controller.canonicalSlug ? (
              <Link href={`/me/product/${controller.canonicalSlug}?from=shelf`}>
                Open exact catalogue product{" "}
                <ArrowRight size={15} aria-hidden="true" />
              </Link>
            ) : null}
            {controller.createdRequest ? (
              <Link
                href={`/me/shelf/request/${encodeURIComponent(controller.createdRequest.id)}`}
              >
                Open saved private request{" "}
                <ArrowRight size={15} aria-hidden="true" />
              </Link>
            ) : null}
          </div>
