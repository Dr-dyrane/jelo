...
function shelfItem(item: CustomerPortalShelfItem): MeContextSheetItem {
  return {
    ...
    href: item.product
      ? `/me/product/${item.product.slug}?from=shelf`
      : "/me/shelf",
  };
}
...
export function createMeContextSheetModel({
  ...
  if (route.kind === "home") {
    ...
    const concernPart =
      concernCount > 0 ? ` · ${count(concernCount, "concern")}` : "";
    const baseSummary = shelfAvailable
      ? `${shelfCount} · ${count(routineStepCount, "step")}`
      : `Shelf unavailable · ${count(routineStepCount, "step")}`;
    return {
      eyebrow: "At a glance",
      ...
      items: [
        {
          id: "shelf",
          label: "My Shelf",
          detail: shelfAvailable
            ? count(viewModel.shelf.length, "product")
            : "Unavailable",
          href: "/me/shelf",
        },
        {
          id: "routine",
          label: "My Routine",
          detail: count(routineStepCount, "step"),
          href: "/me/routine",
        },
      ],
    };
    ...
    return {
      ...
      items: [
        ...
        },
        {
          ...
          detail: "Search exact catalogue first",
          href: "/me/shelf/add",
        },
      ],
    };
      ...
      items: [
        {
          ...
          detail: "Guest-first checkout",
          href: "/products",
        },
        {
          id: "prices",
          label: "Price watch",
          detail: "Observed market prices",
          href: "/share",
        },
      ],
    };
  ...
  if (route.kind === "locations") {
    return {
      ...
      title: "My locations",
      summary: "Delivery and billing details",
      items: [
        {
          id: "orders",
          label: "My orders",
          detail: "Private order requests",
          href: "/me/orders",
        },
        {
          id: "products",
  ...
  const onShelf =
    shelfAvailable && product
      ? viewModel.shelf.some((item) => item.product?.slug === product.slug)
      : false;
  const inRoutine = product
    ? viewModel.routine.some((step) => step.product.slug === product.slug)
    : false;
  return {
    eyebrow: "My product",
    ...
    items: product
      ? [
          ...
          },
          {
            id: "routine",
            label: "My Routine",
            detail: inRoutine ? "Used here" : "My saved steps",
            href: "/me/routine",
          },
        ]
      : [],
