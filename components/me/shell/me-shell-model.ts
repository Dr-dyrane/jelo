...
export function resolveMeActiveParentHref(route: MePortalRoute): string {
  ...
  if (route.kind === "locations") return ME_PRIMARY_HREFS.home;
  if (route.kind === "not-found") return ME_PRIMARY_HREFS.explore;
  if (route.kind === "shelf-add" || route.kind === "shelf-request")
    return ME_PRIMARY_HREFS.shelf;
...
export function createMeDockContext({
  ...
  const label =
    ME_WORKSPACE_NAVIGATION.find((item) => item.id === page)?.label ??
