...
const channelOptions: AdaptiveOption[] = [
  {
    id: "channel:physical-store",
    label: "Physical store",
    detail: "Customers can walk in",
  },
  {
    id: "channel:website",
    label: "Website",
    detail: "Customers can order online",
  },
  {
    id: "channel:instagram",
    label: "Instagram",
    detail: "Orders begin in DMs",
  },
  { id: "channel:whatsapp", label: "WhatsApp", detail: "Orders begin in chat" },
  {
    id: "channel:facebook",
    label: "Facebook",
    detail: "Orders begin on your business page",
  },
  {
    id: "channel:marketplace",
    label: "Marketplace",
    detail: "You sell through another platform",
  },
];
...
export function RetailerPartnershipExperience({
  brands,
}: {
  brands: AdaptiveOption[];
}) {
  const [draft, setDraft] = useState<RetailerPartnershipDraft>(() =>
    emptyRetailerPartnershipDraft(),
  );
  ...
  const persistLocal = useCallback(
    (snapshot: RetailerPartnershipDraft) => {
  ...
  const restoreRemote = useCallback(async (applicationId: string) => {
    const response = await fetch(
      `/api/retailers/applications/${applicationId}`,
    );
    ...
    setStepIndex(
      Math.max(0, Math.min(parsed.data.currentStep - 1, flow.length - 1)),
    );
    ...
    async function restore() {
      ...
      try {
        const local = JSON.parse(
          window.localStorage.getItem(localKey) ?? "null",
        ) as {
          applicationId?: string;
          revision?: number;
          draft?: unknown;
        ...
        if (
          local?.applicationId &&
          (await restoreRemote(local.applicationId))
        ) {
  ...
  const saveSnapshot = useCallback(
    async (snapshot: RetailerPartnershipDraft) => {
  ...
  function patchDraft(patch: Partial<RetailerPartnershipDraft>) {
    setDraft((current) => ({
      ...current,
      ...patch,
      currentStep: Math.min(stepIndex + 1, 8),
    }));
    setError("");
  }
  ...
  function canContinue() {
    ...
    if (currentStep === "location")
      return draft.state.length === 1 && draft.city.trim().length >= 2;
    if (currentStep === "contact") {
      return (
        /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(draft.email) && draft.contactConsent
      );
    }
    return true;
  }
  ...
  async function resendLink() {
    ...
    setError("");
    const response = await fetch(
      `/api/retailers/applications/${applicationId}/send-link`,
      { method: "POST" },
    );
    if (response.ok) {
      setMailState("sent");
  ...
  async function submit() {
    const applicationId = applicationIdRef.current;
    const finalDraft = retailerPartnershipDraftSchema.safeParse({
      ...draft,
      currentStep: 8,
    });
    ...
    try {
      const response = await fetch(
        `/api/retailers/applications/${applicationId}/submit`,
        {
          method: "POST",
          headers: {
  ...
  if (!hydrated)
    return <div className={styles.partnershipLoading}>Loading…</div>;

  if (complete)
    return (
      <div className={styles.partnershipComplete}>
        <span>
        ...
        <button type="button" onClick={startAnother}>
          <RotateCcw size={16} aria-hidden="true" /> Add another store
        </button>
      </div>
    );
  ...
  return (
    <div className={styles.partnershipExperience}>
      <div className={styles.partnershipProgressRow}>
        <div
          className={styles.partnershipProgress}
        ...
        {saveState === "saved" ? (
          <span
            className={styles.partnershipSaveStatus}
            role="status"
            aria-live="polite"
          >
        ...
        {saveState === "error" ? (
          <span
            className={`${styles.partnershipSaveStatus} ${styles.partnershipSaveError}`}
            role="status"
            aria-live="polite"
          >
            Retrying…
      ...
      <div className={styles.partnershipQuestion} key={currentStep}>
        {currentStep === "store" ? (
          <div className={styles.textQuestion}>
            <p>Start here</p>
            <h3 id="retailer-partnership-question">
            ...
            <label>
              ...
              <input
                ...
                maxLength={120}
                onChange={(event) =>
                  patchDraft({ storeName: event.target.value })
                }
            ...
            <div className={styles.inlineFields}>
              ...
              <label>
                ...
                <input
                  ...
                  maxLength={240}
                  onChange={(event) =>
                    patchDraft({ address: event.target.value || null })
                  }
        ...
        {currentStep === "contact" ? (
          <div className={styles.textQuestion}>
            <p>Keep your place</p>
            <h3 id="retailer-partnership-question">
              Where should we send your private link?
            </h3>
            <div className={styles.inlineFields}>
              <label>
                <span>Email</span>
              ...
              <label>
                ...
                <input
                  ...
                  maxLength={40}
                  onChange={(event) =>
                    patchDraft({ whatsapp: event.target.value || null })
                  }
                  placeholder="+234…"
                />
              </label>
            ...
            <label className={styles.consent}>
              <input
                ...
                onChange={(event) =>
                  patchDraft({ contactConsent: event.target.checked })
                }
              />
            ...
            {applicationId ? (
              <div className={styles.mailNote}>
                ...
                <span>
                  {mailState === "sent"
                    ? `Private link sent to ${draft.email}`
                    : "Your store is saved."}
                </span>
        ...
        {currentStep === "online" ? (
          <div className={styles.textQuestion}>
            <p>Optional</p>
            <h3 id="retailer-partnership-question">
              Where can people see your store?
            </h3>
            <div className={styles.inlineFields}>
              <label>
                <span>Website</span>
                <input
                  ...
                  maxLength={240}
                  onChange={(event) =>
                    patchDraft({ website: event.target.value || null })
                  }
                  onBlur={(event) =>
                    patchDraft({
              ...
              <label>
                ...
                <input
                  ...
                  maxLength={120}
                  onChange={(event) =>
                    patchDraft({ instagram: event.target.value || null })
                  }
              ...
              <label>
                ...
                <input
                  ...
                  maxLength={240}
                  onChange={(event) =>
                    patchDraft({ facebook: event.target.value || null })
                  }
            ...
            {mailState === "sent" ? (
              <p className={styles.linkSent}>
                <Mail size={16} aria-hidden="true" /> Your private link is in
                your inbox.
              </p>
            ) : null}
          </div>
        ...
        {currentStep === "services" ? (
          <div className={styles.locationQuestion}>
            ...
            <div className={styles.inlineFields}>
              <label>
                <span>
                  One product <small>Optional</small>
                </span>
                <input
                  value={draft.sampleProduct ?? ""}
                  maxLength={240}
                  onChange={(event) =>
                    patchDraft({ sampleProduct: event.target.value || null })
                  }
                  placeholder="Product name and size"
                />
              </label>
              <label>
                <span>
                  Current price <small>Optional</small>
                </span>
                <span className={styles.partnershipMoney}>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      patchDraft({
                        samplePriceNgn: event.target.value
        ...
        {currentStep === "review" ? (
          <div className={styles.partnershipReview}>
            <p>Ready</p>
            ...
            <dl>
              ...
              <div>
                <dt>Where</dt>
                <dd>
                  {draft.city}, {joined(draft.state)}
                </dd>
              </div>
              <div>
                <dt>Sales</dt>
                <dd>{joined(draft.channels)}</dd>
              </div>
              {draft.brands.length ? (
                <div>
                  <dt>Brands</dt>
                  <dd>{joined(draft.brands)}</dd>
                </div>
              ) : null}
              {draft.website ? (
                <div>
              ...
              {draft.instagram ? (
                <div>
                  <dt>Instagram</dt>
                  <dd>{draft.instagram}</dd>
                </div>
              ) : null}
              {draft.facebook ? (
                <div>
                  <dt>Facebook</dt>
                  <dd>{draft.facebook}</dd>
                </div>
              ) : null}
            </dl>
          </div>
      ...
      {error ? (
        <p className={styles.partnershipError} role="alert">
          {error}
        </p>
      ) : null}
      ...
      <div className={styles.partnershipActions}>
        {stepIndex > 0 ? (
          <button
            type="button"
            className={styles.partnershipBack}
        ...
        {currentStep === "review" ? (
          <button
            type="button"
            className={styles.partnershipPrimary}
            onClick={submit}
            disabled={busy}
          >
            {busy ? "Sharing…" : "Share store"}{" "}
          ...
          >
            {currentStep === "contact" && !applicationId
              ? busy
                ? "Sending…"
