...
function formatAmount(value: number, market: Market) {
  return market === "NG"
    ? formatNaira.format(value)
    : formatDollars.format(value);
}
...
export function RetailerList({
  ...
  const ranked = useMemo(
    () => rankOffers(offers, market, undefined, preferences),
    [offers, market, preferences],
  );
  const liveTrust = (offer: Offer) =>
    nigeriaRetailers.find((r) => r.name === offer.retailer)?.trust ??
    offer.trust;
  ...
  const activeChannel =
    channel === "all" || channels.includes(channel) ? channel : "all";
  const filtered =
    activeChannel === "all"
      ? visible
      : visible.filter((offer) =>
          offerOrderChannels(offer).includes(activeChannel),
        );
  ...
  const lastCheckedAt =
    reading?.state === "priced" || reading?.state === "listing-only"
      ? reading.observedAt
      : null;
  ...
  const coverageNote =
    pricedStoreCount > 0 && listingCount > pricedStoreCount
  ...
  function renderOfferRow(offer: Offer, index: number) {
    ...
    const deliveryFee =
      offer.priceObservation?.landedCost === "excluded"
        ? observedDeliveryFee(offer, market)
        : null;
      ...
      >
        ...
        <span className="retailer-price">
          <span className="retailer-price-line">
            <strong>
              {price != null ? formatAmount(price, market) : "Check price"}
            </strong>
            {price != null ? (
              <PriceTrend
                movement={movement}
                subject={`${offer.retailer} price`}
              />
            ) : null}
          </span>
  ...
  return (
    <div className="retailer-panel">
      <div className="retailer-market">
        <span>
          <MapPin size={15} /> Shopping in
        </span>
        <div role="group" aria-label="Shopping market">
          <button
            className={market === "NG" ? "active" : ""}
            type="button"
            onClick={() => setMarket("NG")}
          >
            Nigeria
          </button>
          <button
      ...
      {listingCount ? (
        <div
          ...
          aria-label={`${market === "NG" ? "Nigeria" : "United States"} market summary`}
        >
          <span>
            <small>
              {priceBasis === "multi-source" ? "Lowest observed" : "Observed"}
            </small>
            <span className="market-price-line">
              <strong>
                {lowestPrice == null
                  ? "Pending"
                  : formatAmount(lowestPrice, market)}
              </strong>
            </span>
          </span>
          {typicalPrice != null && typicalPrice !== lowestPrice ? (
            <span>
              <small>Typical</small>
              <span className="market-price-line">
          ...
          <span>
            <small>Stores</small>
            <strong>{pricedStoreCount}</strong>
          </span>
          {lastCheckedAt ? (
            <span>
              <small>Checked</small>
              <strong>{shortDate(lastCheckedAt)}</strong>
            </span>
          ) : null}
          {savings ? (
            <div className="market-summary-notes">
              {savings ? (
                <span>Save {formatAmount(savings, market)}.</span>
              ) : null}
            </div>
          ) : null}
        </div>
      ...
      {coverageNote ? (
        <p className="market-summary-coverage">{coverageNote}</p>
      ) : null}

      {/* Sort controls — always visible when there are 2+ offers */}
      {filtered.length > 1 ? (
        <div
          className="retailer-market retailer-sort"
          role="group"
      ...
      {bestMatch ? (
        <div
          className="retailer-best-match"
          aria-label="JeloCare verified pick"
        >
      ...
      {showFilters && hasChannelOrFulfilmentFilters ? (
        <div
          id="retailer-advanced-filters"
          className="retailer-advanced-filters"
        >
          {channels.length > 1 ? (
            <div
              className="retailer-channel-filter"
              role="group"
            ...
            >
              ...
              {channels.map((value) => (
                ...
                >
                  {orderChannelLabel(value)}
                </button>
              ))}
            </div>
          ) : null}
          {fulfilments.length > 1 ? (
            <div className="retailer-market retailer-fulfilment">
              <span>
                <Truck size={15} /> Prefer
              </span>
              <div role="group" aria-label="Fulfilment preference">
                <button
                  className={activeFulfilment === "any" ? "active" : ""}
                  type="button"
                  onClick={() => setFulfilment("any")}
                >
                  Any
                </button>
                {fulfilments.map((method) => (
                  <button
      ...
      <div className="retailer-list">
        ...
        {displayed.length === 0 ? (
          <div className="retailer-empty">
            <p>No exact offer yet.</p>
            <button
              type="button"
              onClick={() => setMarket(market === "NG" ? "US" : "NG")}
            >
              Try {market === "NG" ? "United States" : "Nigeria"}
            </button>
          </div>
        ) : null}
      </div>
