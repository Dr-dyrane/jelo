...
import { ArrowUpRight, ChevronDown, Clock3, Moon, RefreshCw, ShieldAlert, Sparkles, Sun } from 'lucide-react';
...
type ClinicalFinding = { ruleId: string; severity: 'info' | 'caution' | 'high' | 'urgent'; title: string; explanation: string; action: 'allow' | 'adjust' | 'avoid' | 'escalate' };
type OptimizedStep = { time: 'Morning' | 'Evening' | 'Weekly'; action: string; rationale: string; frequency: string };
type Clinical = { profile?: Profile; findings: ClinicalFinding[]; blockedIngredientIds: string[]; activeLoad: { exfoliant: number; retinoid: number; antimicrobial: number; total: number }; optimizedRoutine: OptimizedStep[]; routineSummary?: string };
...
export function ConsultExperience() {
  ...
  async function submit(text: string) {
    ...
    const cacheKey = `jelocare:consult:v2:${market}:${query.toLowerCase().replace(/\s+/g, ' ')}:${fingerprint}`;
  ...
  if (result) return (
    <section className="consult-report" aria-live="polite">
      ...
      <div className="report-bottom-grid"><article className="report-panel caution-panel"><p className="eyebrow"><ShieldAlert size={14}/> Clinical checks</p>{result.clinical?.findings?.length ? <div className="finding-list">{result.clinical.findings.map(finding=><div className={`finding finding-${finding.severity}`} key={finding.ruleId}><strong>{finding.title}</strong><p>{finding.explanation}</p></div>)}</div> : <ul>{result.report.cautions.map(item=><li key={item}>{item}</li>)}</ul>}</article><article className="report-panel follow-panel"><p className="eyebrow">Follow-up</p><p>{result.report.followUp}</p></article></div>
      <p className="report-disclaimer">One AI generation was used for this report. Clinical safety checks and routine optimization were deterministic. Guidance, not diagnosis.</p>
    </section>
  );
