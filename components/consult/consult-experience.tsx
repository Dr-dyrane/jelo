...
import { ArrowUpRight, BookOpen, CalendarClock, ChevronDown, Clock3, Moon, RefreshCw, ShieldAlert, Sparkles, Sun } from 'lucide-react';
...
type Consultation = { report: Report; products: ConsultProduct[]; clinical?: Clinical; timeline?: TimelineRecord; meta: { modelCalls: number; market: 'NG' | 'US'; concerns: string[]; fallback?: boolean } };
...
export function ConsultExperience() {
  ...
  useEffect(() => {
    try { setTimeline(JSON.parse(window.localStorage.getItem(timelineKey) ?? '[]')); } catch { setTimeline([]); }
  }, []);
  ...
  async function submit(text: string) {
    ...
    const cacheKey = `jelocare:consult:v5:${market}:${query.toLowerCase().replace(/\s+/g, ' ')}:${fingerprint}`;
