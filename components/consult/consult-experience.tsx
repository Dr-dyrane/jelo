...
type Clinical = { profile?: Profile; findings: ClinicalFinding[]; evidence: Evidence[]; blockedIngredientIds: string[]; activeLoad: { exfoliant: number; retinoid: number; antimicrobial: number; total: number }; optimizedRoutine: OptimizedStep[]; routineSummary?: string };
...
export function ConsultExperience() {
  ...
  async function submit(text: string) {
    ...
    const cacheKey = `jelocare:consult:v3:${market}:${query.toLowerCase().replace(/\s+/g, ' ')}:${fingerprint}`;
  ...
  if (result) return (
    <section className="consult-report" aria-live="polite">
      ...
      <p className="report-disclaimer">One AI generation was used for this report. Clinical safety checks, evidence selection and routine optimization were deterministic. Guidance, not diagnosis.</p>
    </section>
  );
