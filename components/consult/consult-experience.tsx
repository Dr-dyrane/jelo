...
export function ConsultExperience() {
  ...
  async function submit(text: string) {
    ...
    try {
      const response = await fetch('/api/consult', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ query, market }) });
  ...
  if (result) return (
    <section className="consult-report" aria-live="polite">
      ...
      <div className="report-grid">
        ...
        <article className="report-panel"><p className="eyebrow">Routine</p><h3>Keep it deliberate.</h3><div className="routine-list">{result.report.routine.map((item,index)=><div className="routine-step" key={`${item.time}-${index}`}><span className="routine-icon"><RoutineIcon time={item.time}/></span><div><strong>{item.time}</strong><p>{item.action}</p></div></div>)}</div></article>
      </div>
