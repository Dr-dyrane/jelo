...
type ConsultProduct = { slug: string; brand: string; name: string; image: string; size: string; step: string; displayLine: string; price: { amount: number; currency: 'NGN' | 'USD'; retailer: string; market: 'NG' | 'US' } | null; clinicalMatch?: { reasons: string[]; ingredientIds: string[]; score: number }; retailers: { retailer: string; href: string }[] };
...
type Clinical = { profile?: Profile; findings: ClinicalFinding[]; evidence: Evidence[]; blockedIngredientIds: string[]; activeLoad: { exfoliant: number; retinoid: number; antimicrobial: number; total: number }; barrier: Barrier; optimizedRoutine: OptimizedStep[]; routineSummary?: string; eligibleProductCount?: number };
...
type Consultation = { report: Report; products: ConsultProduct[]; clinical?: Clinical; timeline?: TimelineRecord; timelineInsight?: TimelineInsight; recommendationAudit?: { candidateCount: number; selectedCount: number; deterministic: boolean }; meta: { modelCalls: number; market: 'NG' | 'US'; concerns: string[]; fallback?: boolean } };
...
function formatPrice(product: ConsultProduct) { if (!product.price) return 'Check stores'; return new Intl.NumberFormat(product.price.currency === 'NGN' ? 'en-NG' : 'en-US', { style: 'currency', currency: product.price.currency, maximumFractionDigits: product.price.currency === 'NGN' ? 0 : 2 }).format(product.price.amount); }
...
function saveTimeline(record?: TimelineRecord) { if (!record) return []; let current: TimelineRecord[] = []; try { current = JSON.parse(window.localStorage.getItem(timelineKey) ?? '[]'); } catch { current = []; } const next = [record, ...current.filter(item => item.id !== record.id)].slice(0, 24); window.localStorage.setItem(timelineKey, JSON.stringify(next)); return next; }
...
export function ConsultExperience() {
  ...
  if (result) {
    ...
    return <section className="consult-report" aria-live="polite">
      ...
      {result.products.length ? <section className="report-products"><div className="report-section-heading"><div><p className="eyebrow">Clinically eligible matches</p><h3>Start with less.<br/>Choose well.</h3></div><span>{result.recommendationAudit?.deterministic ? 'Safety-filtered before AI selection' : result.meta.market === 'US' ? 'United States offers' : 'Nigeria offers'}</span></div><div className="consult-product-grid">{result.products.map(product=><article className="consult-product" key={product.slug}><Link className="consult-product-image" href={`/products/${product.slug}`}><SafeProductImage src={product.image} alt={`${product.brand} ${product.name}`}/><span className="consult-product-price">{formatPrice(product)}</span><span className="consult-product-arrow"><ArrowUpRight size={19}/></span></Link><div className="consult-product-copy"><p className="eyebrow">{product.brand}</p><h4><Link href={`/products/${product.slug}`}>{product.name}</Link></h4><p>{product.step} · {product.size}</p>{product.clinicalMatch?.reasons.length ? <div className="product-match-reasons">{product.clinicalMatch.reasons.slice(0,2).map(reason=><span key={reason}>{reason}</span>)}</div> : null}<div className="consult-retailers">{product.retailers.map(offer=><a key={offer.retailer} href={offer.href}>{offer.retailer}<ArrowUpRight size={14}/></a>)}</div></div></article>)}</div></section> : null}
      ...
      <p className="report-disclaimer">One AI generation was used for this report. Clinical safety checks, product eligibility, barrier scoring, timeline trend analysis, evidence selection and routine optimization were deterministic. Guidance, not diagnosis.</p>
    </section>;
  }
